# ApiZero 接口详细文档

生成时间：`2026-07-06T08:41:35+00:00`  
接口总数：`105`

这份文档给人和 AI 一起用：每个接口都包含用途、endpoint、计费/QPS、免费额度、请求参数、请求体字段、响应示例和可复制 cURL 模板。

## 快速索引

| # | slug | 名称 | 方法 | 分类 | 增长层级 | 必填 query | 跳转 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| 1 | video-parse | 全平台视频元数据解析服务 | GET | 内容娱乐 | C-caution | url | [详情](#video-parse) |
| 2 | barcode-gs1 | 商品条码查询PRO | GET | 生活服务 | A-verify | code | [详情](#barcode-gs1) |
| 3 | barcode-lookup | 商品条码查询-免费版 | GET | 生活服务 | A-core | barcode | [详情](#barcode-lookup) |
| 4 | hot-baidu | 百度热搜榜 | GET | 生活服务 | B-support |  | [详情](#hot-baidu) |
| 5 | weather | 彩云天气 | GET | 生活服务 | A-core |  | [详情](#weather) |
| 6 | movie-box | 实时电影票房 | GET | 生活服务 | B-support |  | [详情](#movie-box) |
| 7 | whois | Whois 域名查询 | GET | 开发工具 | B-support | domain | [详情](#whois) |
| 8 | qq | QQ 信息 | GET | 生活服务 | A-core | qq | [详情](#qq) |
| 9 | ip-pro | IP 地址查询（街道级） | GET | 地理位置 | D-inventory |  | [详情](#ip-pro) |
| 10 | shici | 随机诗词 | POST | 内容娱乐 | D-inventory |  | [详情](#shici) |
| 11 | company-search | 企业工商信息查询 | GET | 身份核验 | D-inventory | name | [详情](#company-search) |
| 12 | moji-weather | 墨迹天气 | GET | 生活服务 | A-core |  | [详情](#moji-weather) |
| 13 | riddle | 谜语大全 | POST | 内容娱乐 | D-inventory |  | [详情](#riddle) |
| 14 | exchange-rate | 实时汇率查询 | GET | 金融数据 | D-inventory |  | [详情](#exchange-rate) |
| 15 | fund | 基金估值跟踪 | GET | 金融数据 | B-support | action | [详情](#fund) |
| 16 | hitokoto | 一言 · 经典语录 | GET | 生活服务 | D-inventory |  | [详情](#hitokoto) |
| 17 | netease-comment | 网易云热门乐评 | POST | 内容娱乐 | D-inventory |  | [详情](#netease-comment) |
| 18 | wzry | 王者战力查询 | GET | 生活服务 | D-inventory | action | [详情](#wzry) |
| 19 | content-moderation | 内容审核 | POST | 内容娱乐 | D-inventory |  | [详情](#content-moderation) |
| 20 | brain-teaser | 脑筋急转弯 | GET | 生活服务 | D-inventory |  | [详情](#brain-teaser) |
| 21 | oil-price-forecast | 今日油价 | GET | 生活服务 | D-inventory |  | [详情](#oil-price-forecast) |
| 22 | gold | 黄金价格查询 | GET | 金融数据 | D-inventory |  | [详情](#gold) |
| 23 | translate | 文本翻译 | GET | 内容娱乐 | D-inventory | q | [详情](#translate) |
| 24 | ocr-text | OCR 文字识别 | POST | 文档识别 | D-inventory |  | [详情](#ocr-text) |
| 25 | baby-naming | 八字起名 | POST | 生活服务 | D-inventory |  | [详情](#baby-naming) |
| 26 | express | 快递物流查询 | GET | 生活服务 | D-inventory | number | [详情](#express) |
| 27 | image-enhance | AI 图片变清晰 | POST | AI 能力 | D-inventory |  | [详情](#image-enhance) |
| 28 | tts | TTS 语音合成 | POST | AI 能力 | D-inventory |  | [详情](#tts) |
| 29 | wzry-battle | 王者荣耀战绩查询 | POST | 内容娱乐 | D-inventory |  | [详情](#wzry-battle) |
| 30 | crazy-thursday | 疯狂星期四文案 | GET | 生活服务 | D-inventory |  | [详情](#crazy-thursday) |
| 31 | content-extract | 网页正文提取 | GET | 开发工具 | B-support | url | [详情](#content-extract) |
| 32 | visits-counter | 访问量计数器 | GET | 开发工具 | D-inventory |  | [详情](#visits-counter) |
| 33 | doubao-image | 豆包图片生成 | POST | AI 能力 | D-inventory |  | [详情](#doubao-image) |
| 34 | dns-check | DNS 劫持检测 | GET | 开发工具 | D-inventory | domain | [详情](#dns-check) |
| 35 | email-check | 邮箱地址检测 | GET | 身份核验 | D-inventory | email | [详情](#email-check) |
| 36 | bus-realtime | 实时公交到站 | POST | 生活服务 | B-support |  | [详情](#bus-realtime) |
| 37 | wallpaper | 随机壁纸 | GET | 生活服务 | D-inventory |  | [详情](#wallpaper) |
| 38 | text-similarity | 文本相似度 | POST | 开发工具 | D-inventory |  | [详情](#text-similarity) |
| 39 | unshort | 短链还原 | GET | 开发工具 | D-inventory | url | [详情](#unshort) |
| 40 | risk-score | 综合风控评分 | POST | 身份核验 | D-inventory |  | [详情](#risk-score) |
| 41 | nine-grid-cutter | 九宫格切图 | POST | 开发工具 | D-inventory |  | [详情](#nine-grid-cutter) |
| 42 | mobile | 手机号归属地 | GET | 生活服务 | D-inventory | mobile | [详情](#mobile) |
| 43 | icp | ICP 备案查询 | GET | 开发工具 | D-inventory | domain | [详情](#icp) |
| 44 | bencao | 本草纲目·中药查询 | GET | 生活服务 | D-inventory | msg | [详情](#bencao) |
| 45 | address-parse | 中文地址解析 | POST | 地理位置 | D-inventory |  | [详情](#address-parse) |
| 46 | hash | 哈希加密计算 | GET | 开发工具 | D-inventory | text | [详情](#hash) |
| 47 | yiyan | 一言（简版） | GET | 生活服务 | D-inventory |  | [详情](#yiyan) |
| 48 | jd-address | 京东地址解析（4 级） | POST | 地理位置 | D-inventory |  | [详情](#jd-address) |
| 49 | holiday | 中国法定节假日 | GET | 生活服务 | D-inventory |  | [详情](#holiday) |
| 50 | invoice | 增值税发票识别 | POST | 文档识别 | D-inventory |  | [详情](#invoice) |
| 51 | douyin-user | 抖音用户公开信息 | GET | 内容娱乐 | D-inventory | url | [详情](#douyin-user) |
| 52 | dns-query | DNS 记录查询 | GET | 开发工具 | D-inventory | host | [详情](#dns-query) |
| 53 | cdn-ips | CDN 优选 IP | GET | 开发工具 | D-inventory | server, type | [详情](#cdn-ips) |
| 54 | webmeta | 网页元数据提取 | GET | 开发工具 | B-support | url | [详情](#webmeta) |
| 55 | bodyfat | 体脂率与 BMI 计算 | GET | 生活服务 | D-inventory | weight, height, waist, gender | [详情](#bodyfat) |
| 56 | traffic-weather-alert | 限行天气联动 | GET | 生活服务 | D-inventory | city | [详情](#traffic-weather-alert) |
| 57 | ssl | SSL 证书检测 | GET | 开发工具 | B-support | domain | [详情](#ssl) |
| 58 | idcard-region | 身份证归属地查询 | GET | 身份核验 | D-inventory | idcard | [详情](#idcard-region) |
| 59 | blood-type | 血型遗传查询 | GET | 生活服务 | D-inventory | father, mother | [详情](#blood-type) |
| 60 | browser-fingerprint | 浏览器指纹风控 | POST | 开发工具 | D-inventory |  | [详情](#browser-fingerprint) |
| 61 | cf-dns | Cloudflare DNS 更新（DDNS） | POST | 开发工具 | D-inventory |  | [详情](#cf-dns) |
| 62 | bank-card | 银行卡BIN查询 | GET | 金融数据 | D-inventory |  | [详情](#bank-card) |
| 63 | food-license | 食品经营许可证识别 | POST | 文档识别 | D-inventory |  | [详情](#food-license) |
| 64 | douban-movie | 豆瓣电影信息 | GET | 内容娱乐 | D-inventory | id | [详情](#douban-movie) |
| 65 | text-censor | 文本审核 | POST | 内容娱乐 | D-inventory |  | [详情](#text-censor) |
| 66 | earthquake | 全球地震速报 | POST | 生活服务 | D-inventory |  | [详情](#earthquake) |
| 67 | ocr-idcard | 身份证识别 | POST | 文档识别 | D-inventory |  | [详情](#ocr-idcard) |
| 68 | site-check | 网站测速诊断 | GET | 开发工具 | D-inventory | url | [详情](#site-check) |
| 69 | code-beautify | 代码美化图片 | POST | 开发工具 | D-inventory |  | [详情](#code-beautify) |
| 70 | desensitize | 数据脱敏（敏感信息掩码） | POST | 内容娱乐 | D-inventory |  | [详情](#desensitize) |
| 71 | ios-cert | iOS 证书与描述文件检测 | POST | 开发工具 | D-inventory |  | [详情](#ios-cert) |
| 72 | csdn-profile | CSDN 博主信息 | GET | 内容娱乐 | D-inventory | username | [详情](#csdn-profile) |
| 73 | site-security | 网站安全综合评分 | GET | 开发工具 | D-inventory | domain | [详情](#site-security) |
| 74 | company-profile | 企业档案深度查询 | POST | 身份核验 | D-inventory |  | [详情](#company-profile) |
| 75 | bili-dynamic | B 站用户动态 | GET | 内容娱乐 | D-inventory | uid | [详情](#bili-dynamic) |
| 76 | domain-trade | 域名交易市场 | GET | 金融数据 | D-inventory |  | [详情](#domain-trade) |
| 77 | oil-price | 全国油价 | POST | 生活服务 | D-inventory |  | [详情](#oil-price) |
| 78 | soul-soup | 心灵毒鸡汤 | POST | 内容娱乐 | D-inventory |  | [详情](#soul-soup) |
| 79 | dns-hijack | DNS 劫持检测 | GET | 开发工具 | D-inventory | domain | [详情](#dns-hijack) |
| 80 | idcard-organ | 身份证签发机关查询 | GET | 身份核验 | D-inventory | id | [详情](#idcard-organ) |
| 81 | idcard-2c | 身份证二要素核验 | POST | 身份核验 | D-inventory |  | [详情](#idcard-2c) |
| 82 | tencent-weather | 腾讯天气 | POST | 生活服务 | D-inventory |  | [详情](#tencent-weather) |
| 83 | typhoon | 台风实时路径 | POST | 生活服务 | D-inventory |  | [详情](#typhoon) |
| 84 | business-license | 营业执照识别 | POST | 文档识别 | D-inventory |  | [详情](#business-license) |
| 85 | ocr-vehicle-invoice | 机动车发票识别 | POST | 文档识别 | D-inventory |  | [详情](#ocr-vehicle-invoice) |
| 86 | wechat-archive | 微信文章转存 | POST | 内容娱乐 | D-inventory |  | [详情](#wechat-archive) |
| 87 | app-store | App Store 查询 | POST | 开发工具 | D-inventory |  | [详情](#app-store) |
| 88 | stock-trend | A股实时行情 | POST | 金融数据 | D-inventory |  | [详情](#stock-trend) |
| 89 | air-history | 历史空气质量 | POST | 生活服务 | D-inventory |  | [详情](#air-history) |
| 90 | mingyan | 名人名言 | POST | 内容娱乐 | D-inventory |  | [详情](#mingyan) |
| 91 | image-nsfw | 图片鉴黄 | POST | AI 能力 | D-inventory |  | [详情](#image-nsfw) |
| 92 | 2fa | 2FA 动态验证码 | POST | 开发工具 | D-inventory |  | [详情](#2fa) |
| 93 | carrier-3c | 运营商三要素核验 | POST | 身份核验 | D-inventory |  | [详情](#carrier-3c) |
| 94 | ocr-vehicle-license | 行驶证识别 | POST | 文档识别 | D-inventory |  | [详情](#ocr-vehicle-license) |
| 95 | car-owner-check | 车牌车主核验 | POST | 身份核验 | D-inventory |  | [详情](#car-owner-check) |
| 96 | driving-license | 驾驶证识别 | POST | 文档识别 | D-inventory |  | [详情](#driving-license) |
| 97 | ocr-bank-card | 银行卡识别 | POST | 文档识别 | D-inventory |  | [详情](#ocr-bank-card) |
| 98 | ocr-cn-passport | 中国护照识别 | POST | 文档识别 | D-inventory |  | [详情](#ocr-cn-passport) |
| 99 | ocr-health-cert | 健康证识别 | POST | 文档识别 | D-inventory |  | [详情](#ocr-health-cert) |
| 100 | ocr-train-ticket | 火车票识别 | POST | 文档识别 | D-inventory |  | [详情](#ocr-train-ticket) |
| 101 | hot-search | 全网热搜聚合 | POST | 内容娱乐 | D-inventory |  | [详情](#hot-search) |
| 102 | ocr-taxi-invoice | 出租车发票识别 | POST | 文档识别 | D-inventory |  | [详情](#ocr-taxi-invoice) |
| 103 | ocr-tw-permit | 台胞证识别 | POST | 文档识别 | D-inventory |  | [详情](#ocr-tw-permit) |
| 104 | bili-danmaku | B站弹幕分析 | POST | 内容娱乐 | D-inventory |  | [详情](#bili-danmaku) |
| 105 | dns-lookup | DNS 查询 | POST | 开发工具 | B-support |  | [详情](#dns-lookup) |

<a id="video-parse"></a>

## 1. 全平台视频元数据解析服务 `video-parse`

```
全平台视频元数据解析服务 — 专业、合规、稳定

## 服务概述
本服务通过 RESTful API 形式，将用户已能合法访问的视频/图集分享链接，
解析为结构化的元数据信息（标题、封面、作者、原始 URL 等），
便于开发者集成到内容审核、个人备份、学术研究、数据迁移等场景。

## 核心能力
- **覆盖广**：国内主流平台全覆盖（抖音、小红书、哔哩哔哩、快手、微博、
  皮皮虾、最右、贴吧、即梦、可灵 AI 等），海外平台 5 站智能路由
  （YouTube、Vimeo、Twitter 等通过 yt-dlp 兜底）
- **响应快**：智能缓存 + 多通道竞速，平均响应 1.3 秒，QPS 可达 15
- **稳定性**：99.7% 月度可用性（含失败重试与自动降级）
- **合规优先**：
  · 强制在每个响应中返回 source 来源标注字段
  · 不存储任何原始视频/图片内容
  · 完善的 DMCA 侵权处理通道（24 小时响应）
  · 90 天日志保留期，超期自动清理

## 适用场景
✅ 个人内容备份（用户备份自己发布或合法授权的内容）
✅ MCN 机构内容审核（对委托发布的内容进行合规审查）
✅ 学术研究（高校研究机构的传播规律分析）
✅ 合法范围内的市场数据聚合分析
✅ 用户在自有多账号间的内容迁移

## 严禁用途（违反者立即终止服务且不予退款）
❌ 公开传播、二次发布他人受版权保护的内容
❌ 集成到视频下载站、聚合下载工具等以下载为主功能的产品
❌ 大规模爬取、自动化抓取
❌ 用于侵犯他人著作权、肖像权、隐私权的任何用途
❌ 二次商业转售本服务的解析结果

## 调用即同意《用户协议与免责声明》
完整协议：如下免责申明
侵权投诉：1790643379@qq.com
```

新增支持豆包、千问内容分享解析：豆包视频（无水印直链 + 封面）、豆包对话图片、千问图片，按链接自动识别。

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/video-parse` |
| Method | `GET` |
| 分类 | 内容娱乐 (`content`) |
| 计费 | 按次付费 · 月套餐 · 单次 0 积分 |
| QPS | 3 / s |
| 每日免费额度 | 登录用户 20 次；匿名 5 次 |
| 增长层级 | C-caution |
| 推广动作 | 只做排错/合规指南 |
| 推广角度 | 错误率高，暂不作为拉新入口；只讲合法使用、常见错误和可测试输入。 |
| 官网文档 | https://apizero.cn/aidocs/video-parse |
| Marketplace | https://apizero.cn/marketplace/video-parse |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Query 参数

| 参数 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| url | string | 是 | 待解析的视频/图文链接。支持完整 URL 或分享短链（如 v.douyin.com/xxx、vm.tiktok.com/xxx）。最大 2048 字符（同时支持豆包 doubao.com、千问 qianwen.com 分享链接） | https://www.bilibili.com/video/BV1gY411A7y7 |
| flat | number | 否 | 响应结构模式：0=双层 data（默认，兼容旧版）；1=单层 data（推荐，将内层字段直接提升到 data 顶层） | 1 |

### cURL 模板

```bash
curl -sS \
  -X GET \
  -H "X-API-Key: $APIZERO_API_KEY" \
  "https://v1.apizero.cn/api/video-parse?url=https://www.bilibili.com/video/BV1gY411A7y7"
```

<a id="barcode-gs1"></a>

## 2. 商品条码查询PRO `barcode-gs1`

官方权威的商品条码查询接口，数据直通中国物品编码中心官方注册数据库。
与 `barcode-lookup` 定位不同：
- 本接口**官方权威**：返回厂商注册名称、产品登记信息、上市日期、官方商品图等可追溯的官方数据
- 本接口**仅覆盖国内注册条码**(6/690 开头)，进口商品或非注册条码会返回 found=false
- 适合合规核验、溯源、上架审核等对数据权威性要求高的场景

支持输入格式：8 / 12 / 13 / 14 位纯数字，或 16 位 AI(01) 前缀 + GTIN-14。

返回数据包含：产品名称、品牌、通用名、分类（含分类编号）、规格、净含量、上市日期、厂商企业名、官方商品图、条码激活日期、产品创建日期、企业注册日期、已用天数等核心字段，部分情况下额外提供生产国、企业地址、参考售价、厂商识别代码等扩展字段。

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/barcode-gs1` |
| Method | `GET` |
| 分类 | 生活服务 (`life`) |
| 计费 | 按次付费 · 月套餐 · 单次 0 积分 |
| QPS | 2 / s |
| 每日免费额度 | 登录用户 5 次；匿名 2 次 |
| 增长层级 | A-verify |
| 推广动作 | 验证后主推 |
| 推广角度 | PRO 商品条码查询；发文前先用真实条码验证，降低新用户复制失败率。 |
| 官网文档 | https://apizero.cn/aidocs/barcode-gs1 |
| Marketplace | https://apizero.cn/marketplace/barcode-gs1 |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Query 参数

| 参数 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| code | string | 是 | 商品条形码(8/12/13/14 位纯数字，或 16 位 AI(01) 前缀 + GTIN-14) | 6921168509256 |

### Header 参数

| Header | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| Authorization | string | 否 | 登录用户传 API Key 享用更高额度；匿名调用每天 20 次免费 |  |

### cURL 模板

```bash
curl -sS \
  -X GET \
  -H "X-API-Key: $APIZERO_API_KEY" \
  "https://v1.apizero.cn/api/barcode-gs1?code=6921168509256"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "barcode": "6907992700199",
    "gtin14": "06907992700199",
    "found": true,
    "registered": true,
    "registration_message": "该商品条码已经在中国物品编码中心注册，编码信息已按规定通报。",
    "name": "伊利儿童奶酪棒香草冰淇淋味再制干酪",
    "brand": "伊利",
    "general_name": "奶酪（易腐坏）",
    "feature": "伊利儿童奶酪棒香草冰淇淋味再制干酪",
    "category": "奶酪（易腐坏）(10000028)",
    "specification": "90克",
    "net_content": "90克",
    "sale_date": null,
    "manufacturer": "内蒙古伊利实业集团股份有限公司",
    "images": [
      "https://www.gds.org.cn/userfile/uploada/gra/sj201210093039959369/06907992700199/06907992700199.1.jpg"
    ],
    "qr_active_date": null,
    "product_create_date": "2019年11月18日",
    "company_register_date": null,
    "use_days": 2366,
    "country": null,
    "address": null,
    "price": null,
    "category_code": null
  },
  "request_id": "mp0vcq0fab827132"
}
```

<a id="barcode-lookup"></a>

## 3. 商品条码查询-免费版 `barcode-lookup`

通过商品条形码（EAN-13 / UPC-A / UPC-E / EAN-8）一键查询商品信息，覆盖国内日常消费品超千万 SKU，包括食品饮料、日用百货、个护美妆、酒水烟草、母婴用品等。

【核心字段】
- name 商品名称
- brand 品牌
- manufacturer 生产厂商 / 经销商
- spec 规格（如 550ml / 250g / 6 听装）
- price 参考价（人民币元，仅作参考非实时市场价）
- image 商品图片 URL（自有 CDN 代理，跨域友好）

【典型场景】
- 电商 / 零售 ERP：扫码录入商品自动补全字段
- 个人记账 App：扫码识物展示购物明细
- 物流仓储：条码核销 + 商品图展示
- 自动售货机：条码识别后获取商品名称
- 营销活动：用户扫码核销福利时验证商品

【调用示例】
GET /api/barcode-lookup?barcode=6921168509256

【特别说明】
- 平均响应 100ms（不含图片下载）
- 国内主流商品（农夫山泉、可口可乐、康师傅、伊利等）覆盖率 > 95%
- 个别冷门 / 新上市 SKU 可能未收录，此时 found=false
- price 字段仅作参考，非各电商平台实时价格
- image 字段始终返回有效 URL（接图片 onError 即可降级到默认图标）

【⚠️ 注意】
图片资源为公开懒加载代理，不计入接口调用次数；商品文本数据为按次计费。

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/barcode-lookup` |
| Method | `GET` |
| 分类 | 生活服务 (`life`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 2 / s |
| 每日免费额度 | 登录用户 200 次；匿名 20 次 |
| 增长层级 | A-core |
| 推广动作 | 主推 |
| 推广角度 | 免费商品条码查询；适合库存录入、电商资料补全、扫码工具。 |
| 官网文档 | https://apizero.cn/aidocs/barcode-lookup |
| Marketplace | https://apizero.cn/marketplace/barcode-lookup |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Query 参数

| 参数 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| barcode | string | 是 | 8~13 位纯数字条形码。支持 EAN-13、UPC-A、EAN-8、UPC-E 等主流标准。 | 6921168509256 |
| mode | string | 否 | 保留参数：mode=image 时返回该商品的图片二进制（不计费、公开访问，主要由响应中的 image 字段链接自动调用）。 | image |

### Header 参数

| Header | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| Authorization | string | 否 | 可选 API Key 鉴权。未登录每天有 20 次免费体验额度；登录用户每天 200 次免费 |  |

### cURL 模板

```bash
curl -sS \
  -X GET \
  -H "X-API-Key: $APIZERO_API_KEY" \
  "https://v1.apizero.cn/api/barcode-lookup?barcode=6921168509256"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "barcode": "6921168509256",
    "found": true,
    "name": "农夫山泉 饮用天然水550ml",
    "brand": "农夫山泉",
    "manufacturer": "农夫山泉股份有限公司",
    "spec": "550ml",
    "price": 1.5,
    "category": null,
    "description": null,
    "image": "https://v1.apizero.cn/api/barcode-lookup?mode=image&barcode=6921168509256"
  },
  "request_id": "mqx8x12345abc"
}
```

<a id="hot-baidu"></a>

## 4. 百度热搜榜 `hot-baidu`

聚合百度搜索热榜数据，实时返回 4 大榜单的 Top 30~50 条热门内容。零成本调用，适合做内容聚合 / 热点追踪 / 资讯类产品。

【支持榜单】
- realtime（实时热搜）· 默认值，约 30~50 条
- novel（小说）· 约 30 条，含作者
- movie（电影）· 约 10 条，含类型
- tv（电视剧）· 约 10 条，含演员

【返回字段】
- rank · 榜单排名
- title · 标题
- desc · 简短描述
- hot_index · 热度指数（数字）
- link · 详情链接（百度搜索）
- image · 配图 URL（直接可用，无防盗链限制）
- tag · 标签（如「热」「新」「沸」「爆」）
- author / category / actors · 小说 / 电影 / 电视剧专属字段

【典型场景】
- 资讯类 App 的"今日热点"模块
- AI 写作助手的"实时热点选题"参考
- 数据看板 / 大屏展示
- 微信公众号选题辅助
- 热搜监控 / 舆情分析

【调用示例】
GET /api/hot-baidu                  · 默认实时热搜
GET /api/hot-baidu?tab=realtime
GET /api/hot-baidu?tab=novel
GET /api/hot-baidu?tab=movie
GET /api/hot-baidu?tab=tv

【性能与缓存】
- 平均响应 200ms（缓存命中时 < 50ms）
- 后端 60 秒缓存（百度榜单本身分钟级更新，短缓存合理且节省调用方资源）

【⚠️ 数据来源说明】
本接口数据来源于百度搜索热榜公开页面（top.baidu.com），仅用于个人开发测试与小规模商业场景；如需大规模商业使用建议自建抓取或采购官方 API。

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/hot-baidu` |
| Method | `GET` |
| 分类 | 生活服务 (`life`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 10 / s |
| 每日免费额度 | 登录用户 10000 次；匿名 1000 次 |
| 增长层级 | B-support |
| 推广动作 | 补充推广 |
| 推广角度 | 百度热搜 API；适合公众号、AI 写作工具、热点选题池。 |
| 官网文档 | https://apizero.cn/aidocs/hot-baidu |
| Marketplace | https://apizero.cn/marketplace/hot-baidu |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Query 参数

| 参数 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| tab | string | 否 | 榜单类型：realtime（实时热搜，默认）/ novel（小说）/ movie（电影）/ tv（电视剧） | realtime |

### Header 参数

| Header | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| Authorization | string | 否 | 可选 API Key 鉴权。未登录每天 50 次免费体验额度；登录用户更高额度且无需 IP 限制 |  |

### cURL 模板

```bash
curl -sS \
  -X GET \
  -H "X-API-Key: $APIZERO_API_KEY" \
  "https://v1.apizero.cn/api/hot-baidu"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "tab": "realtime",
    "update_time": "2026-05-06 07:15:00",
    "total": 30,
    "list": [
      {
        "rank": 1,
        "title": "总书记对青年学子的殷切期待",
        "desc": "坚定不移走中国特色社会主义道路...",
        "hot_index": 4982315,
        "link": "https://www.baidu.com/s?wd=%E6%80%BB%E4%B9%A6%E8%AE%B0%E5%AF%B9%E9%9D%92%E5%B9%B4...",
        "image": "https://fyb-2.cdn.bcebos.com/hotboard_image/ced2dbd68e9ca16f8fc405530a0a3a5d?x-bce-process=image/resize,m_fill,w_256,h_170",
        "tag": "热"
      },
      {
        "rank": 2,
        "title": "...",
        "desc": "...",
        "hot_index": 3500000,
        "link": "https://www.baidu.com/s?wd=...",
        "image": "https://fyb-2.cdn.bcebos.com/hotboard_image/...",
        "tag": "新"
      }
    ]
  },
  "request_id": "mqx8x12345abc"
}
```

<a id="weather"></a>

## 5. 彩云天气 `weather`

基于彩云天气 v2.6 的全功能天气接口，覆盖：
  • 实时天气（温度/湿度/风/能见度/AQI/PM2.5）
  • 分钟级降水预报（未来 2 小时）
  • 小时预报（最长 360 小时 = 15 天）
  • 天预报（最长 15 天，含日出日落、温度极值、生活指数）
  • 综合数据（一次返回全部 + 气象预警）

特色：
  • 支持城市名直接查询（内置 ~140 个常用中国城市表 + Open-Meteo 兜底，无需高德/百度 Key）
  • 自动中文化：skycon 转中文、风向转 16 方位、风速转蒲福风级、AQI 转等级颜色
  • 预警标准化：从原始 title 提取颜色（蓝/黄/橙/红）和级别（一般/较重/严重/特别严重）
  • 顶层 summary 字段提供"一眼看懂"的实时摘要

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/weather` |
| Method | `GET` |
| 分类 | 生活服务 (`life`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 10 / s |
| 每日免费额度 | 登录用户 2000 次；匿名 100 次 |
| 增长层级 | A-core |
| 推广动作 | 主推 |
| 推广角度 | 免费天气 API；城市名直接查；适合小程序、网站天气模块、日报脚本。 |
| 官网文档 | https://apizero.cn/aidocs/weather |
| Marketplace | https://apizero.cn/marketplace/weather |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Query 参数

| 参数 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| type | string | 否 | 查询类型：realtime / minutely / hourly / daily / weather（默认 weather 综合） |  |
| city | string | 否 | 城市/地区名称（如「北京」「朝阳区」），与 location 二选一 |  |
| location | string | 否 | 经纬度坐标，格式 经度,纬度（如 116.3975,39.9085），优先级高于 city |  |
| alert | string | 否 | 是否包含气象预警，true/false，默认 true（仅 realtime/weather 生效） |  |
| days | number | 否 | 天预报步长，1~15，默认 5（仅 daily/weather 生效） |  |
| hours | number | 否 | 小时预报步长，1~360，默认 24（仅 hourly/weather 生效） |  |

### Header 参数

| Header | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| X-API-Key | string | 否 | API Key（不传走匿名额度） |  |

### cURL 模板

```bash
curl -sS \
  -X GET \
  -H "X-API-Key: $APIZERO_API_KEY" \
  "https://v1.apizero.cn/api/weather?city=北京&type=weather"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "type": "weather",
    "location": {
      "city": "北京",
      "longitude": 116.4074,
      "latitude": 39.9042,
      "timezone": "Asia/Shanghai"
    },
    "server_time": "2026-05-06 08:43:01",
    "forecast_keypoint": "未来两小时不会下雨，放心出门",
    "summary": {
      "skycon": "小雨",
      "skycon_emoji": "🌦️",
      "skycon_code": "LIGHT_RAIN",
      "temperature": 21.5,
      "apparent_temperature": 21.8,
      "humidity_percent": 55,
      "cloudrate_percent": 100,
      "visibility_km": 20.3,
      "wind": {
        "speed_ms": 3.7,
        "direction_deg": 239.9,
        "direction_text": "西南偏南风",
        "level": 3,
        "level_text": "微风"
      },
      "air_quality": {
        "aqi": 68,
        "level": "良",
        "level_color": "yellow",
        "pm25": 27
      },
      "alert_count": 1
    },
    "alerts": [
      {
        "title": "东城区气象台发布大风蓝色预警[IV/一般]",
        "description": "预计 6 日早晨至午后...有 3-4 级偏北风，阵风 6-7 级...",
        "color": "蓝色",
        "level": "一般",
        "status": "预警中",
        "province": "北京市",
        "city": "北京城区",
        "county": "东城区",
        "pub_time": "2026-05-05 16:46:00",
        "source": "国家预警信息发布中心",
        "alert_id": "11010141600000_..."
      }
    ],
    "realtime": "...（彩云原始 realtime 字段，详见上方字段说明）",
    "minutely": "...（type=weather 才有）",
    "hourly": "...（type=weather 才有）",
    "daily": "...（type=weather 才有）"
  },
  "request_id": "mota..."
}
```

<a id="movie-box"></a>

## 6. 实时电影票房 `movie-box`

实时获取猫眼当日电影票房 Top 10，包含累计票房、实时票房、票房占比、排片占比、上座率等核心指标。数据来源猫眼专业版，按 60 秒缓存。

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/movie-box` |
| Method | `GET` |
| 分类 | 生活服务 (`life`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 10 / s |
| 每日免费额度 | 登录用户 20000 次；匿名 1000 次 |
| 增长层级 | B-support |
| 推广动作 | 补充推广 |
| 推广角度 | 实时电影票房 API；适合影视数据看板和小项目教程。 |
| 官网文档 | https://apizero.cn/aidocs/movie-box |
| Marketplace | https://apizero.cn/marketplace/movie-box |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Header 参数

| Header | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| X-API-Key | string | 否 | API Key（不传走匿名额度） |  |

### cURL 模板

```bash
curl -sS \
  -X GET \
  -H "X-API-Key: $APIZERO_API_KEY" \
  "https://v1.apizero.cn/api/movie-box"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "update_time": "2026-05-06 07:30:00",
    "total": 10,
    "list": [
      {
        "rank": 1,
        "name": "消失的人",
        "release_days": "上映6天",
        "total_box": "2.66亿",
        "box_office": 163.25,
        "box_rate": 35.5,
        "show_rate": 28.8,
        "seat_rate": 33
      },
      {
        "rank": 2,
        "name": "给阿嬷的情书",
        "release_days": "上映7天",
        "total_box": "6205.9万",
        "box_office": 98.85,
        "box_rate": 21.5,
        "show_rate": 7.3,
        "seat_rate": 6.1
      }
    ]
  },
  "request_id": "mot9..."
}
```

<a id="whois"></a>

## 7. Whois 域名查询 `whois`

查询域名注册信息（注册商 / 注册主体 / 创建时间 / 到期时间 / DNS 服务器 / 域名状态 / DNSSEC 等）。

• **支持所有主流 TLD**：.com / .cn / .net / .org / .io / .me / .top / .xyz 等几乎所有 ICANN 认证后缀
• **自动归一化**：传入 `https://www.baidu.com/abc` 会自动剥离为 `baidu.com`
• **域名可用性检测**：未注册域名返回 is_available=true，可用于域名抢注前查询
• **到期告警**：30 天内到期返回 expiring_soon=true，便于前端高亮提示
• **域龄展示**：creation_age_text 返回「N 年 M 月」人类可读字符串
• **国内 .cn 主体中文**：whoiscx 上游对国内域名返回中文注册主体名（如「极数本源（福州）科技有限公司」）
• **缓存 24h**：whois 数据年级变化，全天同一域名查询只调 1 次上游
• **修复源码 BUG**：错误响应字段是 `error` 不是 `message`，源码取错丢失了上游真实错误

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/whois` |
| Method | `GET` |
| 分类 | 开发工具 (`dev`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 5 / s |
| 每日免费额度 | 登录用户 1000 次；匿名 300 次 |
| 增长层级 | B-support |
| 推广动作 | 补充推广 |
| 推广角度 | Whois 域名查询；适合站长工具、域名监控、开发者脚本。 |
| 官网文档 | https://apizero.cn/aidocs/whois |
| Marketplace | https://apizero.cn/marketplace/whois |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Query 参数

| 参数 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| domain | string | 是 | 待查询域名。自动剥离 http(s):// 前缀、路径、:port 后缀、子域名（如 www.baidu.com → baidu.com 由上游处理） | baidu.com |
| with_raw | boolean | 否 | 是否返回 raw_whois 原始 whois 文本（约 3KB），默认 false。建议仅调试时启用 | false |

### cURL 模板

```bash
curl -sS \
  -X GET \
  -H "X-API-Key: $APIZERO_API_KEY" \
  "https://v1.apizero.cn/api/whois?domain=apizero.cn"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "domain": "baidu.com",
    "suffix": "com",
    "is_available": false,
    "is_expired": false,
    "expiring_soon": false,
    "registrar": "MarkMonitor Information Technology (Shanghai) Co., Ltd.",
    "registrant": "",
    "registrant_email": "",
    "creation_time": "1999-10-11 19:05:17",
    "expiration_time": "2028-10-11 19:05:17",
    "creation_days": 9704,
    "creation_age_text": "26 年 7 个月",
    "valid_days": 888,
    "domain_status": [
      "clientDeleteProhibited （注册商设置禁止删除）",
      "clientTransferProhibited （注册商设置禁止转移）"
    ],
    "name_servers": [
      "NS1.BAIDU.COM",
      "NS2.BAIDU.COM",
      "NS3.BAIDU.COM"
    ],
    "whois_server": "whois.markmonitor.com",
    "dnssec": "unsigned",
    "registrar_url": "http://markmonitor.com",
    "registrar_abuse_email": "abusecomplaints@markmonitor.com",
    "registrar_abuse_phone": "+1.2083895740",
    "query_time": "2026-05-07 09:18:54"
  },
  "request_id": "abc123def456"
}
```

<a id="qq"></a>

## 8. QQ 信息 `qq`

通过 QQ 号查询昵称、QQ 邮箱、QQ 空间链接，并生成 4 种尺寸的头像直链 URL（s40/s100/s140/s640）。

• 严格 5-11 位数字校验（防上游字符串截断引起的号码错位）
• 安全增强：上游返回的 QQ key 必须与请求严格一致，否则视为未查询到
• 错误兼容：自动识别上游错误响应（_Callback({error:...})）和正常响应（portraitCallBack(...)）
• 编码兜底：腾讯接口历史输出 GBK 含中文昵称时自动转 UTF-8
• 多尺寸头像：avatars 对象内含 s40/s100/s140/s640 四种尺寸 URL，前端 `<img src={data.avatars.s640}>` 直接用

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/qq` |
| Method | `GET` |
| 分类 | 生活服务 (`life`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 10 / s |
| 每日免费额度 | 登录用户 30000 次；匿名 5000 次 |
| 增长层级 | A-core |
| 推广动作 | 主推 |
| 推广角度 | QQ 头像昵称 API；适合头像展示、轻量用户资料补全、网页小工具。 |
| 官网文档 | https://apizero.cn/aidocs/qq |
| Marketplace | https://apizero.cn/marketplace/qq |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Query 参数

| 参数 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| qq | string | 是 | 5-11 位 QQ 号码（纯数字） | 88888888 |

### Header 参数

| Header | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| Authorization | string | 否 | API Key 鉴权头，格式 Bearer sk_live_xxx；匿名调用时可省略（每日 50 次免费） | Bearer sk_live_xxxxxxxxxxxxxx |

### cURL 模板

```bash
curl -sS \
  -X GET \
  -H "X-API-Key: $APIZERO_API_KEY" \
  "https://v1.apizero.cn/api/qq?qq=10001"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "qq": "88888888",
    "name": "腾讯客服",
    "mail": "88888888@qq.com",
    "qzone": "https://user.qzone.qq.com/88888888",
    "avatars": {
      "s40": "https://q1.qlogo.cn/g?b=qq&nk=88888888&s=40",
      "s100": "https://q1.qlogo.cn/g?b=qq&nk=88888888&s=100",
      "s140": "https://q1.qlogo.cn/g?b=qq&nk=88888888&s=140",
      "s640": "https://q1.qlogo.cn/g?b=qq&nk=88888888&s=640"
    },
    "is_found": true
  },
  "request_id": "abc123def456"
}
```

<a id="ip-pro"></a>

## 9. IP 地址查询（街道级） `ip-pro`

IP 地理位置查询，精确到街道级。多数据源自动切换：主源提供街道 + ISP + 风险评分，主源不可用时自动降级到备用源（城市级 + ISP）。响应 data.source 字段标识本次数据档位。

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/ip-pro` |
| Method | `GET` |
| 分类 | 地理位置 (`geo`) |
| 计费 | 按次付费 · 点数包 · 月套餐 · 单次 5 积分 |
| QPS | 3 / s |
| 每日免费额度 | 登录用户 30 次；匿名 5 次 |
| 增长层级 | D-inventory |
| 推广动作 | 先收录 |
| 推广角度 | 先放入接口库，等站内调用量或搜索需求上来后再写专项内容。 |
| 官网文档 | https://apizero.cn/aidocs/ip-pro |
| Marketplace | https://apizero.cn/marketplace/ip-pro |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Query 参数

| 参数 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| ip | string | 否 | 要查询的 IP 地址（IPv4 或 IPv6）。不传时自动使用调用方自身 IP，适用于"查我自己" | 110.87.41.14 |

### Header 参数

| Header | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| Authorization | string | 否 | Bearer Token。匿名调用时可省略（受每日免费额度限制）；付费方案或超过免费额度时必需 | Bearer sk_live_xxxxxxxxxxxxxxxx |

### cURL 模板

```bash
curl -sS \
  -X GET \
  -H "X-API-Key: $APIZERO_API_KEY" \
  "https://v1.apizero.cn/api/ip-pro?ip=110.87.41.14"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "ip": "117.25.49.203",
    "continent": "亚洲",
    "country": "中国",
    "country_code": "CN",
    "province": "福建",
    "city": "福州",
    "district": "永泰",
    "street": "城峰镇",
    "street_alternatives": [
      "福建福州永泰城峰镇",
      "福建福州永泰大洋镇"
    ],
    "area_code": "350125",
    "zip_code": "350000",
    "city_code": "0591",
    "latitude": 25.855039,
    "longitude": 118.94202,
    "elevation": 29,
    "time_zone": "Asia/Shanghai",
    "isp": "电信",
    "risk": {
      "level": "无风险",
      "score": 0,
      "is_proxy": false,
      "proxy_probability": 0,
      "real_rate": 6,
      "mobile_rate": 4.69
    },
    "source": "primary"
  },
  "request_id": "abc123"
}
```

<a id="shici"></a>

## 10. 随机诗词 `shici`

随机返回一首古诗词，支持按 10 种主题筛选（抒情/四季/山水/天气/人物/生活/节日/动物/植物/食物）。

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/shici` |
| Method | `POST` |
| 分类 | 内容娱乐 (`content`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 5 / s |
| 每日免费额度 | 登录用户 50 次；匿名 30 次 |
| 增长层级 | D-inventory |
| 推广动作 | 先收录 |
| 推广角度 | 先放入接口库，等站内调用量或搜索需求上来后再写专项内容。 |
| 官网文档 | https://apizero.cn/aidocs/shici |
| Marketplace | https://apizero.cn/marketplace/shici |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Request Body

- Content-Type：`application/json`
- Body 必填：否

| 字段 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| type | string | 否 | shuqing/siji/shanshui/tianqi/renwu/shenghuo/jieri/dongwu/zhiwu/shiwu |  |
| action | string | 否 | 'types' 返回类型列表（不消耗调用额度） |  |

### cURL 模板

```bash
curl -sS \
  -X POST \
  -H "X-API-Key: $APIZERO_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"type": "<type>", "action": "<action>"}' \
  "https://v1.apizero.cn/api/shici"
```

### 响应示例 `200`

```json
{
  "code": 200,
  "message": "success",
  "data": {}
}
```

<a id="company-search"></a>

## 11. 企业工商信息查询 `company-search`

通过企业名称关键词查询工商信息，返回前 5 条最匹配结果（按上游评分排序）。覆盖企业全称、法人、注册资本、统一社会信用代码、经营范围、注册地、联系方式等核心字段。数据来自天眼查权威工商数据库，6 小时缓存。

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/company-search` |
| Method | `GET` |
| 分类 | 身份核验 (`kyc`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 5 / s |
| 每日免费额度 | 登录用户 50 次；匿名 20 次 |
| 增长层级 | D-inventory |
| 推广动作 | 先收录 |
| 推广角度 | 先放入接口库，等站内调用量或搜索需求上来后再写专项内容。 |
| 官网文档 | https://apizero.cn/aidocs/company-search |
| Marketplace | https://apizero.cn/marketplace/company-search |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Query 参数

| 参数 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| name | string | 是 | 企业名称关键词，2~50 个字符（如「腾讯科技」「阿里巴巴」） |  |

### Header 参数

| Header | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| X-API-Key | string | 否 | API Key（不传走匿名额度） |  |

### cURL 模板

```bash
curl -sS \
  -X GET \
  -H "X-API-Key: $APIZERO_API_KEY" \
  "https://v1.apizero.cn/api/company-search?name=<name>"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "keyword": "腾讯科技",
    "total": 20,
    "list": [
      {
        "id": 1466562059,
        "name": "广州腾讯科技有限公司",
        "logo": "https://img5.tianyancha.com/logo/lll/...",
        "english_name": "Guangzhou Tencent Technology Co., Ltd.",
        "history_names": "",
        "legal_person": "邬红波",
        "establish_time": "2014-12-31",
        "reg_capital": "7000万人民币",
        "reg_status": "存续",
        "credit_code": "91440101327598294H",
        "business_scope": "电子;通信与自动控制技术研究...",
        "reg_location": "广州市海珠区新港中路397号...",
        "phone": "020-81167888",
        "email": "service@tencent.com",
        "category": "研究和试验发展",
        "city": "广州市",
        "district": "海珠区",
        "company_org_type": "有限责任公司",
        "match_field": "股东信息"
      }
    ]
  },
  "request_id": "mota..."
}
```

<a id="moji-weather"></a>

## 12. 墨迹天气 `moji-weather`

接入墨迹天气官方数据，支持全国 3 万+ 城市实况、历史天气、未来 7 天、24 小时趋势、AQI、9 项生活指数、气象预警及农历，一次调用全量返回。

支持 4 种查询模式：
- ?city=北京 · 按城市名查实况（自动模糊匹配第一个）
- ?id=33 · 按 internal_id 直查（跳过搜索，更快）
- ?op=search&keyword=北京 · 仅搜索城市列表，拿 id
- ?op=history&city=北京&day=2026-05-12 · 历史天气（支持单日 / 整月）

历史天气范围：
- 当前月：返回 1 号至昨天的数据
- 历史月（≤ 上月）：返回完整 30 天
- 建议查近 40 天以内，过早的月份可能无数据

服务端缓存策略：
- 实况：5 分钟；历史·当月：30 分钟；历史·过去月：24 小时

注意：数据由墨迹天气提供，仅供参考，不应用于农业、保险、航运、防灾等专业决策场景。

联系站长：apizero.cn

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/moji-weather` |
| Method | `GET` |
| 分类 | 生活服务 (`life`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 5 / s |
| 每日免费额度 | 登录用户 3000 次；匿名 1000 次 |
| 增长层级 | A-core |
| 推广动作 | 主推 |
| 推广角度 | 墨迹天气；和 weather 合并做天气专题，对比字段丰富度和使用场景。 |
| 官网文档 | https://apizero.cn/aidocs/moji-weather |
| Marketplace | https://apizero.cn/marketplace/moji-weather |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Query 参数

| 参数 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| city | string | 否 | 城市中文名（支持 "北京" / "大化" / "杭州"）；与 id 二选一 | 大化 |
| id | number | 否 | 城市 internal_id（可先调 ?op=search 查询）；与 city 二选一 |  |
| op | string | 否 | 查询模式：search=城市搜索；history=历史天气；缺省=实况天气 | history |
| keyword | string | 否 | 【op=search】关键字，支持中文 / 拼音 / 拼音首字母 | 大化 |
| limit | number | 否 | 【op=search】返回条数（1-50，默认 20） |  |
| day | string | 否 | 【op=history】查某一天：YYYY-MM-DD / MM-DD（需配 month）/ DD（需配 month） | 2026-05-12 |
| month | string | 否 | 【op=history】查整月：YYYYMM。当前月返回 1 号到昨天；历史月返回完整 30 天 | 202604 |

### Header 参数

| Header | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| Authorization | string | 否 |  |  |

### cURL 模板

```bash
curl -sS \
  -X GET \
  -H "X-API-Key: $APIZERO_API_KEY" \
  "https://v1.apizero.cn/api/moji-weather?city=北京"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "city": {
      "id": 1205,
      "name": "大化瑶族自治县",
      "parent": "广西壮族自治区",
      "pinyin": "dahuayaozuzizhixian",
      "timezone": 8
    },
    "condition": {
      "condition": "多云",
      "temperature": 32,
      "real_feel": 36,
      "humidity": 62,
      "pressure": 999,
      "wind_dir": "南风",
      "wind_level": 3,
      "uvi": "中等",
      "sun_rise": 1778706000000,
      "sun_set": 1778757660000,
      "lunar_date": "丙午年三月廿八",
      "tips": "防暑||中午外出请注意防暑降温。"
    },
    "aqi": {
      "value": 29,
      "level": 1,
      "description": "优",
      "updatetime": 1778756400000
    },
    "forecast_day": [
      {
        "predict_date": 1778601630000,
        "condition_day": "多云",
        "condition_night": "多云",
        "temp_day": 32,
        "temp_night": 21,
        "wind_dir_day": "南风",
        "wind_level_day": 3,
        "aqi_value": 29,
        "aqi_desc": "优"
      }
    ],
    "forecast_hour": [
      {
        "predict_hour": 1778752800000,
        "condition": "多云",
        "temperature": 32,
        "humidity": 62,
        "wind_dir": "南风",
        "wind_level": "3",
        "aqi_value": 29
      }
    ],
    "index": [
      {
        "name": "限行",
        "status": "不限行"
      },
      {
        "name": "穿衣",
        "status": "炎热"
      }
    ],
    "summary": "大化瑶族自治县，多云，32℃，南风3级，空气优。",
    "_cached": false
  }
}
```

<a id="riddle"></a>

## 13. 谜语大全 `riddle`

谜语接口，支持随机一条、分页列表、类型列表三种模式（action=random/list/types）。

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/riddle` |
| Method | `POST` |
| 分类 | 内容娱乐 (`content`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 5 / s |
| 每日免费额度 | 登录用户 50 次；匿名 30 次 |
| 增长层级 | D-inventory |
| 推广动作 | 先收录 |
| 推广角度 | 先放入接口库，等站内调用量或搜索需求上来后再写专项内容。 |
| 官网文档 | https://apizero.cn/aidocs/riddle |
| Marketplace | https://apizero.cn/marketplace/riddle |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Request Body

- Content-Type：`application/json`
- Body 必填：否

| 字段 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| action | string | 否 | 'random'(默认)/'list'/'types' |  |
| type | string | 否 | [list] 谜语类型，小写字母 |  |
| page | string | 否 | [list] 页码，正整数，默认 1 |  |

### cURL 模板

```bash
curl -sS \
  -X POST \
  -H "X-API-Key: $APIZERO_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"action": "<action>", "type": "<type>", "page": "<page>"}' \
  "https://v1.apizero.cn/api/riddle"
```

### 响应示例 `200`

```json
{
  "code": 200,
  "message": "success",
  "data": {}
}
```

<a id="exchange-rate"></a>

## 14. 实时汇率查询 `exchange-rate`

实时汇率查询，支持 26 种主流货币（CNY/USD/EUR/GBP/JPY 等）一键互转。数据 1 分钟级更新，毫秒响应，免费且无需注册即可调用。

基于 ALAPI 汇率源，覆盖人民币、美元、欧元、英镑、日元、港币、韩元、澳元、加元、新加坡元、瑞士法郎、新台币、泰铢、马来西亚林吉特、俄罗斯卢布、印度卢比、巴西雷亚尔、南非兰特、新西兰元、瑞典/挪威/丹麦克朗、菲律宾比索、印尼盾、越南盾、阿联酋迪拉姆等。

支持「同币种快路径」（from=to 时本地直接返回，0 上游消耗），以及「货币列表查询」action=currencies 子命令（返回支持的全部 26 种货币代码与中文名）。

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/exchange-rate` |
| Method | `GET` |
| 分类 | 金融数据 (`finance`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 5 / s |
| 每日免费额度 | 登录用户 200 次；匿名 50 次 |
| 增长层级 | D-inventory |
| 推广动作 | 先收录 |
| 推广角度 | 先放入接口库，等站内调用量或搜索需求上来后再写专项内容。 |
| 官网文档 | https://apizero.cn/aidocs/exchange-rate |
| Marketplace | https://apizero.cn/marketplace/exchange-rate |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Query 参数

| 参数 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| money | number | 否 | 要转换的金额，必须 > 0；缺省 1 |  |
| from | string | 否 | 源货币代码（ISO 4217 三字母），如 CNY、USD、EUR；缺省 CNY | CNY |
| to | string | 否 | 目标货币代码（ISO 4217 三字母），如 USD、JPY；缺省 USD | USD |
| action | string | 否 | 传 "currencies" 直接返回 26 种支持货币列表（不调上游、不计费） | currencies |

### Header 参数

| Header | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| Authorization | string | 否 | API Key 鉴权头，格式 Bearer sk_live_xxx；匿名调用时可省略（每日 50 次免费） | Bearer sk_live_xxxxxxxxxxxxxx |

### cURL 模板

```bash
curl -sS \
  -X GET \
  -H "X-API-Key: $APIZERO_API_KEY" \
  "https://v1.apizero.cn/api/exchange-rate?money=<money>&from=CNY"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "money": 1,
    "from": "CNY",
    "from_name": "人民币",
    "to": "USD",
    "to_name": "美元",
    "rate": 0.146405,
    "result": 0.1464,
    "update_time": "2026-05-06 13:00:02"
  },
  "request_id": "abc123def456"
}
```

<a id="fund"></a>

## 15. 基金估值跟踪 `fund`

基金估值跟踪，一接口聚合「基金实时估值 / 指数行情 / 基金详情 / 全部常用指数批量」4 大功能，覆盖国内基金 + A 股核心指数。

基于天天基金、新浪财经、东方财富三路上游，无需注册即可调用：

• action=estimate — 基金实时估值（盘中每分钟更新，含单位净值、估算净值、估算涨跌幅、上次净值日期）

• action=index — 单个指数行情（新浪主、东方财富备双通道容灾，含当前点位、涨跌点、涨跌幅）

• action=info — 基金详细信息（名称、净值、累计净值、近 1 月/3 月/6 月/1 年/3 年/成立来收益、基金类型、风险等级、规模、基金经理、成立日、管理人）

• action=indices — 6 个常用指数批量（上证、深证成指、创业板、上证 50、沪深 300、中证 500）

本接口已修复源码中「新浪通道缺 sh/sz 前缀」的 bug，并优化了 HTML 爬虫的容错。

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/fund` |
| Method | `GET` |
| 分类 | 金融数据 (`finance`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 5 / s |
| 每日免费额度 | 登录用户 200 次；匿名 50 次 |
| 增长层级 | B-support |
| 推广动作 | 补充推广 |
| 推广角度 | 基金估值 API；适合 Python 自用脚本、看板、投资工具教程。 |
| 官网文档 | https://apizero.cn/aidocs/fund |
| Marketplace | https://apizero.cn/marketplace/fund |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Query 参数

| 参数 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| action | string | 是 | 操作类型：estimate=基金估值 / index=单个指数 / info=基金详情 / indices=全部常用指数 | estimate |
| code | string | 否 | 基金或指数代码（6 位数字）；当 action 为 estimate/index/info 时必填，indices 时不需要 | 005827 |

### Header 参数

| Header | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| Authorization | string | 否 | API Key 鉴权头，格式 Bearer sk_live_xxx；匿名调用时可省略（每日 50 次免费） | Bearer sk_live_xxxxxxxxxxxxxx |

### cURL 模板

```bash
curl -sS \
  -X GET \
  -H "X-API-Key: $APIZERO_API_KEY" \
  "https://v1.apizero.cn/api/fund?action=estimate&code=005827"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "action": "estimate",
    "fund_code": "005827",
    "fund_name": "易方达蓝筹精选混合",
    "nav_date": "2026-04-30",
    "net_value": 1.7393,
    "estimate": 1.727,
    "change_rate": -0.71,
    "update_time": "2026-05-06 15:00"
  },
  "request_id": "abc123def456"
}
```

<a id="hitokoto"></a>

## 16. 一言 · 经典语录 `hitokoto`

从 370+ 条经典语录库（动漫/漫画/游戏/文学/影视/诗词/哲学/网络等 12 类）随机返回一条，含原文、出处、作者、分类、字数。支持按分类与长度筛选。

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/hitokoto` |
| Method | `GET` |
| 分类 | 生活服务 (`life`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 20 / s |
| 每日免费额度 | 登录用户 500000 次；匿名 200000 次 |
| 增长层级 | D-inventory |
| 推广动作 | 先收录 |
| 推广角度 | 先放入接口库，等站内调用量或搜索需求上来后再写专项内容。 |
| 官网文档 | https://apizero.cn/aidocs/hitokoto |
| Marketplace | https://apizero.cn/marketplace/hitokoto |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Query 参数

| 参数 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| c | string | 否 | 分类单字母 a-l（不传则全类别随机） | i |
| min_length | number | 否 | 最小字符数 |  |
| max_length | number | 否 | 最大字符数 |  |

### cURL 模板

```bash
curl -sS \
  -X GET \
  -H "X-API-Key: $APIZERO_API_KEY" \
  "https://v1.apizero.cn/api/hitokoto?c=i&min_length=<min_length>"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "id": 1234,
    "hitokoto": "落霞与孤鹜齐飞，秋水共长天一色。",
    "type": "i",
    "type_name": "诗词",
    "from": "滕王阁序",
    "from_who": "王勃",
    "length": 16,
    "total_pool": 370
  }
}
```

<a id="netease-comment"></a>

## 17. 网易云热门乐评 `netease-comment`

随机返回一条网易云音乐高赞热评，含歌曲信息（标题/作者/封面/试听）和评论详情。

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/netease-comment` |
| Method | `POST` |
| 分类 | 内容娱乐 (`content`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 5 / s |
| 每日免费额度 | 登录用户 50 次；匿名 30 次 |
| 增长层级 | D-inventory |
| 推广动作 | 先收录 |
| 推广角度 | 先放入接口库，等站内调用量或搜索需求上来后再写专项内容。 |
| 官网文档 | https://apizero.cn/aidocs/netease-comment |
| Marketplace | https://apizero.cn/marketplace/netease-comment |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Request Body

- Content-Type：`application/json`
- Body 必填：否

### cURL 模板

```bash
curl -sS \
  -X POST \
  -H "X-API-Key: $APIZERO_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{}' \
  "https://v1.apizero.cn/api/netease-comment"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "song": {
      "title": "寂寞烟火",
      "author": "朱婧汐Akini Jing",
      "album": "以梦为马",
      "image": "https://p2.music.126.net/lDlFdy62rPxCM9kSnymGQA==/6645448278712402.jpg",
      "mp3_url": "https://v2.alapi.cn/api/music/url/token?id=29357087&timestamp=1780108420&sign=275f4cde2b0ba6aefe345a3d5ae36534",
      "published_date": "2016-01-09 16:54:52"
    },
    "comment": {
      "nickname": "麋鹿和迷雾",
      "avatar": "",
      "content": "走过黑暗后才明白只有自己才是自己的阳光 以后会好好生活好好爱惜自己 也会耐心等待值得的人的出现 你不需要对我愧疚我不稀罕你在电话里说的那句对我很愧疚 终于能清楚彻底的有个了断我觉得很好 不然还不知道要被你拖到什么时候 不介意孤独 比爱你舒服。",
      "liked_count": 18057,
      "published_date": "2016-01-09 16:54:52"
    }
  },
  "request_id": "mprqlbgf64636962"
}
```

<a id="wzry"></a>

## 18. 王者战力查询 `wzry`

王者荣耀全国战力查询，一接口聚合「英雄列表 + 区服战力分布」2 大功能。

• action=heroes — 返回 130+ 个英雄完整目录（中文名 / ename / 称号 / 头像 URL），优先腾讯官方源 https://pvp.qq.com/web201605/js/herolist.json

• action=query — 查询某英雄在某区服的全国战力分布数据，返回 90 条左右的省市区战力榜（含同地区相近排名）

支持 4 大区服：
  · aqq = Android QQ
  · awx = Android 微信
  · iqq = iOS QQ
  · iwx = iOS 微信

查询类型：
  · all = 完整 heroList（按 level 混合排列，约 90 项）
  · min = 各级（省/市/区）最低战力 + 相近排名（rank ≤ +100）
  · max = 各级（省/市/区）最高战力 + 相近排名

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/wzry` |
| Method | `GET` |
| 分类 | 生活服务 (`life`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 5 / s |
| 每日免费额度 | 登录用户 200 次；匿名 50 次 |
| 增长层级 | D-inventory |
| 推广动作 | 先收录 |
| 推广角度 | 先放入接口库，等站内调用量或搜索需求上来后再写专项内容。 |
| 官网文档 | https://apizero.cn/aidocs/wzry |
| Marketplace | https://apizero.cn/marketplace/wzry |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Query 参数

| 参数 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| action | string | 是 | 操作类型：heroes=英雄列表 / query=战力查询 | query |
| hero | string | 否 | 英雄中文名（如「赵云」）；action=query 时与 hero_id 二选一必传 | 赵云 |
| hero_id | number | 否 | 英雄 ename（如赵云=107）；action=query 时与 hero 二选一必传，hero_id 优先级更高 |  |
| zone | string | 否 | 区服代码：aqq=Android QQ / awx=Android 微信 / iqq=iOS QQ / iwx=iOS 微信；action=query 必填 | aqq |
| type | string | 否 | 返回类型：all=完整列表（默认）/ min=最低战力 / max=最高战力 | all |

### Header 参数

| Header | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| Authorization | string | 否 | API Key 鉴权头，格式 Bearer sk_live_xxx；匿名调用时可省略（每日 50 次免费） | Bearer sk_live_xxxxxxxxxxxxxx |

### cURL 模板

```bash
curl -sS \
  -X GET \
  -H "X-API-Key: $APIZERO_API_KEY" \
  "https://v1.apizero.cn/api/wzry?action=query"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "action": "query",
    "hero": {
      "name": "赵云",
      "title": "苍天翔龙",
      "ename": "107",
      "avatar": "https://game.gtimg.cn/images/yxzj/img201606/heroimg/107/107.jpg"
    },
    "zone": {
      "code": "aqq",
      "system": "Android",
      "platform": "QQ"
    },
    "type": "最低战力",
    "type_code": "min",
    "syn_date": "2026-05-06",
    "rank_data": {
      "extreme": {
        "province": {
          "address": "云南",
          "level": "province",
          "rank": 4500,
          "adcode": "..."
        },
        "city": {
          "address": "海南/三亚市",
          "level": "city",
          "rank": 1800,
          "adcode": "..."
        },
        "district": {
          "address": "北京/朝阳区",
          "level": "district",
          "rank": 800,
          "adcode": "..."
        }
      },
      "similar": {
        "province": [
          {
            "address": "云南",
            "rank": 4500
          },
          {
            "address": "甘肃",
            "rank": 4520
          }
        ],
        "city": [],
        "district": []
      }
    }
  },
  "request_id": "abc123def456"
}
```

<a id="content-moderation"></a>

## 19. 内容审核 `content-moderation`

基于「敏感词库 + 正则规则 + AI 特征评分」三重策略检测文本中的色情 / 政治 / 广告 / 联系方式 / 引流 / 谩骂 6 大类敏感内容。识别变体绕过（谐音、拼音、符号替换），输出 safe/low/medium/high 风险等级，可选自动脱敏。

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/content-moderation` |
| Method | `POST` |
| 分类 | 内容娱乐 (`content`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 10 / s |
| 每日免费额度 | 登录用户 500 次；匿名 100 次 |
| 增长层级 | D-inventory |
| 推广动作 | 先收录 |
| 推广角度 | 先放入接口库，等站内调用量或搜索需求上来后再写专项内容。 |
| 官网文档 | https://apizero.cn/aidocs/content-moderation |
| Marketplace | https://apizero.cn/marketplace/content-moderation |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Header 参数

| Header | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| Authorization | string | 否 |  |  |

### Request Body

- Content-Type：`application/json`
- Body 必填：是

| 字段 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| action | string | 否 | 操作：moderate（默认）/ batch / categories | moderate |
| text | string | 是 | 待审核文本（仅 action=moderate 用），1-5000 字 | 今天天气不错 |
| texts | array | 否 | 批量文本（仅 action=batch 用），最多 50 条 | ["文本1", "文本2"] |
| mask | boolean | 否 | 是否返回脱敏文本，默认 false | true |

请求体示例：

```json
{
  "action": "moderate",
  "text": "今天天气不错",
  "texts": "[\"文本1\", \"文本2\"]",
  "mask": "true"
}
```

### cURL 模板

```bash
curl -sS \
  -X POST \
  -H "X-API-Key: $APIZERO_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"action": "moderate", "text": "今天天气不错", "texts": "[\"文本1\", \"文本2\"]", "mask": "true"}' \
  "https://v1.apizero.cn/api/content-moderation"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "is_pass": false,
    "risk_level": "high",
    "categories": [
      "谩骂"
    ],
    "details": [
      {
        "category": "谩骂",
        "method": "敏感词",
        "matches": [
          "傻逼",
          "脑残"
        ],
        "count": 2
      }
    ],
    "masked_text": "你这个**，**吧",
    "original_length": 10
  },
  "request_id": "abc123"
}
```

<a id="brain-teaser"></a>

## 20. 脑筋急转弯 `brain-teaser`

从 4500+ 条本地题库随机返回一条脑筋急转弯，包含题目和答案。零上游依赖，毫秒级返回，适合聊天机器人、APP 互动模块。

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/brain-teaser` |
| Method | `GET` |
| 分类 | 生活服务 (`life`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 20 / s |
| 每日免费额度 | 登录用户 200000 次；匿名 10000 次 |
| 增长层级 | D-inventory |
| 推广动作 | 先收录 |
| 推广角度 | 先放入接口库，等站内调用量或搜索需求上来后再写专项内容。 |
| 官网文档 | https://apizero.cn/aidocs/brain-teaser |
| Marketplace | https://apizero.cn/marketplace/brain-teaser |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### cURL 模板

```bash
curl -sS \
  -X GET \
  -H "X-API-Key: $APIZERO_API_KEY" \
  "https://v1.apizero.cn/api/brain-teaser"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "question": "什么动物最爱贴在墙上？",
    "answer": "海报。",
    "total_pool": 4500
  }
}
```

<a id="oil-price-forecast"></a>

## 21. 今日油价 `oil-price-forecast`

提供 32 省份基准油价（92/95/98 号汽油 + 0 号柴油），并基于新浪财经国际原油（WTI、布伦特）实时走势，预测下一次国内调价方向（涨/跌/搁浅）和幅度。附带 2025-2026 年完整调价窗口日历。

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/oil-price-forecast` |
| Method | `GET` |
| 分类 | 生活服务 (`life`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 3 / s |
| 每日免费额度 | 登录用户 10000 次；匿名 5000 次 |
| 增长层级 | D-inventory |
| 推广动作 | 先收录 |
| 推广角度 | 先放入接口库，等站内调用量或搜索需求上来后再写专项内容。 |
| 官网文档 | https://apizero.cn/aidocs/oil-price-forecast |
| Marketplace | https://apizero.cn/marketplace/oil-price-forecast |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Query 参数

| 参数 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| action | string | 否 | 操作：forecast（默认）/ price / price-all / schedule | forecast |
| province | string | 否 | 省份名（仅 action=price 用），如 北京/广东/上海 | 北京 |
| year | number | 否 | 调价年份（仅 action=schedule 用），仅 2025/2026 | 2026 |

### Header 参数

| Header | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| Authorization | string | 否 |  |  |

### cURL 模板

```bash
curl -sS \
  -X GET \
  -H "X-API-Key: $APIZERO_API_KEY" \
  "https://v1.apizero.cn/api/oil-price-forecast?action=forecast&province=北京"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "crude_oil": {
      "wti": 61.5,
      "brent": 64.8,
      "wti_change": -0.3,
      "brent_change": -0.2,
      "source": "sina_finance"
    },
    "next_adjust_date": "2026-05-11",
    "days_remaining": 1,
    "prediction": {
      "direction": "搁浅",
      "direction_emoji": "⏸️",
      "estimated_change_per_ton": -30,
      "estimated_change_per_liter": 0.022,
      "confidence": "高",
      "analysis": "当前国际油价布伦特约 64.8 美元/桶，日均变动 -0.25 美元……"
    }
  },
  "request_id": "abc123"
}
```

<a id="gold"></a>

## 22. 黄金价格查询 `gold`

实时黄金价格聚合查询，一次返回三大维度的金价数据：

• 国际黄金报价（4 项）：国际金价、国际铂金、国际银价、国际钯金，含最新价 / 涨跌额 / 涨跌幅 / 最高价 / 最低价 / 报价时间

• 国内黄金报价（7 项）：国内金价、国内银价、投资金条、黄金回收价格、铂金回收价格、18K 金回收价格、钯金回收价格

• 十大品牌金店金价（17 家）：内地周大福、周生生、六福珠宝、老凤祥、老庙黄金、中国黄金、周六福、菜百、明牌珠宝、潮宏基等十大品牌的当日黄金 / 铂金 / 金条价格（元/克）

通过参数 type 可只取某一类（all=全部，brand=品牌金店，international=国际，domestic=国内）。

基于 huangjinjiage.cn 数据源，已修复源码 GBK 编码乱码 bug，内建 10 分钟缓存。

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/gold` |
| Method | `GET` |
| 分类 | 金融数据 (`finance`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 5 / s |
| 每日免费额度 | 登录用户 200 次；匿名 50 次 |
| 增长层级 | D-inventory |
| 推广动作 | 先收录 |
| 推广角度 | 先放入接口库，等站内调用量或搜索需求上来后再写专项内容。 |
| 官网文档 | https://apizero.cn/aidocs/gold |
| Marketplace | https://apizero.cn/marketplace/gold |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Query 参数

| 参数 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| type | string | 否 | 返回类型：all=全部（默认）/ brand=品牌金店 / international=国际黄金 / domestic=国内黄金 | all |

### Header 参数

| Header | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| Authorization | string | 否 | API Key 鉴权头，格式 Bearer sk_live_xxx；匿名调用时可省略（每日 50 次免费） | Bearer sk_live_xxxxxxxxxxxxxx |

### cURL 模板

```bash
curl -sS \
  -X GET \
  -H "X-API-Key: $APIZERO_API_KEY" \
  "https://v1.apizero.cn/api/gold?type=all"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "update_time": "2026-05-06 15:30:00",
    "source": "huangjinjiage.cn",
    "type": "all",
    "international": [
      {
        "name": "国际金价",
        "price": "4647.3",
        "change": "7.19",
        "percent": "0.39%",
        "high": "1828.34",
        "low": "1815.50",
        "time": "2026-5-6"
      },
      {
        "name": "国际银价",
        "price": "75.28",
        "change": "0.21",
        "percent": "0.91%",
        "high": "23.24",
        "low": "22.78",
        "time": "2026-5-6"
      }
    ],
    "domestic": [
      {
        "name": "国内金价",
        "price": "1022.9",
        "change": "2.08",
        "percent": "0.56%",
        "high": "375",
        "low": "372.76",
        "time": "2026-5-6"
      }
    ],
    "brand": [
      {
        "brand": "内地周大福",
        "gold_price": "1413",
        "pt_price": "-",
        "bar_price": "1239",
        "unit": "元/克",
        "time": "2026-5-6"
      }
    ]
  },
  "request_id": "abc123def456"
}
```

<a id="translate"></a>

## 23. 文本翻译 `translate`

文本翻译，支持 19 种语言互译（含粤语/文言文）。参数 q 传入文本，from/to 指定语言代码，默认中文→英文。注意使用百度系语言代码：日语 jp、韩语 kor、法语 fra。

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/translate` |
| Method | `GET` |
| 分类 | 内容娱乐 (`content`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 5 / s |
| 每日免费额度 | 登录用户 5000 次；匿名 2000 次 |
| 增长层级 | D-inventory |
| 推广动作 | 先收录 |
| 推广角度 | 先放入接口库，等站内调用量或搜索需求上来后再写专项内容。 |
| 官网文档 | https://apizero.cn/aidocs/translate |
| Marketplace | https://apizero.cn/marketplace/translate |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Query 参数

| 参数 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| q | string | 是 | 待翻译文本，最长 5000 字；兼容 text 参数名 | 你好世界 |
| from | string | 否 | 源语言代码，默认 zh；见下方语言代码表 | zh |
| to | string | 否 | 目标语言代码，默认 en；见下方语言代码表 | en |

### Header 参数

| Header | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| Authorization | string | 是 |  |  |

### cURL 模板

```bash
curl -sS \
  -X GET \
  -H "X-API-Key: $APIZERO_API_KEY" \
  "https://v1.apizero.cn/api/translate?q=你好世界"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "source_text": "你好世界",
    "target_text": "Hello World",
    "from": "zh",
    "to": "en",
    "from_name": "中文（简体）",
    "to_name": "英文",
    "char_count": 4
  },
  "request_id": "abc123"
}
```

<a id="ocr-text"></a>

## 24. OCR 文字识别 `ocr-text`

通用 OCR 文字识别，从图片中提取任意文字内容（中文/英文/数字/符号/手写体等），返回逐行文本列表 + 完整拼接文本。

• 双输入模式：
  - input_type=url：传入公网可访问的图片 URL（http/https）
  - input_type=base64：传入 base64 编码字符串（≤6MB，自动剥离 data:image/...;base64, 前缀）

• 三路输出：
  - text_list：按原图顺序的逐行文本数组
  - full_text：用换行符 \n 拼接的完整文本字符串
  - text_count：识别到的文本行数

• 适用场景：截图转文字、表格识别、身份证/名片文字提取、字幕识别、笔记 OCR 等通用场景。专用发票识别请用 /api/invoice。

• OCR 结果缓存 1 小时（同图同结果），重复调用不消耗上游配额。

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/ocr-text` |
| Method | `POST` |
| 分类 | 文档识别 (`ocrdata`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 2 / s |
| 每日免费额度 | 登录用户 300 次；匿名 50 次 |
| 增长层级 | D-inventory |
| 推广动作 | 先收录 |
| 推广角度 | 先放入接口库，等站内调用量或搜索需求上来后再写专项内容。 |
| 官网文档 | https://apizero.cn/aidocs/ocr-text |
| Marketplace | https://apizero.cn/marketplace/ocr-text |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Header 参数

| Header | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| Authorization | string | 否 | API Key 鉴权头，格式 Bearer sk_live_xxx；匿名调用时可省略（每日 5 次免费） | Bearer sk_live_xxxxxxxxxxxxxx |
| Content-Type | string | 是 | POST 请求体类型，固定 application/x-www-form-urlencoded | application/x-www-form-urlencoded |

### Request Body

- Content-Type：`application/json`
- Body 必填：是

| 字段 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| input_type | string | 是 | 输入类型：url=图片URL / base64=图片base64编码 | url |
| input_data | string | 是 | 图片数据。input_type=url 时为 http/https 完整 URL；input_type=base64 时为 base64 字符串（最大 6MB，可选 data:image/jpeg;base64, 前缀） | https://dummyimage.com/400x100/000/fff.png&text=Hello+World |

请求体示例：

```json
{
  "input_type": "url",
  "input_data": "https://dummyimage.com/400x100/000/fff.png&text=Hello+World"
}
```

### cURL 模板

```bash
curl -sS \
  -X POST \
  -H "X-API-Key: $APIZERO_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"input_type": "url", "input_data": "https://dummyimage.com/400x100/000/fff.png&text=Hello+World"}' \
  "https://v1.apizero.cn/api/ocr-text"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "input_type": "url",
    "text_count": 3,
    "text_list": [
      "商品名称：无线蓝牙耳机",
      "单价：¥299.00",
      "数量：2"
    ],
    "full_text": "商品名称：无线蓝牙耳机\n单价：¥299.00\n数量：2"
  },
  "request_id": "abc123def456"
}
```

<a id="baby-naming"></a>

## 25. 八字起名 `baby-naming`

输入父母姓氏与出生时间，基于「八字五行 + 五格数理 + 三才配置」综合评分生成高分名字候选，附寓意标签与全国重名预估。三个 action：naming（智能起名）/ duplicate（重名查询）/ bazi（八字查询）。内置 396 姓氏笔画 + 175 起名字 + 88 姓人口。

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/baby-naming` |
| Method | `POST` |
| 分类 | 生活服务 (`life`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 2 / s |
| 每日免费额度 | 登录用户 5000 次；匿名 3000 次 |
| 增长层级 | D-inventory |
| 推广动作 | 先收录 |
| 推广角度 | 先放入接口库，等站内调用量或搜索需求上来后再写专项内容。 |
| 官网文档 | https://apizero.cn/aidocs/baby-naming |
| Marketplace | https://apizero.cn/marketplace/baby-naming |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Header 参数

| Header | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| Authorization | string | 否 |  |  |

### Request Body

- Content-Type：`application/json`
- Body 必填：是

| 字段 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| action | string | 否 | naming（默认）/ duplicate / bazi |  |
| surname | string | 是 | naming/duplicate 必填；姓氏（≤ 2 字） | 王 |
| mother_surname | string | 否 | 母姓（仅 naming，可选；填写则生成双姓名） |  |
| birth_year | number | 否 | naming/bazi 必填；2000-2100 | 2026 |
| birth_month | number | 否 | naming/bazi 必填；1-12 | 5 |
| birth_day | number | 否 | naming/bazi 必填；1-31 | 10 |
| birth_hour | number | 否 | 出生时辰；0-23，默认 12 |  |
| gender | string | 否 | 性别偏好：male/female/neutral，默认 neutral |  |
| count | number | 否 | 返回名字数量；1-30，默认 10 |  |
| name | string | 否 | duplicate 必填；要查的名字（不含姓） | 梓涵 |

请求体示例：

```json
{
  "surname": "王",
  "birth_year": "2026",
  "birth_month": "5",
  "birth_day": "10",
  "name": "梓涵"
}
```

### cURL 模板

```bash
curl -sS \
  -X POST \
  -H "X-API-Key: $APIZERO_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"surname": "王", "birth_year": "2026", "birth_month": "5", "birth_day": "10", "name": "梓涵"}' \
  "https://v1.apizero.cn/api/baby-naming"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "bazi": {
      "八字": "丙午癸巳辛未癸巳",
      "四柱": [
        "丙午",
        "癸巳",
        "辛未",
        "癸巳"
      ],
      "日主": "金"
    },
    "wu_xing_analysis": {
      "五行分布": {
        "金": 1,
        "木": 0,
        "水": 2,
        "火": 4,
        "土": 1
      },
      "五行缺失": [
        "木"
      ],
      "建议补充": [
        "木"
      ]
    },
    "needed_wuxing": [
      "木"
    ],
    "names": [
      {
        "name": "王梓森",
        "surname": "王",
        "given_name": "梓森",
        "score": 95,
        "wuge": {
          "天格": 5,
          "人格": 15,
          "地格": 23,
          "外格": 16,
          "总格": 27
        },
        "wuge_score": 88,
        "wuxing_chars": "木+木",
        "meaning_tags": [
          "栋梁",
          "繁盛"
        ],
        "duplicate_rate": {
          "estimated_count": 320,
          "level": "较低",
          "description": "..."
        }
      }
    ]
  },
  "request_id": "abc123"
}
```

<a id="express"></a>

## 26. 快递物流查询 `express`

快递物流查询，支持顺丰、圆通、中通、申通、韵达、极兔、京东、EMS 等 100+ 主流快递公司，自动识别单号所属公司，返回完整物流轨迹。

基于 ALAPI 物流源，覆盖国内全部主流快递。支持「自动识别」（不传 com 参数即可）和「手动指定公司编码」两种模式。顺丰、中通因隐私保护需求，必须传手机号后 4 位（参数 phone）。

返回字段包含：单号、快递公司编码、公司中文名、状态码（0=未查到, 1=已揽收, 2=在途, 3=签收, 4=问题件）、状态描述、完整物流轨迹（按时间倒序，包含每条轨迹的时间和文字描述）。

建议前端轮询频率不超过每分钟 1 次，本接口已内建 5 分钟缓存以节省调用次数。

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/express` |
| Method | `GET` |
| 分类 | 生活服务 (`life`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 5 / s |
| 每日免费额度 | 登录用户 100 次；匿名 30 次 |
| 增长层级 | D-inventory |
| 推广动作 | 先收录 |
| 推广角度 | 先放入接口库，等站内调用量或搜索需求上来后再写专项内容。 |
| 官网文档 | https://apizero.cn/aidocs/express |
| Marketplace | https://apizero.cn/marketplace/express |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Query 参数

| 参数 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| number | string | 是 | 快递单号（8-40 位字母或数字） | YT7460266600081 |
| com | string | 否 | 快递公司编码（如 sf=顺丰、yto=圆通、zto=中通、sto=申通、yunda=韵达、jt=极兔、jd=京东、ems=EMS）；缺省由上游自动识别 | yto |
| phone | string | 否 | 手机号后 4 位数字（仅 顺丰 / 中通 必填，因隐私保护要求） | 1234 |

### Header 参数

| Header | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| Authorization | string | 否 | API Key 鉴权头，格式 Bearer sk_live_xxx；匿名调用时可省略（每日 30 次免费） | Bearer sk_live_xxxxxxxxxxxxxx |

### cURL 模板

```bash
curl -sS \
  -X GET \
  -H "X-API-Key: $APIZERO_API_KEY" \
  "https://v1.apizero.cn/api/express?number=YT7460266600081"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "number": "YT7460266600081",
    "com": "yto",
    "com_name": "圆通快递",
    "state": 3,
    "status": "DELIVERED",
    "status_desc": "已签收",
    "trace_count": 3,
    "traces": [
      {
        "time": "2026-05-06 14:23:11",
        "content": "【上海市】您的快件已签收，签收人：本人"
      },
      {
        "time": "2026-05-06 09:15:32",
        "content": "【上海市】快件正在派送途中（派件员：张三 138****1234）"
      },
      {
        "time": "2026-05-05 22:41:08",
        "content": "【广州市】快件离开 广州转运中心 发往 上海转运中心"
      }
    ]
  },
  "request_id": "abc123def456"
}
```

<a id="image-enhance"></a>

## 27. AI 图片变清晰 `image-enhance`

基于先进 AI 超分辨率算法的图片高清化服务。输入一张模糊 / 低分辨率图片 URL，约 4 秒内返回 4 倍超分辨率的高清版本（如 300×300 → 1200×1200），支持 JPEG / PNG / WebP / BMP 等主流格式。

【核心能力】
- 4 倍超分辨率（最大边自动放大约 4 倍）
- 自动去噪 / 去糊 / 锐化细节
- 保留原图色彩与构图
- 适合老照片修复、缩略图放大、低质截图增强、网络图片优化
- 输出 JPEG 高质量（HD 模式）

【典型场景】
- 电商：商品低清图自动高清化
- 自媒体：老照片 / 截图修复用于配图
- 设计：素材尺寸不够时快速放大
- 视频缩略图、社交头像高清化
- 旧扫描件 / 截屏文字增强

【调用示例】
POST /api/image-enhance
Content-Type: application/x-www-form-urlencoded

img=https://example.com/your-blurry-image.jpg

【⚠️ 输入限制】
- 必须是 **公网可访问** 的 http/https URL（私有 OSS 链接需先签名）
- 文件大小 ≤ 10 MB
- 支持格式：JPEG / PNG / WebP / BMP

【⚠️ 输出说明】
- 返回的 enhanced_url 为平台代理 URL（自有域名，跨域友好）
- 实际图片在 6 小时内有效，请尽快下载保存
- 平均响应 4~6 秒，复杂图片可能需要 10~30 秒

【计费说明】
登录用户每日 10 次免费体验，超出按点数扣费（详见控制台）。

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/image-enhance` |
| Method | `POST` |
| 分类 | AI 能力 (`ai`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 1 / s |
| 每日免费额度 | 登录用户 10 次；匿名 0 次 |
| 增长层级 | D-inventory |
| 推广动作 | 先收录 |
| 推广角度 | 先放入接口库，等站内调用量或搜索需求上来后再写专项内容。 |
| 官网文档 | https://apizero.cn/aidocs/image-enhance |
| Marketplace | https://apizero.cn/marketplace/image-enhance |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Header 参数

| Header | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| Authorization | string | 是 | API Key 鉴权（在控制台申请） |  |
| Content-Type | string | 否 | 使用 form-urlencoded 或 JSON body 都可 |  |

### Request Body

- Content-Type：`application/json`
- Body 必填：是

| 字段 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| img | string | 是 | 待增强的图片 URL，必须是公网可访问的 http/https 地址；文件 ≤ 10 MB；JPEG / PNG / WebP / BMP 格式 | https://example.com/blurry-photo.jpg |

请求体示例：

```json
{
  "img": "https://example.com/blurry-photo.jpg"
}
```

### cURL 模板

```bash
curl -sS \
  -X POST \
  -H "X-API-Key: $APIZERO_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"img": "https://example.com/blurry-photo.jpg"}' \
  "https://v1.apizero.cn/api/image-enhance"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "original_url": "https://example.com/blurry-photo.jpg",
    "enhanced_url": "https://v1.apizero.cn/api/image-enhance?mode=image&u=aHR0cHM6Ly9...&s=a1b2c3d4e5f6",
    "width": 1200,
    "height": 1200,
    "expires_in": 21600
  },
  "request_id": "mqx8x12345abc"
}
```

<a id="tts"></a>

## 28. TTS 语音合成 `tts`

对接 alapi.cn 文本转语音（TTS）服务，输入文本返回 base64 编码的 MP3 音频。

• **5 种音色**：女声主播 / 男声主播 / 男声说唱 / 女声四川话 / 男声低沉
• **500 字以内**：单次请求最多 500 字符（中英文均按 1 字符计）
• **MP3 格式**：上游返回 audio/mpeg 编码，前端可直接 `<audio src="data:audio/mpeg;base64,...">` 播放
• **便利字段**：返回 audio_data_url（拼好的 data URL）+ audio_size_bytes（解码后字节数）+ audio_format / audio_mime
• **典型场景**：新闻播报 / 短视频配音 / 有声书 / 语音通知 / 客服 IVR
• **不缓存**：500 字音频约 1MB，重复合成概率低，避免 Redis 内存膨胀

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/tts` |
| Method | `POST` |
| 分类 | AI 能力 (`ai`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 3 / s |
| 每日免费额度 | 登录用户 30 次；匿名 10 次 |
| 增长层级 | D-inventory |
| 推广动作 | 先收录 |
| 推广角度 | 先放入接口库，等站内调用量或搜索需求上来后再写专项内容。 |
| 官网文档 | https://apizero.cn/aidocs/tts |
| Marketplace | https://apizero.cn/marketplace/tts |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Header 参数

| Header | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| Authorization | string | 否 | API Key 鉴权头，格式 Bearer sk_live_xxx；匿名调用时可省略（每日 10 次免费） | Bearer sk_live_xxxxxxxxxxxxxx |
| Content-Type | string | 否 | 支持 application/x-www-form-urlencoded 或 application/json | application/json |

### Request Body

- Content-Type：`application/json`
- Body 必填：是

| 字段 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| text | string | 是 | 待合成文本，1-500 字符（中英文均按 1 字符计） | 欢迎使用语音合成服务 |
| voice_type | string | 否 | 音色代码。可选：female_zhubo（女声主播） / male_zhubo（男声主播） / male_rap（男声说唱） / female_sichuan（女声四川话） / male_db（男声低沉）。默认 female_zhubo | female_zhubo |

请求体示例：

```json
{
  "text": "欢迎使用语音合成服务",
  "voice_type": "female_zhubo"
}
```

### cURL 模板

```bash
curl -sS \
  -X POST \
  -H "X-API-Key: $APIZERO_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"text": "欢迎使用语音合成服务", "voice_type": "female_zhubo"}' \
  "https://v1.apizero.cn/api/tts"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "text": "欢迎使用语音合成服务",
    "text_length": 10,
    "voice_type": "female_zhubo",
    "voice_name": "女声主播",
    "voice_desc": "标准普通话女声，主播风格，适合资讯播报",
    "audio": "SUQzAwAAAAAAAAAAAAAAA...（约 17000 字符 base64）",
    "audio_format": "mp3",
    "audio_mime": "audio/mpeg",
    "audio_size_bytes": 12750,
    "audio_data_url": "data:audio/mpeg;base64,SUQzAwAAAAA..."
  },
  "request_id": "abc123def456"
}
```

<a id="wzry-battle"></a>

## 29. 王者荣耀战绩查询 `wzry-battle`

通过王者营地小程序接口查询玩家战报，支持搜索玩家、查询角色列表、获取历史战绩与 KDA 统计。

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/wzry-battle` |
| Method | `POST` |
| 分类 | 内容娱乐 (`content`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 2 / s |
| 每日免费额度 | 登录用户 20 次；匿名 5 次 |
| 增长层级 | D-inventory |
| 推广动作 | 先收录 |
| 推广角度 | 先放入接口库，等站内调用量或搜索需求上来后再写专项内容。 |
| 官网文档 | https://apizero.cn/aidocs/wzry-battle |
| Marketplace | https://apizero.cn/marketplace/wzry-battle |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Request Body

- Content-Type：`application/json`
- Body 必填：是

| 字段 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| action | string | 是 | 操作类型：search / roles / battles |  |
| keyword | string | 否 | [search] 玩家昵称关键词 |  |
| user_id | string | 否 | [roles/battles] 玩家 userId |  |
| role_id | string | 否 | [battles] 角色 roleId |  |
| last_time | number | 否 | [battles] 翻页游标（Unix 时间戳），0=最新 |  |
| option | number | 否 | [battles] 模式筛选：0=全部 1=排位 2=巅峰 3=娱乐 |  |
| pages | number | 否 | [battles] 拉取页数（1~3，每页约 30 条） |  |

### cURL 模板

```bash
curl -sS \
  -X POST \
  -H "X-API-Key: $APIZERO_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"action": "<action>", "keyword": "<keyword>", "user_id": "<user_id>", "role_id": "<role_id>", "last_time": "<last_time>", "option": "<option>", "pages": "<pages>"}' \
  "https://v1.apizero.cn/api/wzry-battle"
```

### 响应示例 `200`

```json
{
  "code": 200,
  "message": "success",
  "data": {}
}
```

<a id="crazy-thursday"></a>

## 30. 疯狂星期四文案 `crazy-thursday`

随机返回一段脑洞大开的 KFC「疯狂星期四」文学。内置 52 条精选文案，覆盖情感/搞笑/职场/文艺/学术/古风/悬疑/科幻/鸡汤/日常 10 个分类。支持随机、批量、分类列表与疯四倒计时。

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/crazy-thursday` |
| Method | `GET` |
| 分类 | 生活服务 (`life`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 5 / s |
| 每日免费额度 | 登录用户 20000 次；匿名 10000 次 |
| 增长层级 | D-inventory |
| 推广动作 | 先收录 |
| 推广角度 | 先放入接口库，等站内调用量或搜索需求上来后再写专项内容。 |
| 官网文档 | https://apizero.cn/aidocs/crazy-thursday |
| Marketplace | https://apizero.cn/marketplace/crazy-thursday |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Query 参数

| 参数 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| action | string | 否 | 操作：random（默认）/ batch / categories / countdown | random |
| category | string | 否 | 分类筛选（仅 random/batch 用）：情感/搞笑/职场/文艺/学术/古风/悬疑/科幻/鸡汤/日常 | 搞笑 |
| count | number | 否 | 批量数量（仅 action=batch 用），1-20，默认 5 | 5 |

### Header 参数

| Header | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| Authorization | string | 否 |  |  |

### cURL 模板

```bash
curl -sS \
  -X GET \
  -H "X-API-Key: $APIZERO_API_KEY" \
  "https://v1.apizero.cn/api/crazy-thursday?action=random&category=搞笑"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "text": "我是秦始皇，我打下了万里江山，统一了六国文字和度量衡，但是我没有统一KFC疯狂星期四的价格。V朕50。",
    "category": "搞笑",
    "is_thursday": true,
    "thursday_tip": "今天就是疯狂星期四！冲！"
  },
  "request_id": "abc123"
}
```

<a id="content-extract"></a>

## 31. 网页正文提取 `content-extract`

从新闻、博客、公众号等网页抽取主体正文，自动去除导航/侧栏/广告/评论。基于文本密度算法，同时返回标题、发布时间、配图列表、字数与预估阅读时长。

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/content-extract` |
| Method | `GET` |
| 分类 | 开发工具 (`dev`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 5 / s |
| 每日免费额度 | 登录用户 10000 次；匿名 5000 次 |
| 增长层级 | B-support |
| 推广动作 | 补充推广 |
| 推广角度 | 网页正文提取；适合 RAG、AI 摘要、内容采集前清洗。 |
| 官网文档 | https://apizero.cn/aidocs/content-extract |
| Marketplace | https://apizero.cn/marketplace/content-extract |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Query 参数

| 参数 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| url | string | 是 | 目标网页 URL |  |

### cURL 模板

```bash
curl -sS \
  -X GET \
  -H "X-API-Key: $APIZERO_API_KEY" \
  "https://v1.apizero.cn/api/content-extract?url=https://apizero.cn"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "title": "示例文章标题",
    "publish_time": "2024-01-15",
    "content": "...",
    "word_count": 2300,
    "reading_time": "5分钟",
    "image_count": 3,
    "images": [
      "https://..."
    ]
  }
}
```

<a id="visits-counter"></a>

## 32. 访问量计数器 `visits-counter`

SVG/JSON访问量计数器，支持多套动效主题卡片（咒纹夜/术式蓝焰/五条悟风/无量空处/宿傩咒印等），可嵌入GitHub README或网站。按站点隔离计数。

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/visits-counter` |
| Method | `GET` |
| 分类 | 开发工具 (`dev`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 10 / s |
| 每日免费额度 | 登录用户 1000 次；匿名 500 次 |
| 增长层级 | D-inventory |
| 推广动作 | 先收录 |
| 推广角度 | 先放入接口库，等站内调用量或搜索需求上来后再写专项内容。 |
| 官网文档 | https://apizero.cn/aidocs/visits-counter |
| Marketplace | https://apizero.cn/marketplace/visits-counter |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Query 参数

| 参数 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| site | string | 否 | 站点标识，用于区分不同来源（建议传你的域名）。不传时全局共享一个计数器。 | apizero.cn |
| name | string | 否 | 计数器名称，同一站点下可挂多个不同位置（首页/产品页/文章页等）。默认 demo。 | home |
| mode | string | 否 | 计数模式：daily=每日清零 / total=累计不清零。默认 daily。 | total |
| theme | string | 否 | 主题：默认 gojo_board（像素角色举牌+数字 GIF 动画）。可选 gojo_board/hoops_board/zero_board/jjk_infinity/rule34/moebooru/gelbooru/cursed_night/seal_blue/talisman_red/void_gold/gojo_satoru/infinity_void/sukuna_mark。前 7 个为像素牌主题（角色帧动画），后 7 个为 SVG 渐变主题。 | gojo_board |
| format | string | 否 | 输出格式：svg（默认，可直接 <img> 嵌入网页）/ png（静态图，scale=1~4）/ json（原始 JSON 数据，便于自集成）。 | svg |
| length | number | 否 | 数字位数 4~12，默认 7（前导补 0）。 | 7 |
| no_increment | number | 否 | 只读模式：1=只查询不递增（适合预览/调试）。默认 0。 | 0 |

### cURL 模板

```bash
curl -sS \
  -X GET \
  -H "X-API-Key: $APIZERO_API_KEY" \
  "https://v1.apizero.cn/api/visits-counter?site=apizero.cn&name=home"
```

### 响应示例 `200`

```json
{
  "code": "200",
  "desc": "success",
  "data": {
    "name": "home",
    "mode": "daily",
    "theme": "gojo_board",
    "theme_name": "像素牌-苍空",
    "value": 42,
    "display_value": "0000042",
    "record": {
      "total": 1024,
      "daily": 42,
      "day": "2026-05-09",
      "updated_at": "2026-05-09T21:48:52+08:00"
    },
    "incremented": true,
    "step": 1,
    "length": 7,
    "format": "json"
  },
  "tips": "极数本源 · https://apizero.cn"
}
```

<a id="doubao-image"></a>

## 33. 豆包图片生成 `doubao-image`

基于字节跳动豆包 Seedream 3.0 大模型的高质量文生图服务，支持中英文混合提示词，平均 3~4 秒出图。

【核心能力】
- 影视级画质：擅长摄影、插画、电影感构图与中文场景理解
- 多种尺寸：方形 1024×1024、宽屏 1792×1024、竖屏 1024×1792 等 6 种主流分辨率
- 中文友好：原生理解中文语意，无需翻译为英文也能精准生成

【适用场景】
- 内容创作：插图、海报、Banner、社交媒体图片
- 营销素材：广告图、商品概念图、宣传背景
- 设计辅助：灵感参考、构图草稿、风格化展示
- 应用集成：聊天机器人配图、自媒体配图、AI 应用接入

【提示词建议】
- 推荐结构：主体 + 风格 + 构图 + 光线 + 氛围
- 示例：「一只赛博朋克猫在雨夜的霓虹街头，低角度，电影感，Wong Kar-Wai 风格」
- 越具体描述效果越好，可以指定材质、镜头、色调、画家风格等
- 建议 prompt 长度控制在 50~300 字符之间

【⚠️ 重要提示】
返回的图片 URL 是字节云 TOS 临时直链，**24 小时后失效**。请务必在有效期内：
- 下载保存到本地
- 或转存到自己的对象存储（阿里 OSS / 腾讯 COS 等）
- 不要长期引用此直链

【计费说明】
- 方形 1024×1024：约 4096 tokens
- 宽屏 1792×1024 / 1080p：约 7168 tokens
- 实际点数扣费请参考下方定价

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/doubao-image` |
| Method | `POST` |
| 分类 | AI 能力 (`ai`) |
| 计费 | 按次付费 · 月套餐 · 单次 0 积分 |
| QPS | 2 / s |
| 每日免费额度 | 登录用户 20 次；匿名 1 次 |
| 增长层级 | D-inventory |
| 推广动作 | 先收录 |
| 推广角度 | 先放入接口库，等站内调用量或搜索需求上来后再写专项内容。 |
| 官网文档 | https://apizero.cn/aidocs/doubao-image |
| Marketplace | https://apizero.cn/marketplace/doubao-image |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Header 参数

| Header | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| Authorization | string | 是 | API Key 鉴权（在控制台申请） |  |
| Content-Type | string | 否 | 使用 JSON body 时设置。也可通过 form-urlencoded 或 query string 传参。 |  |

### Request Body

- Content-Type：`application/json`
- Body 必填：是

| 字段 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| prompt | string | 是 | 图片描述文本，支持中英文混合。建议 50~300 字符，越具体效果越好。 | 一只赛博朋克猫在雨夜的霓虹街头，低角度，电影感，Wong Kar-Wai 风格 |
| size | string | 否 | 图片尺寸，可选值：1024x1024（方形，默认）/ 1792x1024（宽屏 16:9）/ 1024x1792（竖屏 9:16）/ 1280x720 / 720x1280 / 1920x1080。 | 1024x1024 |

请求体示例：

```json
{
  "prompt": "一只赛博朋克猫在雨夜的霓虹街头，低角度，电影感，Wong Kar-Wai 风格",
  "size": "1024x1024"
}
```

### cURL 模板

```bash
curl -sS \
  -X POST \
  -H "X-API-Key: $APIZERO_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"prompt": "一只赛博朋克猫在雨夜的霓虹街头，低角度，电影感，Wong Kar-Wai 风格", "size": "1024x1024"}' \
  "https://v1.apizero.cn/api/doubao-image"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "url": "https://ark-content-generation-v2-cn-beijing.tos-cn-beijing.volces.com/doubao-seedream-3-0-t2i/021777940499xxx_0.jpeg?X-Tos-Algorithm=TOS4-HMAC-SHA256&X-Tos-Expires=86400&X-Tos-Signature=...",
    "size": "1024x1024",
    "prompt": "一只赛博朋克猫在雨夜的霓虹街头，低角度，电影感，Wong Kar-Wai 风格",
    "created": 1777940499,
    "expires_in": 86400,
    "tokens": 4096
  },
  "request_id": "mqx8x12345abc"
}
```

<a id="dns-check"></a>

## 34. DNS 劫持检测 `dns-check`

通过 4 家公共 DoH（DNS over HTTPS）服务器并发查询同一域名，对比解析结果检测 DNS 是否被运营商劫持、终端 hosts 篡改或遭遇异常 GeoDNS 分流。

服务器列表：
  • AliDNS（阿里云 - 中国）
  • DNSPod（腾讯 - 中国）
  • 360 安全 DNS（中国）
  • Cloudflare（美国 - 在中国大陆服务器侧可能超时，会被标记为 timeout）

特性：
  • A + AAAA 记录同时查询（IPv4 / IPv6）
  • 提取 CNAME 链路（CDN 跳转可见）
  • TTL 信息汇总（缓存提示）
  • 智能风险分级算法：safe / safe_with_geodns / suspicious / hijack_likely / unknown
  • 支持各类输入：domain / https://domain / domain:port / domain/path（自动剥离）

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/dns-check` |
| Method | `GET` |
| 分类 | 开发工具 (`dev`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 5 / s |
| 每日免费额度 | 登录用户 2000 次；匿名 50 次 |
| 增长层级 | D-inventory |
| 推广动作 | 先收录 |
| 推广角度 | 先放入接口库，等站内调用量或搜索需求上来后再写专项内容。 |
| 官网文档 | https://apizero.cn/aidocs/dns-check |
| Marketplace | https://apizero.cn/marketplace/dns-check |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Query 参数

| 参数 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| domain | string | 是 | 要检测的域名（如 baidu.com，自动剥离 http(s):// 协议头与路径，最长 253 字符） |  |

### Header 参数

| Header | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| X-API-Key | string | 否 | API Key（不传走匿名额度） |  |

### cURL 模板

```bash
curl -sS \
  -X GET \
  -H "X-API-Key: $APIZERO_API_KEY" \
  "https://v1.apizero.cn/api/dns-check?domain=<domain>"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "domain": "taobao.com",
    "input": "https://www.taobao.com/page",
    "exec_ms": 312,
    "summary": {
      "status_text": "DNS 解析正常",
      "risk_level": "safe",
      "risk_score": 95,
      "explain": "4 家 DoH 全部相互验证通过，未检测到劫持迹象。",
      "cname_chain": [
        "www.taobao.com.danuoyi.tbcache.com"
      ],
      "success_count": 3,
      "total_count": 4,
      "unique_ipv4_count": 8,
      "unique_ipv6_count": 0
    },
    "unique_ipv4": [
      "59.82.121.163",
      "59.82.122.130",
      "59.82.122.140",
      "59.82.122.165",
      "59.82.43.234",
      "59.82.43.238",
      "59.82.43.239",
      "59.82.44.240"
    ],
    "unique_ipv6": [],
    "servers": [
      {
        "key": "alidns",
        "name": "AliDNS",
        "status": "ok",
        "error": null,
        "latency_ms": 105,
        "ipv4": [
          "59.82.121.163",
          "59.82.122.130",
          "..."
        ],
        "ipv6": [],
        "cname_chain": [
          "www.taobao.com.danuoyi.tbcache.com"
        ],
        "ttl_min": 60
      },
      {
        "key": "dnspod",
        "name": "DNSPod",
        "status": "ok",
        "latency_ms": 180,
        "ipv4": [
          "59.82.121.163",
          "..."
        ]
      },
      {
        "key": "china360",
        "name": "360",
        "status": "ok",
        "latency_ms": 117
      },
      {
        "key": "cloudflare",
        "name": "Cloudflare",
        "status": "timeout",
        "error": "Operation timed out after 6000 ms",
        "latency_ms": 6000
      }
    ]
  },
  "request_id": "mota..."
}
```

<a id="email-check"></a>

## 35. 邮箱地址检测 `email-check`

综合性邮箱质量评估接口，一次请求完成 6 项检测：
  ① RFC 5322 格式校验
  ② 临时/一次性邮箱检测（基于 72,345 条开源域名库，3 个数据源合并去重）
  ③ MX 记录验证（用 AliDNS DoH，避开 PHP getmxrr 不稳定）
  ④ 拼写纠正（gmial.com → 提示 gmail.com）
  ⑤ 服务商识别（QQ邮箱/Gmail/网易/Outlook 等 40+ 主流邮箱）
  ⑥ 综合风险评分 0-100，附详细原因清单

典型用例：用户注册时的反垃圾邮箱过滤、邮件营销列表清洗、KYC 风控辅助。

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/email-check` |
| Method | `GET` |
| 分类 | 身份核验 (`kyc`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 10 / s |
| 每日免费额度 | 登录用户 500 次；匿名 100 次 |
| 增长层级 | D-inventory |
| 推广动作 | 先收录 |
| 推广角度 | 先放入接口库，等站内调用量或搜索需求上来后再写专项内容。 |
| 官网文档 | https://apizero.cn/aidocs/email-check |
| Marketplace | https://apizero.cn/marketplace/email-check |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Query 参数

| 参数 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| email | string | 是 | 要检测的邮箱地址，最长 254 字符（RFC 上限） |  |

### Header 参数

| Header | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| X-API-Key | string | 否 | API Key（不传走匿名额度） |  |

### cURL 模板

```bash
curl -sS \
  -X GET \
  -H "X-API-Key: $APIZERO_API_KEY" \
  "https://v1.apizero.cn/api/email-check?email=<email>"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "email": "test@gmial.com",
    "input": "test@gmial.com",
    "valid_format": true,
    "local": "test",
    "domain": "gmial.com",
    "is_disposable": false,
    "disposable_match": null,
    "is_trusted": false,
    "provider": null,
    "has_mx": false,
    "mx_records": [],
    "spelling_suggestion": "gmail.com",
    "risk_score": 5,
    "risk_level": "invalid",
    "reasons": [
      "⚠️ 域名无 MX 记录，无法接收邮件",
      "⚠️ 域名疑似拼写错误，建议用 gmail.com",
      "⚠️ 本地部分含测试/系统类关键词"
    ]
  },
  "request_id": "mota..."
}
```

<a id="bus-realtime"></a>

## 36. 实时公交到站 `bus-realtime`

输入城市和站名，实时查询该站各公交线路的到站信息：车牌、预计到站时间、剩余站数、票价、终点方向。

覆盖全国数百城市，支持双向查询（direction）。数据实时、结构清晰，适用于出行助手、公交小程序、生活服务集成等场景。完全免费。

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/bus-realtime` |
| Method | `POST` |
| 分类 | 生活服务 (`life`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 10 / s |
| 每日免费额度 | 登录用户 1000 次；匿名 500 次 |
| 增长层级 | B-support |
| 推广动作 | 补充推广 |
| 推广角度 | 实时公交 API；适合城市出行小程序、通勤提醒工具。 |
| 官网文档 | https://apizero.cn/aidocs/bus-realtime |
| Marketplace | https://apizero.cn/marketplace/bus-realtime |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Header 参数

| Header | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| Content-Type | string | 是 | 请求体格式 |  |

### Request Body

- Content-Type：`application/json`
- Body 必填：是

| 字段 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| city | string | 是 | 城市名 | 长沙 |
| station | string | 是 | 站名或关键词（兼容别名 line） | 五一广场 |
| direction | number | 否 | 方向：1=默认，2=反方向 | 1 |

请求体示例：

```json
{
  "city": "长沙",
  "station": "五一广场",
  "direction": "1"
}
```

### cURL 模板

```bash
curl -sS \
  -X POST \
  -H "X-API-Key: $APIZERO_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"city": "长沙", "station": "五一广场", "direction": "1"}' \
  "https://v1.apizero.cn/api/bus-realtime"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "city": "长沙",
    "station": "五一广场",
    "direction": 1,
    "line_count": 2,
    "lines": [
      {
        "line": "401路",
        "price": "2",
        "terminal": "汽车西站",
        "bus_count": 1,
        "buses": [
          {
            "bus_id": "湘A02882D",
            "arrival_time": "2026-07-01 12:34",
            "arrival_timestamp": 1751344440000,
            "travel_minutes": 6,
            "stops_remaining": 5,
            "status": "5站"
          }
        ]
      }
    ],
    "updated_at": "2026-07-01 12:30:00"
  },
  "request_id": "a1b2c3d4"
}
```

<a id="wallpaper"></a>

## 37. 随机壁纸 `wallpaper`

对接 360 公开壁纸库，16 个分类 × 7 种分辨率（含 1920×1080 原图）随机壁纸。

• **16 个分类**：美女 / 风景 / 游戏 / 影视 / 时尚 / 明星 / 汽车 / 萌宠 / 清新 / 体育 / 萌娃 / 军事 / 动漫 / 日历 / 爱情 / 格言
• **7 种分辨率**：1920×1080（原图）/ 1600×900 / 1440×900 / 1366×768 / 1280×800 / 1280×1024 / 1024×768
• **修复源码 BUG**：源码注释默认 1920×1080 但代码默认 1600×900 且 resMap 缺 1920×1080 → 现已用上游 url 字段返回真正 1920×1080 原图
• **修复源码 BUG**：源码 tag 字段返回内部标记（如 `_category_海洋天堂_`）不可读 → 现已拆分 utag 输出为 tags 数组（如 `['海洋天堂','日出东方','松树','海岛']`）
• **修复源码 BUG**：上游图片 URL 是 http:// → 现已强制改写为 https://，避免 HTTPS 站点混合内容警告
• **缓存优化**：上游响应按 (cid, start_bucket) 缓存 1h，shuffle 在本地做，**节省 80% 上游请求同时保证随机性**
• **典型场景**：登录页背景 / 桌面壁纸应用 / 文章封面图 / 小程序首页轮播

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/wallpaper` |
| Method | `GET` |
| 分类 | 生活服务 (`life`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 20 / s |
| 每日免费额度 | 登录用户 10000 次；匿名 2000 次 |
| 增长层级 | D-inventory |
| 推广动作 | 先收录 |
| 推广角度 | 先放入接口库，等站内调用量或搜索需求上来后再写专项内容。 |
| 官网文档 | https://apizero.cn/aidocs/wallpaper |
| Marketplace | https://apizero.cn/marketplace/wallpaper |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Query 参数

| 参数 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| category | string | 否 | 分类中文名，可选：美女 / 风景 / 游戏 / 影视 / 时尚 / 明星 / 汽车 / 萌宠 / 清新 / 体育 / 萌娃 / 军事 / 动漫 / 日历 / 爱情 / 格言。默认「风景」 | 风景 |
| resolution | string | 否 | 分辨率，可选：1920x1080（原图）/ 1600x900 / 1440x900 / 1366x768 / 1280x800 / 1280x1024 / 1024x768。默认 1920x1080 | 1920x1080 |
| count | number | 否 | 返回数量，1-20。默认 1 | 5 |

### cURL 模板

```bash
curl -sS \
  -X GET \
  -H "X-API-Key: $APIZERO_API_KEY" \
  "https://v1.apizero.cn/api/wallpaper?category=风景&resolution=1920x1080"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "category": "风景",
    "category_id": 9,
    "resolution": "1920x1080",
    "count": 2,
    "requested": 2,
    "images": [
      {
        "url": "https://p2.qhimg.com/bdr/__85/t019ca75cd449be50c1.jpg",
        "id": 2054209,
        "resolution": "1920x1080",
        "tags": [
          "海洋天堂",
          "日出东方",
          "松树",
          "海岛"
        ],
        "tag_text": "海洋天堂 日出东方 松树 海岛"
      },
      {
        "url": "https://p2.qhimg.com/bdr/__85/t021abc.jpg",
        "id": 2054210,
        "resolution": "1920x1080",
        "tags": [
          "雪山",
          "日出",
          "云海"
        ],
        "tag_text": "雪山 日出 云海"
      }
    ]
  },
  "request_id": "abc123def456"
}
```

<a id="text-similarity"></a>

## 38. 文本相似度 `text-similarity`

纯本地计算的文本相似度比对，输入两段文本（每段 ≤ 5000 字）输出 4 种相似度指标 + 加权综合评分 + 5 级评级。

• **零延迟零成本**：纯 PHP 本地计算，无外部上游依赖，平均响应 < 100ms
• **4 种指标**：余弦相似度（35%）+ Jaccard 系数（25%）+ 编辑距离归一化（20%）+ LCS 比率（20%）
• **5 级评级**：几乎相同 / 高度相似 / 中度相似 / 轻度相似 / 差异较大
• **修复源码 bug**：自实现 mb_levenshtein 字符级编辑距离，杜绝 PHP 内置 levenshtein 按字节计算导致的 85 汉字崩溃问题
• **超长保护**：超过 500 字符自动截取并按比例还原，5000×5000 字符比对约 60-80ms
• **典型场景**：评论查重、AI 生成内容检测、多语言翻译质量评估、客服话术匹配

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/text-similarity` |
| Method | `POST` |
| 分类 | 开发工具 (`dev`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 10 / s |
| 每日免费额度 | 登录用户 50000 次；匿名 10000 次 |
| 增长层级 | D-inventory |
| 推广动作 | 先收录 |
| 推广角度 | 先放入接口库，等站内调用量或搜索需求上来后再写专项内容。 |
| 官网文档 | https://apizero.cn/aidocs/text-similarity |
| Marketplace | https://apizero.cn/marketplace/text-similarity |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Header 参数

| Header | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| Authorization | string | 否 | API Key 鉴权头，格式 Bearer sk_live_xxx；匿名调用时可省略（每日 100 次免费） | Bearer sk_live_xxxxxxxxxxxxxx |
| Content-Type | string | 否 | 支持 application/x-www-form-urlencoded 或 application/json | application/json |

### Request Body

- Content-Type：`application/json`
- Body 必填：是

| 字段 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| text1 | string | 是 | 第一段文本，1-5000 字符（中英文均按 1 字符计） | 今天天气不错，适合出门散步 |
| text2 | string | 是 | 第二段文本，1-5000 字符 | 今天天气真好，适合出门走走 |

请求体示例：

```json
{
  "text1": "今天天气不错，适合出门散步",
  "text2": "今天天气真好，适合出门走走"
}
```

### cURL 模板

```bash
curl -sS \
  -X POST \
  -H "X-API-Key: $APIZERO_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"text1": "今天天气不错，适合出门散步", "text2": "今天天气真好，适合出门走走"}' \
  "https://v1.apizero.cn/api/text-similarity"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "text1_length": 13,
    "text2_length": 13,
    "overall_score": 0.6471,
    "similarity_level": "moderately_similar",
    "level_name": "中度相似",
    "metrics": {
      "cosine": 0.5833,
      "jaccard": 0.4118,
      "edit_similarity": 0.6923,
      "lcs_similarity": 0.9231,
      "edit_distance": 4,
      "lcs_length": 12
    },
    "truncated": false
  },
  "request_id": "abc123def456"
}
```

<a id="unshort"></a>

## 39. 短链还原 `unshort`

完整还原一个短链的全部跳转链路，输出每跳的状态码、跳转方式（Location/Meta-Refresh）和耗时。适合钓鱼链接溯源、推广短链分析、二维码内容核对。

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/unshort` |
| Method | `GET` |
| 分类 | 开发工具 (`dev`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 5 / s |
| 每日免费额度 | 登录用户 2000 次；匿名 300 次 |
| 增长层级 | D-inventory |
| 推广动作 | 先收录 |
| 推广角度 | 先放入接口库，等站内调用量或搜索需求上来后再写专项内容。 |
| 官网文档 | https://apizero.cn/aidocs/unshort |
| Marketplace | https://apizero.cn/marketplace/unshort |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Query 参数

| 参数 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| url | string | 是 | 要展开的短链 | https://t.cn/A6xxxx |
| max_hops | number | 否 | 最大跳转次数（1-30，默认 10） |  |

### cURL 模板

```bash
curl -sS \
  -X GET \
  -H "X-API-Key: $APIZERO_API_KEY" \
  "https://v1.apizero.cn/api/unshort?url=https://t.cn/A6xxxx"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "original_url": "https://t.cn/Aabc",
    "final_url": "https://example.com/landing",
    "hops": 2,
    "total_time_ms": 412,
    "chain": [
      {
        "hop": 1,
        "url": "https://t.cn/Aabc",
        "status": 302,
        "method": "Location",
        "duration_ms": 120,
        "next": "https://example.com/landing"
      },
      {
        "hop": 2,
        "url": "https://example.com/landing",
        "status": 200,
        "method": "final",
        "duration_ms": 292
      }
    ],
    "is_redirect": true
  }
}
```

<a id="risk-score"></a>

## 40. 综合风控评分 `risk-score`

输入手机号 / IP / 邮箱，秒级输出 0~100 风险分与放行/二次验证/拦截建议，专治注册薅羊毛、批量养号、营销作弊。三维信号：手机号识别虚拟运营商与物联网卡号段，IP 识别代理/VPN/机房与风险分，邮箱识别临时邮箱与 MX 异常。一次调用拿到可直接用于业务决策的结论。

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/risk-score` |
| Method | `POST` |
| 分类 | 身份核验 (`kyc`) |
| 计费 | 按次付费 · 单次 0 积分 |
| QPS | 2 / s |
| 每日免费额度 | 登录用户 200 次；匿名 10 次 |
| 增长层级 | D-inventory |
| 推广动作 | 先收录 |
| 推广角度 | 先放入接口库，等站内调用量或搜索需求上来后再写专项内容。 |
| 官网文档 | https://apizero.cn/aidocs/risk-score |
| Marketplace | https://apizero.cn/marketplace/risk-score |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Header 参数

| Header | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| Authorization | string | 是 |  |  |

### Request Body

- Content-Type：`application/json`
- Body 必填：否

| 字段 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| mobile | string | 否 | 手机号（11 位手机号或 13 位物联网卡号），识别虚拟运营商/物联网卡 | 17012345678 |
| ip | string | 否 | IPv4/IPv6 地址；传 self 检测调用者自身 IP | 47.88.1.1 |
| email | string | 否 | 邮箱地址，识别临时邮箱/MX/本地异常 | abc@guerrillamail.com |
| scene | string | 否 | 业务场景 register/login/order/coupon，默认 register | register |

请求体示例：

```json
{
  "mobile": "17012345678",
  "ip": "47.88.1.1",
  "email": "abc@guerrillamail.com",
  "scene": "register"
}
```

### cURL 模板

```bash
curl -sS \
  -X POST \
  -H "X-API-Key: $APIZERO_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"mobile": "17012345678", "ip": "47.88.1.1", "email": "abc@guerrillamail.com", "scene": "register"}' \
  "https://v1.apizero.cn/api/risk-score"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "risk_score": 88,
    "risk_level": "critical",
    "decision": "reject",
    "checked": {
      "mobile": true,
      "ip": true,
      "email": true
    },
    "hit_rules": [
      {
        "code": "MOBILE_MVNO",
        "weight": 35,
        "desc": "虚拟运营商号段（170），实名宽松，薅羊毛高发"
      },
      {
        "code": "EMAIL_DISPOSABLE",
        "weight": 35,
        "desc": "一次性/临时邮箱域名，典型用于注册套利"
      },
      {
        "code": "IP_DATACENTER",
        "weight": 30,
        "desc": "机房/IDC IP（非真实用户网络，脚本批量常用）"
      }
    ],
    "signals": {
      "mobile": {
        "checked": true,
        "input_mask": "170****5678",
        "valid": true,
        "number_type": "mvno",
        "carrier": "虚拟运营商",
        "risk": "high"
      },
      "email": {
        "checked": true,
        "email": "abc@guerrillamail.com",
        "valid_format": true,
        "is_disposable": true,
        "is_trusted": false,
        "has_mx": true,
        "risk": "high"
      },
      "ip": {
        "checked": true,
        "ip": "47.88.x.x",
        "valid": true,
        "is_private": false,
        "is_proxy": false,
        "is_datacenter": true,
        "province": "",
        "isp": "阿里云",
        "risk": "medium"
      }
    },
    "scene": "register"
  },
  "request_id": "k9x2p4mabc12"
}
```

<a id="nine-grid-cutter"></a>

## 41. 九宫格切图 `nine-grid-cutter`

上传一张图片（multipart 文件 / base64 / URL 三选一），自动居中裁切为正方形后均分为 N×N 宫格小图，按朋友圈发图顺序返回（左到右、上到下编号 1..N²）。支持 2×2 / 3×3 / 4×4，可设置切片间留白，输出 base64 数组或直接下载 zip。

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/nine-grid-cutter` |
| Method | `POST` |
| 分类 | 开发工具 (`dev`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 2 / s |
| 每日免费额度 | 登录用户 5000 次；匿名 2000 次 |
| 增长层级 | D-inventory |
| 推广动作 | 先收录 |
| 推广角度 | 先放入接口库，等站内调用量或搜索需求上来后再写专项内容。 |
| 官网文档 | https://apizero.cn/aidocs/nine-grid-cutter |
| Marketplace | https://apizero.cn/marketplace/nine-grid-cutter |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Header 参数

| Header | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| Authorization | string | 否 |  |  |

### Request Body

- Content-Type：`application/json`
- Body 必填：否

| 字段 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| file | string | 否 | 图片文件（multipart 上传，三选一之一），支持 jpg/png/webp/gif |  |
| image_base64 | string | 否 | 图片 base64（dataURL 或纯 base64，三选一） |  |
| image_url | string | 否 | 公网图片 URL（http/https，三选一） |  |
| grid | number | 否 | 宫格数：2 / 3 / 4，默认 3 | 3 |
| gap | number | 否 | 切片间留白像素 0-20，默认 0 | 0 |
| output | string | 否 | 输出格式：base64（默认，JSON）/ zip（直接返回 application/zip） | base64 |

请求体示例：

```json
{
  "grid": "3",
  "gap": "0",
  "output": "base64"
}
```

### cURL 模板

```bash
curl -sS \
  -X POST \
  -H "X-API-Key: $APIZERO_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"grid": "3", "gap": "0", "output": "base64"}' \
  "https://v1.apizero.cn/api/nine-grid-cutter"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "grid": "3x3",
    "total": 9,
    "gap": 0,
    "original_size": "1080x1920",
    "square_side": 1080,
    "cell_size": "360x360",
    "source": "multipart",
    "pieces": [
      {
        "index": 1,
        "size": "360x360",
        "base64": "iVBORw0K...",
        "data_url": "data:image/png;base64,iVBORw0K..."
      },
      {
        "index": 2,
        "size": "360x360",
        "base64": "...",
        "data_url": "..."
      }
    ]
  },
  "request_id": "abc123"
}
```

<a id="mobile"></a>

## 42. 手机号归属地 `mobile`

通过 11 位中国大陆手机号查询归属省份/城市与运营商，支持移动/联通/电信主流号段及虚拟运营商号段（170/171/174 等）。

• 严格 11 位手机号校验：`^1[3-9]\d{9}$`，非法号码直接返回 4000
• 双形态响应：查询成功 → is_found=true 含 province/carrier；号段未收录 → is_found=false 字段为空（非错误，前端可直接 if 判断）
• 缓存策略：成功结果 7 天（号段分配静态稳定）；未查询到 1 小时（避免长期误判新放号段）
• 隐私保护：错误日志中手机号自动脱敏（138****0000）

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/mobile` |
| Method | `GET` |
| 分类 | 生活服务 (`life`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 10 / s |
| 每日免费额度 | 登录用户 30000 次；匿名 5000 次 |
| 增长层级 | D-inventory |
| 推广动作 | 先收录 |
| 推广角度 | 先放入接口库，等站内调用量或搜索需求上来后再写专项内容。 |
| 官网文档 | https://apizero.cn/aidocs/mobile |
| Marketplace | https://apizero.cn/marketplace/mobile |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Query 参数

| 参数 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| mobile | string | 是 | 11 位中国大陆手机号（必须 1[3-9] 开头） | 13800138000 |

### Header 参数

| Header | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| Authorization | string | 否 | API Key 鉴权头，格式 Bearer sk_live_xxx；匿名调用时可省略（每日 50 次免费） | Bearer sk_live_xxxxxxxxxxxxxx |

### cURL 模板

```bash
curl -sS \
  -X GET \
  -H "X-API-Key: $APIZERO_API_KEY" \
  "https://v1.apizero.cn/api/mobile?mobile=13800138000"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "mobile": "13800138000",
    "province": "北京",
    "carrier": "中国移动",
    "is_found": true
  },
  "request_id": "abc123def456"
}
```

<a id="icp"></a>

## 43. ICP 备案查询 `icp`

通过域名查询工信部 ICP 备案信息，返回备案号、网站名称、主办单位、单位性质（企业/个人/事业单位等）和审核通过时间。

• 自动域名清洗：自动剥离 https?:// 协议、/path 路径、:port 端口、www. 前缀，无论传入 https://www.baidu.com/abc 还是 baidu.com 都正确识别

• 已备案 → is_filed=true，返回完整 6 字段；未备案/境外/已注销 → is_filed=false，返回空字段（非错误，便于前端 if 判断）

• 缓存策略：已备案 24 小时缓存（变更频率低），未备案 1 小时缓存（避免新备案被长期误判）

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/icp` |
| Method | `GET` |
| 分类 | 开发工具 (`dev`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 5 / s |
| 每日免费额度 | 登录用户 100 次；匿名 30 次 |
| 增长层级 | D-inventory |
| 推广动作 | 先收录 |
| 推广角度 | 先放入接口库，等站内调用量或搜索需求上来后再写专项内容。 |
| 官网文档 | https://apizero.cn/aidocs/icp |
| Marketplace | https://apizero.cn/marketplace/icp |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Query 参数

| 参数 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| domain | string | 是 | 要查询的域名，支持完整 URL 输入（自动剥离协议/路径/端口/www）。例如：baidu.com、https://www.baidu.com/abc、m.baidu.com:8080/foo 都等价 | baidu.com |

### Header 参数

| Header | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| Authorization | string | 否 | API Key 鉴权头，格式 Bearer sk_live_xxx；匿名调用时可省略（每日 30 次免费） | Bearer sk_live_xxxxxxxxxxxxxx |

### cURL 模板

```bash
curl -sS \
  -X GET \
  -H "X-API-Key: $APIZERO_API_KEY" \
  "https://v1.apizero.cn/api/icp?domain=baidu.com"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "domain": "baidu.com",
    "is_filed": true,
    "icp_code": "京ICP证030173号-1",
    "site_name": "百度一下，你就知道",
    "company_name": "北京百度网讯科技有限公司",
    "company_type": "企业",
    "audit_time": "2019-05-16 16:06:21"
  },
  "request_id": "abc123def456"
}
```

<a id="bencao"></a>

## 44. 本草纲目·中药查询 `bencao`

传统中药材知识查询接口，覆盖《本草纲目》及常见中药材数据。输入药材名称（如「人参」「丁香」「甘草」「枸杞」），返回该药材的释名、气味、主治、附方等详细记载。

【典型场景】
- 中医养生 / 食疗 App 的药材百科
- 中药知识科普类小程序
- AI 中医问诊辅助参考
- 古籍数字化项目
- 国学 / 中医文化教学

【调用示例】
GET /api/bencao?msg=人参
GET /api/bencao?msg=丁香
GET /api/bencao?msg=甘草

【模糊匹配】
- 精确名称查得到 → 直接返回详情（matched=exact）
- 找不到时返回 4040 + suggestions 数组（最多 10 个相关建议）
- 比如查「人参枸杞」会建议「人参、枸杞」等单味药

【⚠️ 数据来源说明】
本接口数据整理自《本草纲目》及网络公开整理资料，仅供学习参考；中医药疗用请遵医嘱。

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/bencao` |
| Method | `GET` |
| 分类 | 生活服务 (`life`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 10 / s |
| 每日免费额度 | 登录用户 10000 次；匿名 1000 次 |
| 增长层级 | D-inventory |
| 推广动作 | 先收录 |
| 推广角度 | 先放入接口库，等站内调用量或搜索需求上来后再写专项内容。 |
| 官网文档 | https://apizero.cn/aidocs/bencao |
| Marketplace | https://apizero.cn/marketplace/bencao |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Query 参数

| 参数 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| msg | string | 是 | 药品名称（中文），如：人参、丁香、甘草、当归、枸杞。最长 50 个字符。 | 人参 |

### Header 参数

| Header | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| Authorization | string | 否 | 可选 API Key 鉴权。未登录每天 30 次免费体验 |  |

### cURL 模板

```bash
curl -sS \
  -X GET \
  -H "X-API-Key: $APIZERO_API_KEY" \
  "https://v1.apizero.cn/api/bencao?msg=人参"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "name": "人参",
    "detail": "「释名」黄参、神草、土精、血参...\n「气味」（根）甘、温、无毒...\n「主治」补五脏，安精神...",
    "matched": "exact"
  },
  "request_id": "mqx8x12345abc"
}
```

<a id="address-parse"></a>

## 45. 中文地址解析 `address-parse`

一行 API 从中文地址字符串中提取「省市区街道 + 详细地址 + 姓名 + 手机号 + 邮编」。

适用场景：电商收货地址自动拆分、快递下单页智能填充、CRM 客户资料清洗、办公地址结构化入库。

特点：纯本地正则算法，无任何上游依赖，毫秒级响应；支持 34 个省级行政区及其简称识别（如「北京」→「北京市」、「新疆」→「新疆维吾尔自治区」）；支持快递场景下「张三 138xxxxxxxx 上海市浦东新区xx路xx号 200000」这类混合输入的自动拆分。

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/address-parse` |
| Method | `POST` |
| 分类 | 地理位置 (`geo`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 20 / s |
| 每日免费额度 | 登录用户 1000 次；匿名 100 次 |
| 增长层级 | D-inventory |
| 推广动作 | 先收录 |
| 推广角度 | 先放入接口库，等站内调用量或搜索需求上来后再写专项内容。 |
| 官网文档 | https://apizero.cn/aidocs/address-parse |
| Marketplace | https://apizero.cn/marketplace/address-parse |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Header 参数

| Header | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| Authorization | string | 否 | Bearer sk_live_xxx（可选；未登录匿名调用受更严格的限流） |  |

### Request Body

- Content-Type：`application/json`
- Body 必填：是

| 字段 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| address | string | 是 | 中文地址字符串。支持姓名/手机/邮编混合输入，长度 ≤ 500 | 张三 13812345678 上海市浦东新区张江镇科苑路88号 201203 |

请求体示例：

```json
{
  "address": "张三 13812345678 上海市浦东新区张江镇科苑路88号 201203"
}
```

### cURL 模板

```bash
curl -sS \
  -X POST \
  -H "X-API-Key: $APIZERO_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"address": "张三 13812345678 上海市浦东新区张江镇科苑路88号 201203"}' \
  "https://v1.apizero.cn/api/address-parse"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "original": "张三 138****1234 上海市浦东新区张江镇科苑路88号 201203",
    "province": "上海市",
    "city": "上海市",
    "district": "浦东新区",
    "street": "张江镇",
    "detail": "科苑路88号",
    "zipcode": "201203",
    "phone": "138****1234",
    "name": "张三"
  },
  "request_id": "kx8n9q2a1b3c4d5e6f7g"
}
```

<a id="hash"></a>

## 46. 哈希加密计算 `hash`

哈希加密计算，支持 12 种主流算法，可一次性返回全部，也可指定单个算法。支持 HMAC 签名模式与 hex / base64 双输出编码，毫秒级返回。

• 通用摘要：MD5、SHA-1、SHA-256、SHA-384、SHA-512

• 现代抗碰撞：SHA3-256、SHA3-512、RIPEMD-160、Whirlpool

• 校验和：CRC32、CRC32B、Adler-32

• HMAC 签名：传入 hmac_key 自动切换为 HMAC 模式（用于 API 签名 / 防篡改场景）

• 输出编码：hex（默认，更可读）/ base64（节省 25% 字节，适合 URL 传输）

纯本地计算，零外部上游依赖，毫秒级响应。HMAC 密钥仅参与计算，不写日志、不缓存、不回显。

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/hash` |
| Method | `GET` |
| 分类 | 开发工具 (`dev`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 20 / s |
| 每日免费额度 | 登录用户 1000 次；匿名 200 次 |
| 增长层级 | D-inventory |
| 推广动作 | 先收录 |
| 推广角度 | 先放入接口库，等站内调用量或搜索需求上来后再写专项内容。 |
| 官网文档 | https://apizero.cn/aidocs/hash |
| Marketplace | https://apizero.cn/marketplace/hash |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Query 参数

| 参数 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| text | string | 是 | 要计算哈希的文本，UTF-8 编码，最长 10000 字节（中文 1 字 = 3 字节，约支持 3300 个中文） | hello |
| algorithm | string | 否 | 算法标识，默认 all。支持：all / md5 / sha1 / sha256 / sha384 / sha512 / sha3-256 / sha3-512 / ripemd160 / whirlpool / crc32 / crc32b / adler32（自动兼容无连字符写法 sha3256） | all |
| encoding | string | 否 | 输出编码：hex（默认，更可读）/ base64（节省 25% 字节） | hex |
| hmac_key | string | 否 | HMAC 密钥，传入则自动切换为 HMAC 模式生成签名；密钥仅参与计算，不写入任何日志或缓存 | mysecret |

### Header 参数

| Header | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| Authorization | string | 否 | API Key 鉴权头，格式 Bearer sk_live_xxx；匿名调用时可省略（每日 200 次免费） | Bearer sk_live_xxxxxxxxxxxxxx |

### cURL 模板

```bash
curl -sS \
  -X GET \
  -H "X-API-Key: $APIZERO_API_KEY" \
  "https://v1.apizero.cn/api/hash?text=hello"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "text_length": 5,
    "text_bytes": 5,
    "encoding": "hex",
    "hmac": false,
    "hash_count": 3,
    "hashes": {
      "md5": {
        "algorithm": "MD5",
        "bits": 128,
        "value": "5d41402abc4b2a76b9719d911017c592",
        "length": 32
      },
      "sha1": {
        "algorithm": "SHA-1",
        "bits": 160,
        "value": "aaf4c61ddcc5e8a2dabede0f3b482cd9aea9434d",
        "length": 40
      },
      "sha256": {
        "algorithm": "SHA-256",
        "bits": 256,
        "value": "2cf24dba5fb0a30e26e83b2ac5b9e29e1b161e5c1fa7425e73043362938b9824",
        "length": 64
      }
    }
  },
  "request_id": "abc123def456"
}
```

<a id="yiyan"></a>

## 47. 一言（简版） `yiyan`

hitokoto 的精简版：纯文本随机一句，不带分类/出处。同时支持 format=text 直接返回纯文本（适合站点页脚、小程序欢迎语等装饰场景）。

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/yiyan` |
| Method | `GET` |
| 分类 | 生活服务 (`life`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 20 / s |
| 每日免费额度 | 登录用户 500000 次；匿名 200000 次 |
| 增长层级 | D-inventory |
| 推广动作 | 先收录 |
| 推广角度 | 先放入接口库，等站内调用量或搜索需求上来后再写专项内容。 |
| 官网文档 | https://apizero.cn/aidocs/yiyan |
| Marketplace | https://apizero.cn/marketplace/yiyan |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Query 参数

| 参数 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| format | string | 否 | json（默认）/ text | json |

### cURL 模板

```bash
curl -sS \
  -X GET \
  -H "X-API-Key: $APIZERO_API_KEY" \
  "https://v1.apizero.cn/api/yiyan?format=json"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "content": "落霞与孤鹜齐飞，秋水共长天一色。",
    "length": 16,
    "total_pool": 370
  }
}
```

<a id="jd-address"></a>

## 48. 京东地址解析（4 级） `jd-address`

调用京东地址智能识别接口，将自然语言地址解析为「省/市/区/镇」四级行政区划，并返回每级对应的京东 ID。比传统三级行政区划接口多一个「镇」级别。

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/jd-address` |
| Method | `POST` |
| 分类 | 地理位置 (`geo`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 5 / s |
| 每日免费额度 | 登录用户 1000 次；匿名 10 次 |
| 增长层级 | D-inventory |
| 推广动作 | 先收录 |
| 推广角度 | 先放入接口库，等站内调用量或搜索需求上来后再写专项内容。 |
| 官网文档 | https://apizero.cn/aidocs/jd-address |
| Marketplace | https://apizero.cn/marketplace/jd-address |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Header 参数

| Header | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| Authorization | string | 是 |  |  |
| Content-Type | string | 是 |  |  |

### Request Body

- Content-Type：`application/json`
- Body 必填：是

| 字段 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| address | string | 是 | 地址文本（≤200 字符） | 北京市朝阳区三里屯街道工体北路 8 号 |

请求体示例：

```json
{
  "address": "北京市朝阳区三里屯街道工体北路 8 号"
}
```

### cURL 模板

```bash
curl -sS \
  -X POST \
  -H "X-API-Key: $APIZERO_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"address": "北京市朝阳区三里屯街道工体北路 8 号"}' \
  "https://v1.apizero.cn/api/jd-address"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "province_id": 1,
    "province": "北京",
    "city_id": 72,
    "city": "北京市",
    "county_id": 2818,
    "county": "朝阳区",
    "town_id": 53124,
    "town": "三里屯街道",
    "detail": "工体北路 8 号"
  }
}
```

<a id="holiday"></a>

## 49. 中国法定节假日 `holiday`

查询中国法定节假日安排，包含放假日期和调休日期，支持2020-2030年

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/holiday` |
| Method | `GET` |
| 分类 | 生活服务 (`life`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 20 / s |
| 每日免费额度 | 登录用户 10000 次；匿名 1000 次 |
| 增长层级 | D-inventory |
| 推广动作 | 先收录 |
| 推广角度 | 先放入接口库，等站内调用量或搜索需求上来后再写专项内容。 |
| 官网文档 | https://apizero.cn/aidocs/holiday |
| Marketplace | https://apizero.cn/marketplace/holiday |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### cURL 模板

```bash
curl -sS \
  -X GET \
  -H "X-API-Key: $APIZERO_API_KEY" \
  "https://v1.apizero.cn/api/holiday"
```

### 响应示例 `200`

```json
{
  "code": 200,
  "message": "success",
  "data": {}
}
```

<a id="invoice"></a>

## 50. 增值税发票识别 `invoice`

增值税发票 OCR 识别，支持增值税专用发票、普通发票、电子普票，输出 22+ 个结构化字段。

• 双输入模式：
  - input_type=url：传入公网可访问的图片 URL（http/https）
  - input_type=base64：传入 base64 编码字符串（≤6MB，自动剥离 data:image/...;base64, 前缀）

• 完整识别字段：
  - 发票基本：发票名称、代码、号码、开票日期、校验码、机器编号
  - 金额：价税合计、税额、不含税金额、大写金额
  - 购销方：名称、纳税人识别号、地址电话、开户行账号
  - 经办人：收款人、复核人、开票人
  - 商品明细：items 数组，含品名/规格/数量/单价/金额/税率/税额
  - 备注与盖章信息

• OCR 结果缓存 1 小时（同图同结果），减少重复调用消耗。

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/invoice` |
| Method | `POST` |
| 分类 | 文档识别 (`ocrdata`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 2 / s |
| 每日免费额度 | 登录用户 300 次；匿名 50 次 |
| 增长层级 | D-inventory |
| 推广动作 | 先收录 |
| 推广角度 | 先放入接口库，等站内调用量或搜索需求上来后再写专项内容。 |
| 官网文档 | https://apizero.cn/aidocs/invoice |
| Marketplace | https://apizero.cn/marketplace/invoice |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Header 参数

| Header | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| Authorization | string | 否 | API Key 鉴权头，格式 Bearer sk_live_xxx；匿名调用时可省略（每日 5 次免费） | Bearer sk_live_xxxxxxxxxxxxxx |
| Content-Type | string | 是 | POST 请求体类型，固定 application/x-www-form-urlencoded；如使用 base64 模式建议 POST | application/x-www-form-urlencoded |

### Request Body

- Content-Type：`application/json`
- Body 必填：是

| 字段 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| input_type | string | 是 | 输入类型：url=图片URL / base64=图片base64编码 | url |
| input_data | string | 是 | 发票图片数据。input_type=url 时为 http/https 完整 URL；input_type=base64 时为 base64 字符串（最大 6MB，可选 data:image/jpeg;base64, 前缀） | https://example.com/invoice.jpg |

请求体示例：

```json
{
  "input_type": "url",
  "input_data": "https://example.com/invoice.jpg"
}
```

### cURL 模板

```bash
curl -sS \
  -X POST \
  -H "X-API-Key: $APIZERO_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"input_type": "url", "input_data": "https://example.com/invoice.jpg"}' \
  "https://v1.apizero.cn/api/invoice"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "invoice_name": "增值税电子普通发票",
    "invoice_code": "011002000311",
    "invoice_no": "12345678",
    "invoice_date": "2024-08-15",
    "check_code": "12345 67890 12345 67890",
    "machine_num": "499099111111",
    "total_price": "94.34",
    "total_tax": "5.66",
    "total_price_and_tax": "100.00",
    "big_total_price_and_tax": "壹佰圆整",
    "buyer": {
      "name": "某某科技有限公司",
      "taxpayer_no": "91110000XXXXXXXXXX",
      "address_phone": "北京市XX区XX路XX号 010-12345678",
      "account": "中国银行 6217001234567890"
    },
    "seller": {
      "name": "某某商贸有限公司",
      "taxpayer_no": "91310000YYYYYYYYYY",
      "address_phone": "上海市XX区XX路XX号 021-87654321",
      "account": "工商银行 6222001234567890"
    },
    "payee": "张三",
    "reviewer": "李四",
    "drawer": "王五",
    "remarks": "",
    "items": [
      {
        "name": "*技术服务*软件开发服务",
        "specification": "",
        "unit": "",
        "quantity": "",
        "unit_price": "",
        "amount": "94.34",
        "tax_rate": "6%",
        "tax": "5.66"
      }
    ]
  },
  "request_id": "abc123def456"
}
```

<a id="douyin-user"></a>

## 51. 抖音用户公开信息 `douyin-user`

通过抖音用户主页链接（短链或长链），获取昵称、头像、签名、作品数、粉丝数、关注数、获赞总数等公开信息。自动展开 v.douyin.com 短链。

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/douyin-user` |
| Method | `GET` |
| 分类 | 内容娱乐 (`content`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 5 / s |
| 每日免费额度 | 登录用户 1000 次；匿名 200 次 |
| 增长层级 | D-inventory |
| 推广动作 | 先收录 |
| 推广角度 | 先放入接口库，等站内调用量或搜索需求上来后再写专项内容。 |
| 官网文档 | https://apizero.cn/aidocs/douyin-user |
| Marketplace | https://apizero.cn/marketplace/douyin-user |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Query 参数

| 参数 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| url | string | 是 | 抖音用户主页链接（v.douyin.com 短链或 douyin.com/user/ 长链） |  |

### cURL 模板

```bash
curl -sS \
  -X GET \
  -H "X-API-Key: $APIZERO_API_KEY" \
  "https://v1.apizero.cn/api/douyin-user?url=<url>"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "nickname": "张三",
    "follower_count": 9999,
    "aweme_count": 123,
    "total_favorited": 100000
  }
}
```

<a id="dns-query"></a>

## 52. DNS 记录查询 `dns-query`

查询域名的各类 DNS 记录，支持 A / AAAA / NS / CNAME / MX / TXT / CAA / SOA 共 8 种标准类型，以及 ANY（一次返回所有类型）。

✨ 特色：
  • 不依赖第三方工具站，完全自建 — 3 家国内 DoH 并发（AliDNS / DNSPod / 360）
  • 所有记录结构化解析：MX 拆 priority/exchange、CAA 拆 flags/tag/value、SOA 拆 mname/serial/...
  • 多家结果合并去重 + 各记录标注来自哪些 DoH（sources 字段）
  • 兼容数字 qtype（如 1=A / 15=MX / 257=CAA），不破坏旧调用方式
  • ANY 模式：单次请求拿到域名的全部 DNS 配置，免去多次查询

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/dns-query` |
| Method | `GET` |
| 分类 | 开发工具 (`dev`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 10 / s |
| 每日免费额度 | 登录用户 500 次；匿名 100 次 |
| 增长层级 | D-inventory |
| 推广动作 | 先收录 |
| 推广角度 | 先放入接口库，等站内调用量或搜索需求上来后再写专项内容。 |
| 官网文档 | https://apizero.cn/aidocs/dns-query |
| Marketplace | https://apizero.cn/marketplace/dns-query |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Query 参数

| 参数 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| host | string | 是 | 域名（自动剥离 http(s)://、路径、端口） |  |
| type | string | 否 | 记录类型，默认 A。支持名称（A/AAAA/NS/CNAME/MX/TXT/CAA/SOA/ANY）或数字编码（1/2/5/6/15/16/28/257） |  |

### Header 参数

| Header | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| X-API-Key | string | 否 | API Key（不传走匿名额度） |  |

### cURL 模板

```bash
curl -sS \
  -X GET \
  -H "X-API-Key: $APIZERO_API_KEY" \
  "https://v1.apizero.cn/api/dns-query?host=<host>"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "host": "baidu.com",
    "input": "baidu.com",
    "type": "MX",
    "type_code": 15,
    "total": 2,
    "exec_ms": 234,
    "sources": [
      "alidns",
      "china360",
      "dnspod"
    ],
    "notes": null,
    "records": [
      {
        "type": "MX",
        "type_code": 15,
        "name": "baidu.com",
        "ttl": 600,
        "data": "10 mx.maillb.baidu.com.",
        "parsed": {
          "priority": 10,
          "exchange": "mx.maillb.baidu.com"
        },
        "sources": [
          "alidns",
          "china360",
          "dnspod"
        ]
      },
      {
        "type": "MX",
        "type_code": 15,
        "name": "baidu.com",
        "ttl": 600,
        "data": "20 mx.baidu.com.",
        "parsed": {
          "priority": 20,
          "exchange": "mx.baidu.com"
        },
        "sources": [
          "alidns",
          "china360",
          "dnspod"
        ]
      }
    ]
  },
  "request_id": "mota..."
}
```

<a id="cdn-ips"></a>

## 53. CDN 优选 IP `cdn-ips`

获取主流 CDN 服务商的官方公网 IP 段（CIDR）列表。当前支持 CloudFlare、AWS CloudFront、Gcore 三家，覆盖 IPv4 与 IPv6。常用于 CDN 优选、反向代理、流量分流、防火墙白名单等运维场景。

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/cdn-ips` |
| Method | `GET` |
| 分类 | 开发工具 (`dev`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 10 / s |
| 每日免费额度 | 登录用户 2000 次；匿名 100 次 |
| 增长层级 | D-inventory |
| 推广动作 | 先收录 |
| 推广角度 | 先放入接口库，等站内调用量或搜索需求上来后再写专项内容。 |
| 官网文档 | https://apizero.cn/aidocs/cdn-ips |
| Marketplace | https://apizero.cn/marketplace/cdn-ips |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Query 参数

| 参数 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| server | string | 是 | CDN 服务商：cloudflare / cloudfront / gcore（兼容 1/2/3 数字） |  |
| type | string | 是 | IP 协议版本：v4（IPv4）/ v6（IPv6） |  |

### Header 参数

| Header | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| X-API-Key | string | 否 | API Key（不传走匿名额度） |  |

### cURL 模板

```bash
curl -sS \
  -X GET \
  -H "X-API-Key: $APIZERO_API_KEY" \
  "https://v1.apizero.cn/api/cdn-ips?server=<server>&type=<type>"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "server": "CloudFlare",
    "type": "v4",
    "count": 15,
    "ips": [
      "173.245.48.0/20",
      "103.21.244.0/22",
      "103.22.200.0/22",
      "...（共 15 条）"
    ],
    "update_time": "2026-05-06 07:55:00"
  },
  "request_id": "mota..."
}
```

<a id="webmeta"></a>

## 54. 网页元数据提取 `webmeta`

提取网页的标题、描述、关键词、OG/Twitter Card、favicon，并自动识别使用的 30+ 项技术栈（React/Vue/Next.js/WordPress/Cloudflare 等）。适合 SEO 检测、技术调研、链接预览。

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/webmeta` |
| Method | `GET` |
| 分类 | 开发工具 (`dev`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 5 / s |
| 每日免费额度 | 登录用户 10000 次；匿名 5000 次 |
| 增长层级 | B-support |
| 推广动作 | 补充推广 |
| 推广角度 | 网页 Meta 信息提取；适合链接预览、采集前预处理、站点分析。 |
| 官网文档 | https://apizero.cn/aidocs/webmeta |
| Marketplace | https://apizero.cn/marketplace/webmeta |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Query 参数

| 参数 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| url | string | 是 | 网页 URL，自动补 https:// | https://baidu.com |

### cURL 模板

```bash
curl -sS \
  -X GET \
  -H "X-API-Key: $APIZERO_API_KEY" \
  "https://v1.apizero.cn/api/webmeta?url=https://apizero.cn"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "url": "https://www.baidu.com/",
    "title": "百度一下，你就知道",
    "tech_stack": [
      "jQuery",
      "Baidu Analytics"
    ],
    "http_code": 200
  }
}
```

<a id="bodyfat"></a>

## 55. 体脂率与 BMI 计算 `bodyfat`

一站式综合计算 BMI、体脂率（Deurenberg 公式）、基础代谢率（Mifflin-St Jeor）、理想体重区间、腰围身高比与健康风险评估。男女区分阈值，附带专业建议。

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/bodyfat` |
| Method | `GET` |
| 分类 | 生活服务 (`life`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 20 / s |
| 每日免费额度 | 登录用户 200000 次；匿名 100000 次 |
| 增长层级 | D-inventory |
| 推广动作 | 先收录 |
| 推广角度 | 先放入接口库，等站内调用量或搜索需求上来后再写专项内容。 |
| 官网文档 | https://apizero.cn/aidocs/bodyfat |
| Marketplace | https://apizero.cn/marketplace/bodyfat |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Query 参数

| 参数 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| weight | number | 是 | 体重（kg） |  |
| height | number | 是 | 身高（米，非厘米！） |  |
| waist | number | 是 | 腰围（cm） |  |
| gender | string | 是 | 性别：男/female/m/f | 男 |
| age | number | 否 | 年龄，默认 30 |  |

### cURL 模板

```bash
curl -sS \
  -X GET \
  -H "X-API-Key: $APIZERO_API_KEY" \
  "https://v1.apizero.cn/api/bodyfat?weight=<weight>&height=<height>&waist=<waist>&gender=男"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "bmi": 22.86,
    "bfp": 18.43,
    "bmr": 1632.5,
    "category": "正常",
    "ideal_weight_min": 56.66,
    "ideal_weight_max": 76.25,
    "waist_height_ratio": 0.46,
    "health_risk": "低",
    "advice": "体脂率正常，保持现有的生活方式。"
  }
}
```

<a id="traffic-weather-alert"></a>

## 56. 限行天气联动 `traffic-weather-alert`

输入国内城市拼音（兼容中文），返回当天的限行尾号。如遇暴雨/台风等恶劣天气，附加居家办公建议。覆盖北京、天津、成都、杭州、贵阳、长春 6 个限行城市，天气支持所有国内主要城市。

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/traffic-weather-alert` |
| Method | `GET` |
| 分类 | 生活服务 (`life`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 3 / s |
| 每日免费额度 | 登录用户 10000 次；匿名 5000 次 |
| 增长层级 | D-inventory |
| 推广动作 | 先收录 |
| 推广角度 | 先放入接口库，等站内调用量或搜索需求上来后再写专项内容。 |
| 官网文档 | https://apizero.cn/aidocs/traffic-weather-alert |
| Marketplace | https://apizero.cn/marketplace/traffic-weather-alert |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Query 参数

| 参数 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| city | string | 是 | 城市拼音（如 beijing/chengdu/hangzhou），兼容中文（如 北京/成都） | beijing |
| action | string | 否 | 操作：restriction（默认）/ cities（支持限行的城市列表） | restriction |

### Header 参数

| Header | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| Authorization | string | 否 |  |  |

### cURL 模板

```bash
curl -sS \
  -X GET \
  -H "X-API-Key: $APIZERO_API_KEY" \
  "https://v1.apizero.cn/api/traffic-weather-alert?city=beijing"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "city": "beijing",
    "city_cn": "北京",
    "date": "2026-05-11",
    "weekday": "周一",
    "restricted_numbers": "5,0",
    "restriction_active": true,
    "weather": {
      "weather": "晴",
      "temperature": "26°C",
      "is_severe": false,
      "severe_type": null
    },
    "work_from_home_advisory": null,
    "message": "今日北京（周一）限行尾号为 5,0"
  },
  "request_id": "abc123"
}
```

<a id="ssl"></a>

## 57. SSL 证书检测 `ssl`

通过域名远程探测其 HTTPS/SSL 证书信息：颁发者、颁发机构、签名算法、覆盖域名、有效期、是否过期、距过期天数、SHA-1 指纹、远端 IP 等。

• 智能预处理：自动剥离 http(s):// 前缀、路径、查询串、端口、www. 子域
• 严格域名校验：合法域名格式（最长 253 字符 + RFC 1123 标签规则）才打上游
• 三态响应：有 SSL（is_ssl=true）→ 完整信息；无 SSL/连接失败 → is_ssl=false 其余字段 null；上游异常 → 502
• 字段标准化：上游 null 字段透传 null 不补 0；自动转 int/string 类型；is_expire 重命名 is_expired
• 缓存策略：成功 6h（证书短期不变）；无证书 30min（避免错误域名反复打上游）

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/ssl` |
| Method | `GET` |
| 分类 | 开发工具 (`dev`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 5 / s |
| 每日免费额度 | 登录用户 10000 次；匿名 3000 次 |
| 增长层级 | B-support |
| 推广动作 | 补充推广 |
| 推广角度 | SSL 证书查询；适合站长工具、证书到期提醒。 |
| 官网文档 | https://apizero.cn/aidocs/ssl |
| Marketplace | https://apizero.cn/marketplace/ssl |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Query 参数

| 参数 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| domain | string | 是 | 要检测的域名。可带 http(s):// 前缀、路径、端口、www.，会自动剥离 | apizero.cn |

### Header 参数

| Header | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| Authorization | string | 否 | API Key 鉴权头，格式 Bearer sk_live_xxx；匿名调用时可省略（每日 30 次免费） | Bearer sk_live_xxxxxxxxxxxxxx |

### cURL 模板

```bash
curl -sS \
  -X GET \
  -H "X-API-Key: $APIZERO_API_KEY" \
  "https://v1.apizero.cn/api/ssl?domain=apizero.cn"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "domain": "apizero.cn",
    "common_name": "*.apizero.cn",
    "domains": [
      "*.apizero.cn",
      "apizero.cn"
    ],
    "issuing_agency": "Asseco Data Systems S.A.",
    "issuer": "Certum DV TLS G2 R39 CA",
    "signature_algorithm": "RSA-SHA256",
    "is_ssl": true,
    "is_expired": false,
    "life_span_days": 198,
    "expire_days": 184,
    "start_date": "2026-04-22 17:05:00",
    "expire_date": "2026-11-07 17:04:59",
    "fingerprint": "f4633adfd1cb59185ba094dc3edeef5a8ea26889",
    "remote_address": "119.36.225.184:443"
  },
  "request_id": "abc123def456"
}
```

<a id="idcard-region"></a>

## 58. 身份证归属地查询 `idcard-region`

根据身份证号前 6 位查询省/市/区三级归属地。支持完整 18 位身份证号自动脱敏回显。省份数据按需从 CDN 拉取并本地缓存 30 天，网关 Redis 缓存 24h。

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/idcard-region` |
| Method | `GET` |
| 分类 | 身份核验 (`kyc`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 10 / s |
| 每日免费额度 | 登录用户 1000 次；匿名 300 次 |
| 增长层级 | D-inventory |
| 推广动作 | 先收录 |
| 推广角度 | 先放入接口库，等站内调用量或搜索需求上来后再写专项内容。 |
| 官网文档 | https://apizero.cn/aidocs/idcard-region |
| Marketplace | https://apizero.cn/marketplace/idcard-region |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Query 参数

| 参数 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| idcard | string | 是 | 6 位区划代码或 15/18 位身份证号 | 110101 |

### Header 参数

| Header | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| Authorization | string | 是 |  |  |

### cURL 模板

```bash
curl -sS \
  -X GET \
  -H "X-API-Key: $APIZERO_API_KEY" \
  "https://v1.apizero.cn/api/idcard-region?idcard=110101"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "idcard": "110101************",
    "province": {
      "code": "110000",
      "name": "北京市"
    },
    "city": {
      "code": "110100",
      "name": "北京市"
    },
    "district": {
      "code": "110101",
      "name": "东城区"
    }
  }
}
```

<a id="blood-type"></a>

## 59. 血型遗传查询 `blood-type`

根据父母血型查询子女可能/不可能的血型。基于 ABO 显性遗传规律，覆盖全部 16 种父母组合，毫秒级返回。常用于科普教育、亲子问答场景。

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/blood-type` |
| Method | `GET` |
| 分类 | 生活服务 (`life`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 20 / s |
| 每日免费额度 | 登录用户 20000 次；匿名 10000 次 |
| 增长层级 | D-inventory |
| 推广动作 | 先收录 |
| 推广角度 | 先放入接口库，等站内调用量或搜索需求上来后再写专项内容。 |
| 官网文档 | https://apizero.cn/aidocs/blood-type |
| Marketplace | https://apizero.cn/marketplace/blood-type |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Query 参数

| 参数 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| father | string | 是 | 父亲血型，A / B / O / AB（大小写不敏感） | A |
| mother | string | 是 | 母亲血型，A / B / O / AB | B |

### cURL 模板

```bash
curl -sS \
  -X GET \
  -H "X-API-Key: $APIZERO_API_KEY" \
  "https://v1.apizero.cn/api/blood-type?father=A&mother=B"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "father": "A",
    "mother": "B",
    "possible": [
      "A",
      "B",
      "AB",
      "O"
    ],
    "impossible": [],
    "summary": "子女可能为 A、B、AB、O 型血；无不可能的血型"
  },
  "request_id": "abc123"
}
```

<a id="browser-fingerprint"></a>

## 60. 浏览器指纹风控 `browser-fingerprint`

接收前端 SDK 采集的浏览器指纹（UA / Canvas / WebGL / 字体 / 时区 / 硬件 / WebDriver / 自动化标记等 20+ 维度），综合 9 类规则输出 0-100 风险评分、等级（safe/low/medium/high/critical）、命中的风险因子和异常项，识别爬虫、Headless Chrome、Selenium、虚拟机等异常环境。

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/browser-fingerprint` |
| Method | `POST` |
| 分类 | 开发工具 (`dev`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 5 / s |
| 每日免费额度 | 登录用户 200 次；匿名 50 次 |
| 增长层级 | D-inventory |
| 推广动作 | 先收录 |
| 推广角度 | 先放入接口库，等站内调用量或搜索需求上来后再写专项内容。 |
| 官网文档 | https://apizero.cn/aidocs/browser-fingerprint |
| Marketplace | https://apizero.cn/marketplace/browser-fingerprint |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Header 参数

| Header | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| Authorization | string | 否 |  |  |
| Content-Type | string | 是 |  |  |

### Request Body

- Content-Type：`application/json`
- Body 必填：是

| 字段 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| ua | string | 是 | navigator.userAgent | Mozilla/5.0 ... |
| platform | string | 否 | navigator.platform |  |
| language | string | 否 | 主语言（如 zh-CN） |  |
| timezone | string | 否 | IANA 时区（如 Asia/Shanghai） |  |
| timezoneOffset | number | 否 | getTimezoneOffset()，分钟 |  |
| screenWidth | number | 否 | screen.width |  |
| screenHeight | number | 否 | screen.height |  |
| colorDepth | number | 否 | screen.colorDepth |  |
| pixelRatio | number | 否 | devicePixelRatio |  |
| hardwareConcurrency | number | 否 | navigator.hardwareConcurrency |  |
| deviceMemory | number | 否 | navigator.deviceMemory（GB） |  |
| maxTouchPoints | number | 否 | navigator.maxTouchPoints |  |
| canvasHash | string | 否 | Canvas 指纹哈希 |  |
| webglVendor | string | 否 | WebGL UNMASKED_VENDOR_WEBGL |  |
| webglRenderer | string | 否 | WebGL UNMASKED_RENDERER_WEBGL |  |
| webglHash | string | 否 | WebGL 参数哈希 |  |
| fonts | array | 否 | 检测到的字体列表 |  |
| fontCount | number | 否 | 字体数量 |  |
| plugins | array | 否 | 插件列表 |  |
| pluginCount | number | 否 | 插件数量 |  |
| webdriver | boolean | 否 | navigator.webdriver |  |
| automation | array | 否 | 检测到的自动化框架（如 selenium / phantom） |  |
| cookieEnabled | boolean | 否 | navigator.cookieEnabled |  |
| audioHash | string | 否 | AudioContext 指纹 |  |
| webrtcIPs | array | 否 | WebRTC 检测到的本机 IP |  |
| storageAvailable | array | 否 | 可用的存储类型 |  |
| permissions | array | 否 | 权限状态 |  |
| connection | object | 否 | navigator.connection 信息 |  |
| battery | object | 否 | 电池状态 |  |

请求体示例：

```json
{
  "ua": "Mozilla/5.0 ..."
}
```

### cURL 模板

```bash
curl -sS \
  -X POST \
  -H "X-API-Key: $APIZERO_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"ua": "Mozilla/5.0 ..."}' \
  "https://v1.apizero.cn/api/browser-fingerprint"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "fingerprint_id": "a1b2c3d4...",
    "risk": 72,
    "risk_level": "high",
    "risk_label": "高风险",
    "factors": [
      {
        "name": "webdriver",
        "score": 30,
        "desc": "WebDriver 标记为 true"
      },
      {
        "name": "virtual_gpu",
        "score": 15,
        "desc": "WebGL 渲染器包含虚拟/软渲染特征: SwiftShader"
      }
    ],
    "anomalies": [
      "UA 声称 Windows 但 platform 不匹配"
    ],
    "device_profile": {
      "os": "Windows",
      "browser": "Chrome 125",
      "device_type": "Desktop",
      "screen": "1920x1080",
      "gpu": "ANGLE (NVIDIA, GeForce RTX 3060)",
      "cores": 8,
      "memory": "8GB",
      "fonts_count": 42,
      "plugins_count": 3,
      "touch": false
    },
    "timestamp": 1715097600
  },
  "request_id": "abc123"
}
```

<a id="cf-dns"></a>

## 61. Cloudflare DNS 更新（DDNS） `cf-dns`

通过用户自有的 Cloudflare API Token 更新指定域名的 A/AAAA 记录。家庭宽带 DDNS、办公室外网 IP 自动同步等场景必备。Token 仅作转发使用，不持久化。

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/cf-dns` |
| Method | `POST` |
| 分类 | 开发工具 (`dev`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 5 / s |
| 每日免费额度 | 登录用户 5000 次；匿名 1000 次 |
| 增长层级 | D-inventory |
| 推广动作 | 先收录 |
| 推广角度 | 先放入接口库，等站内调用量或搜索需求上来后再写专项内容。 |
| 官网文档 | https://apizero.cn/aidocs/cf-dns |
| Marketplace | https://apizero.cn/marketplace/cf-dns |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Header 参数

| Header | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| Authorization | string | 是 |  |  |

### Request Body

- Content-Type：`application/json`
- Body 必填：是

| 字段 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| domain | string | 是 | 根域名 | example.com |
| host | string | 是 | 主机记录 @ / www | home |
| ip | string | 是 | 要设置的 IP |  |
| token | string | 是 | Cloudflare API Token |  |
| type | string | 否 | A / AAAA（默认 A） |  |
| ttl | number | 否 | TTL 120-86400（默认 120） |  |
| proxied | boolean | 否 | 是否启用 CDN 代理（默认 false） |  |

请求体示例：

```json
{
  "domain": "example.com",
  "host": "home"
}
```

### cURL 模板

```bash
curl -sS \
  -X POST \
  -H "X-API-Key: $APIZERO_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"domain": "example.com", "host": "home"}' \
  "https://v1.apizero.cn/api/cf-dns"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "message": "DNS 记录更新成功",
    "full_name": "home.example.com",
    "type": "A",
    "old_ip": "1.2.3.4",
    "new_ip": "5.6.7.8",
    "changed": true
  }
}
```

<a id="bank-card"></a>

## 62. 银行卡BIN查询 `bank-card`

根据银行卡号前6位查询发卡行、卡类型、卡品牌等信息

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/bank-card` |
| Method | `GET` |
| 分类 | 金融数据 (`finance`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 20 / s |
| 每日免费额度 | 登录用户 10000 次；匿名 1000 次 |
| 增长层级 | D-inventory |
| 推广动作 | 先收录 |
| 推广角度 | 先放入接口库，等站内调用量或搜索需求上来后再写专项内容。 |
| 官网文档 | https://apizero.cn/aidocs/bank-card |
| Marketplace | https://apizero.cn/marketplace/bank-card |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### cURL 模板

```bash
curl -sS \
  -X GET \
  -H "X-API-Key: $APIZERO_API_KEY" \
  "https://v1.apizero.cn/api/bank-card"
```

### 响应示例 `200`

```json
{
  "code": 200,
  "message": "success",
  "data": {}
}
```

<a id="food-license"></a>

## 63. 食品经营许可证识别 `food-license`

上传食品经营许可证图片（URL 或 base64），自动识别证件字段，包括许可证编号、经营者名称、法定代表人、经营场所、主体业态、经营项目、有效期等共 13 个字段。适用于企业资质审核、供应链合规验证等场景。

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/food-license` |
| Method | `POST` |
| 分类 | 文档识别 (`ocrdata`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 2 / s |
| 每日免费额度 | 登录用户 200 次；匿名 50 次 |
| 增长层级 | D-inventory |
| 推广动作 | 先收录 |
| 推广角度 | 先放入接口库，等站内调用量或搜索需求上来后再写专项内容。 |
| 官网文档 | https://apizero.cn/aidocs/food-license |
| Marketplace | https://apizero.cn/marketplace/food-license |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Header 参数

| Header | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| Content-Type | string | 是 | application/json |  |

### Request Body

- Content-Type：`application/json`
- Body 必填：是

| 字段 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| key | string | 否 | API 密钥（Bearer 令牌或 key 参数） |  |
| input_type | string | 是 | 图片传入方式：url（图片链接）或 base64（base64 编码字符串） |  |
| input_data | string | 是 | 图片 URL 地址（input_type=url）或 base64 编码字符串（input_type=base64，最大 5 MB） |  |

### cURL 模板

```bash
curl -sS \
  -X POST \
  -H "X-API-Key: $APIZERO_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"key": "<key>", "input_type": "<input_type>", "input_data": "<input_data>"}' \
  "https://v1.apizero.cn/api/food-license"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "message": "success",
  "data": {
    "license_number": "JY14012800001234",
    "operator": "某某餐饮有限公司",
    "legal_representative": "张三",
    "premise": "北京市朝阳区某街道1号",
    "main_body": "餐饮服务经营者",
    "operating_item": "热食类食品制售",
    "validity_period": "长期",
    "domicile": "北京市朝阳区某街道1号",
    "issuing_authority": "北京市朝阳区市场监督管理局",
    "issuer": "李四",
    "daily_supervisor": "王五",
    "daily_supervisory_authorities": "北京市朝阳区市场监督管理局",
    "complaints_hotline": "12315"
  }
}
```

<a id="douban-movie"></a>

## 64. 豆瓣电影信息 `douban-movie`

通过豆瓣 ID 或 URL 查询电影详情：评分、导演、演员、类型、地区、片长、集数（剧集）、热门短评等。基于豆瓣公开 JSON API。

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/douban-movie` |
| Method | `GET` |
| 分类 | 内容娱乐 (`content`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 5 / s |
| 每日免费额度 | 登录用户 1000 次；匿名 300 次 |
| 增长层级 | D-inventory |
| 推广动作 | 先收录 |
| 推广角度 | 先放入接口库，等站内调用量或搜索需求上来后再写专项内容。 |
| 官网文档 | https://apizero.cn/aidocs/douban-movie |
| Marketplace | https://apizero.cn/marketplace/douban-movie |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Query 参数

| 参数 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| id | string | 是 | 豆瓣 ID（如 1292052）或豆瓣电影 URL | 1292052 |

### cURL 模板

```bash
curl -sS \
  -X GET \
  -H "X-API-Key: $APIZERO_API_KEY" \
  "https://v1.apizero.cn/api/douban-movie?id=1292052"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "douban_id": "1292052",
    "name": "肖申克的救赎",
    "year": "1994",
    "score": "9.7",
    "director": "弗兰克·德拉邦特"
  }
}
```

<a id="text-censor"></a>

## 65. 文本审核 `text-censor`

对文本进行多维度违规内容审核，覆盖政治敏感、谩骂、色情、违规广告、暴恐、低俗等类别，返回审核结论 + 命中违规词详情列表。

• 5000 字以内（中英文均按 1 字符计），单次请求审核
• 三态结论：合规（is_compliant=true）/ 不合规（is_compliant=false）/ 疑似（is_suspected=true，需人工复核）
• 详细命中信息：每条违规含 category（类别）/ word（触发词）/ level（等级）/ msg（说明）
• 衍生字段：violations（去重违规词数组）/ violation_categories（命中类别去重）/ violation_count（违规条数），前端可直接展示「触发 X 项违规：政治、谩骂…」
• 隐私保护：缓存 key 用 sha256 哈希，原文不进入 key；错误日志不记录文本内容

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/text-censor` |
| Method | `POST` |
| 分类 | 内容娱乐 (`content`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 5 / s |
| 每日免费额度 | 登录用户 100 次；匿名 30 次 |
| 增长层级 | D-inventory |
| 推广动作 | 先收录 |
| 推广角度 | 先放入接口库，等站内调用量或搜索需求上来后再写专项内容。 |
| 官网文档 | https://apizero.cn/aidocs/text-censor |
| Marketplace | https://apizero.cn/marketplace/text-censor |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Header 参数

| Header | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| Authorization | string | 否 | API Key 鉴权头，格式 Bearer sk_live_xxx；匿名调用时可省略（每日 30 次免费） | Bearer sk_live_xxxxxxxxxxxxxx |
| Content-Type | string | 否 | 支持 application/x-www-form-urlencoded 或 application/json | application/json |

### Request Body

- Content-Type：`application/json`
- Body 必填：是

| 字段 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| text | string | 是 | 待审核文本，1-5000 个字符（中英文均按 1 字符计） | 今天天气不错，适合出门散步。 |

请求体示例：

```json
{
  "text": "今天天气不错，适合出门散步。"
}
```

### cURL 模板

```bash
curl -sS \
  -X POST \
  -H "X-API-Key: $APIZERO_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"text": "今天天气不错，适合出门散步。"}' \
  "https://v1.apizero.cn/api/text-censor"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "text": "法轮功是邪教组织",
    "text_length": 8,
    "conclusion": "不合规",
    "conclusion_type": 2,
    "is_compliant": false,
    "is_suspected": false,
    "violation_count": 2,
    "violation_categories": [
      "政治"
    ],
    "violations": [
      "法轮功",
      "邪教"
    ],
    "details": [
      {
        "category": "政治",
        "level": 3,
        "word": "法轮功",
        "msg": "存在政治内容不合规"
      },
      {
        "category": "政治",
        "level": 3,
        "word": "邪教",
        "msg": "存在政治内容不合规"
      }
    ]
  },
  "request_id": "abc123def456"
}
```

<a id="earthquake"></a>

## 66. 全球地震速报 `earthquake`

汇聚全球地震监测数据，按时间范围与震级档位查询近期地震事件，返回震级、震源深度、经纬度、发生时间、烈度、海啸提示与警报级别等结构化信息。

支持按 hour/day/week/month 时间范围与 all/significant/4.5/2.5/1.0 震级档位组合查询，可再按最小震级过滤、按时间或震级排序并限制返回条数。数据每分钟更新（网关缓存 60 秒），适合地震速报播报、防灾监控大屏、科研数据采集等场景。地名为国际标准英文描述，覆盖全球（含中国境内较大地震）。

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/earthquake` |
| Method | `POST` |
| 分类 | 生活服务 (`life`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 10 / s |
| 每日免费额度 | 登录用户 1000 次；匿名 500 次 |
| 增长层级 | D-inventory |
| 推广动作 | 先收录 |
| 推广角度 | 先放入接口库，等站内调用量或搜索需求上来后再写专项内容。 |
| 官网文档 | https://apizero.cn/aidocs/earthquake |
| Marketplace | https://apizero.cn/marketplace/earthquake |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Header 参数

| Header | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| Authorization | string | 是 | Bearer <你的 API Key> |  |
| Content-Type | string | 否 | 请求体格式（POST JSON 时） |  |

### Request Body

- Content-Type：`application/json`
- Body 必填：否

| 字段 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| period | string | 否 | 时间范围：hour=1小时 / day=1天 / week=7天 / month=30天 | day |
| level | string | 否 | 震级档位：all=全部 / significant=显著地震 / 4.5=M4.5+ / 2.5=M2.5+ / 1.0=M1.0+ | all |
| min_magnitude | number | 否 | 最小震级过滤（0=不过滤） | 4.5 |
| order | string | 否 | 排序：time=最新在前 / magnitude=震级由大到小 | time |
| limit | number | 否 | 返回条数上限（1~500） | 100 |

请求体示例：

```json
{
  "period": "day",
  "level": "all",
  "min_magnitude": "4.5",
  "order": "time",
  "limit": "100"
}
```

### cURL 模板

```bash
curl -sS \
  -X POST \
  -H "X-API-Key: $APIZERO_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"period": "day", "level": "all", "min_magnitude": "4.5", "order": "time", "limit": "100"}' \
  "https://v1.apizero.cn/api/earthquake"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "summary": {
      "count": 100,
      "total": 240,
      "max_magnitude": 6.5,
      "period": "day",
      "level": "all",
      "generated_at": "2026-07-01 12:45:33"
    },
    "earthquakes": [
      {
        "id": "us7000abcd",
        "magnitude": 6.5,
        "magnitude_type": "mww",
        "place": "34 km WSW of Sarangani, Philippines",
        "time": "2026-07-01 09:12:30",
        "timestamp": 1782877783910,
        "longitude": 125.11,
        "latitude": 5.62,
        "depth_km": 42.3,
        "alert": "green",
        "tsunami": false,
        "felt": 49,
        "intensity_mmi": 5.2,
        "significance": 650,
        "status": "人工复核",
        "event_type": "地震"
      }
    ]
  },
  "request_id": "abc123"
}
```

<a id="ocr-idcard"></a>

## 67. 身份证识别 `ocr-idcard`

上传身份证正面或反面图片（URL 或 base64），自动识别并结构化返回姓名、身份证号、性别、民族、出生日期、地址、签发机关、有效期等 10 个字段。

支持正反面自动判断，适用于实名认证、用户信息录入、KYC 核验辅助等场景。需要登录后使用，保护用户隐私。

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/ocr-idcard` |
| Method | `POST` |
| 分类 | 文档识别 (`ocrdata`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 2 / s |
| 每日免费额度 | 登录用户 20 次；匿名 0 次 |
| 增长层级 | D-inventory |
| 推广动作 | 先收录 |
| 推广角度 | 先放入接口库，等站内调用量或搜索需求上来后再写专项内容。 |
| 官网文档 | https://apizero.cn/aidocs/ocr-idcard |
| Marketplace | https://apizero.cn/marketplace/ocr-idcard |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Header 参数

| Header | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| Authorization | string | 是 | Bearer <你的 API Key> |  |
| Content-Type | string | 是 | 请求体格式 |  |

### Request Body

- Content-Type：`application/json`
- Body 必填：是

| 字段 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| input_type | string | 是 | 图片传入方式 | url |
| input_data | string | 是 | 图片 URL 或 base64 编码字符串（jpg/png，base64 最大 10 MB） | https://example.com/idcard-front.jpg |

请求体示例：

```json
{
  "input_type": "url",
  "input_data": "https://example.com/idcard-front.jpg"
}
```

### cURL 模板

```bash
curl -sS \
  -X POST \
  -H "X-API-Key: $APIZERO_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"input_type": "url", "input_data": "https://example.com/idcard-front.jpg"}' \
  "https://v1.apizero.cn/api/ocr-idcard"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "side": 1,
    "identity_name": "张三",
    "identity_code": "310101199001011234",
    "gender": "男",
    "race": "汉",
    "birth": "1990-01-01",
    "address": "上海市浦东新区某某路123号",
    "issued_by": null,
    "valid_date_start": null,
    "valid_date_end": null
  },
  "request_id": "req_abc123"
}
```

<a id="site-check"></a>

## 68. 网站测速诊断 `site-check`

测量网站的 DNS 解析、TCP 连接、SSL 握手、TTFB、总耗时，跟踪重定向链并提取 SSL 证书摘要、命中 IP/端口、页面体积。一次请求 6 大维度全监测。

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/site-check` |
| Method | `GET` |
| 分类 | 开发工具 (`dev`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 2 / s |
| 每日免费额度 | 登录用户 5000 次；匿名 1000 次 |
| 增长层级 | D-inventory |
| 推广动作 | 先收录 |
| 推广角度 | 先放入接口库，等站内调用量或搜索需求上来后再写专项内容。 |
| 官网文档 | https://apizero.cn/aidocs/site-check |
| Marketplace | https://apizero.cn/marketplace/site-check |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Query 参数

| 参数 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| url | string | 是 | 目标 URL（自动补 https://） |  |

### Header 参数

| Header | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| Authorization | string | 是 |  |  |

### cURL 模板

```bash
curl -sS \
  -X GET \
  -H "X-API-Key: $APIZERO_API_KEY" \
  "https://v1.apizero.cn/api/site-check?url=<url>"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "url": "https://baidu.com",
    "final_url": "https://www.baidu.com/",
    "http_code": 200,
    "timing": {
      "dns_ms": 15,
      "connect_ms": 32.5,
      "ssl_ms": 78.4,
      "ttfb_ms": 145.2,
      "total_ms": 156.7
    },
    "redirect_count": 1
  }
}
```

<a id="code-beautify"></a>

## 69. 代码美化图片 `code-beautify`

将代码片段渲染为精美的SVG/PNG卡片图片。支持16种编程语言语法高亮、8套设计主题，适合社交分享和技术文档配图。

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/code-beautify` |
| Method | `POST` |
| 分类 | 开发工具 (`dev`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 3 / s |
| 每日免费额度 | 登录用户 3000 次；匿名 2000 次 |
| 增长层级 | D-inventory |
| 推广动作 | 先收录 |
| 推广角度 | 先放入接口库，等站内调用量或搜索需求上来后再写专项内容。 |
| 官网文档 | https://apizero.cn/aidocs/code-beautify |
| Marketplace | https://apizero.cn/marketplace/code-beautify |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Header 参数

| Header | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| Authorization | string | 是 |  |  |

### Request Body

- Content-Type：`application/json`
- Body 必填：是

| 字段 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| code | string | 是 | 代码内容 | const sum = (a, b) => a + b; |
| language | string | 否 | 语言：auto/python/javascript/typescript/json/bash/go/rust/java/c/cpp/html/css/sql/yaml/markdown | typescript |
| theme | string | 否 | 主题：aurora/sunset/forest/midnight/rose/ocean/volcano/mono | aurora |
| title | string | 否 | 卡片顶部标题 | snippet.ts |
| line_numbers | number | 否 | 是否显示行号 1/0 | 1 |
| scale | number | 否 | PNG放大倍数 1-4 | 2 |
| output | string | 否 | 输出格式：svg/png/json | json |

请求体示例：

```json
{
  "code": "const sum = (a, b) => a + b;",
  "language": "typescript",
  "theme": "aurora",
  "title": "snippet.ts",
  "line_numbers": "1",
  "scale": "2",
  "output": "json"
}
```

### cURL 模板

```bash
curl -sS \
  -X POST \
  -H "X-API-Key: $APIZERO_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"code": "const sum = (a, b) => a + b;", "language": "typescript", "theme": "aurora", "title": "snippet.ts", "line_numbers": "1", "scale": "2", "output": "json"}' \
  "https://v1.apizero.cn/api/code-beautify"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "language": "typescript",
    "theme": "aurora",
    "theme_name": "极光",
    "title": "snippet.ts",
    "line_count": 3,
    "width": 680,
    "height": 180,
    "svg": "<svg>...</svg>",
    "png_base64": "iVBORw0..."
  },
  "request_id": "req_abc123"
}
```

<a id="desensitize"></a>

## 70. 数据脱敏（敏感信息掩码） `desensitize`

自动检测并脱敏文本中的手机号、身份证（15/18 位）、银行卡（16-19 位）、邮箱、中文姓名等敏感信息。支持按类型组合，纯本地正则匹配，毫秒级返回。默认不回显原文，避免日志泄漏。

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/desensitize` |
| Method | `POST` |
| 分类 | 内容娱乐 (`content`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 10 / s |
| 每日免费额度 | 登录用户 200 次；匿名 50 次 |
| 增长层级 | D-inventory |
| 推广动作 | 先收录 |
| 推广角度 | 先放入接口库，等站内调用量或搜索需求上来后再写专项内容。 |
| 官网文档 | https://apizero.cn/aidocs/desensitize |
| Marketplace | https://apizero.cn/marketplace/desensitize |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Request Body

- Content-Type：`application/json`
- Body 必填：是

| 字段 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| text | string | 是 | 要脱敏的文本，最长 50000 字节 |  |
| types | string | 否 | 类型逗号分隔：phone,idcard,bankcard,email,name 或 all（默认） |  |
| with_original | boolean | 否 | 是否在 detections 中回显原文，默认 false |  |

### cURL 模板

```bash
curl -sS \
  -X POST \
  -H "X-API-Key: $APIZERO_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"text": "<text>", "types": "<types>", "with_original": "<with_original>"}' \
  "https://v1.apizero.cn/api/desensitize"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "masked_text": "联系人：张*，电话 138****8000",
    "detection_count": 2,
    "summary": {
      "name": 1,
      "phone": 1
    },
    "detections": [
      {
        "type": "name",
        "masked": "张*"
      },
      {
        "type": "phone",
        "masked": "138****8000"
      }
    ],
    "types_applied": [
      "phone",
      "idcard",
      "bankcard",
      "email",
      "name"
    ]
  }
}
```

<a id="ios-cert"></a>

## 71. iOS 证书与描述文件检测 `ios-cert`

解析 .p12 证书与 .mobileprovision 描述文件，输出证书有效期、吊销状态、Team ID、证书类型（开发/分发/企业）、设备列表、25 项 entitlements 权限以及证书与描述文件匹配性验证。iOS 开发者必备。

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/ios-cert` |
| Method | `POST` |
| 分类 | 开发工具 (`dev`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 5 / s |
| 每日免费额度 | 登录用户 10000 次；匿名 5000 次 |
| 增长层级 | D-inventory |
| 推广动作 | 先收录 |
| 推广角度 | 先放入接口库，等站内调用量或搜索需求上来后再写专项内容。 |
| 官网文档 | https://apizero.cn/aidocs/ios-cert |
| Marketplace | https://apizero.cn/marketplace/ios-cert |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Header 参数

| Header | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| Authorization | string | 是 |  |  |
| Content-Type | string | 是 |  |  |

### Request Body

- Content-Type：`application/json`
- Body 必填：是

| 字段 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| cert | string | 是 | Base64 编码的 .p12 文件内容 |  |
| provision | string | 是 | Base64 编码的 .mobileprovision |  |
| password | string | 否 | 证书密码，默认为空 |  |

### cURL 模板

```bash
curl -sS \
  -X POST \
  -H "X-API-Key: $APIZERO_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"cert": "<cert>", "provision": "<provision>", "password": "<password>"}' \
  "https://v1.apizero.cn/api/ios-cert"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "certificate": {
      "name": "iPhone Developer: ...",
      "status": "正常",
      "is_revoked": false
    },
    "mobileprovision": {
      "cert_type": "Development",
      "cert_end_days": 350
    },
    "permissions": {
      "aps": true,
      "keychain": true,
      "debug": true
    },
    "is_matching": true
  }
}
```

<a id="csdn-profile"></a>

## 72. CSDN 博主信息 `csdn-profile`

通过 CSDN 用户名查询博主公开档案：昵称、头像、码龄、博客等级、原创数、粉丝数、博客排名、IP 属地、原力等级、勋章列表与成就明细。

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/csdn-profile` |
| Method | `GET` |
| 分类 | 内容娱乐 (`content`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 5 / s |
| 每日免费额度 | 登录用户 1000 次；匿名 200 次 |
| 增长层级 | D-inventory |
| 推广动作 | 先收录 |
| 推广角度 | 先放入接口库，等站内调用量或搜索需求上来后再写专项内容。 |
| 官网文档 | https://apizero.cn/aidocs/csdn-profile |
| Marketplace | https://apizero.cn/marketplace/csdn-profile |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Query 参数

| 参数 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| username | string | 是 | CSDN 用户名（仅字母/数字/下划线） | weixin_44906759 |

### cURL 模板

```bash
curl -sS \
  -X GET \
  -H "X-API-Key: $APIZERO_API_KEY" \
  "https://v1.apizero.cn/api/csdn-profile?username=weixin_44906759"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "nickname": "XXX",
    "code_age_years": 5,
    "fans_count": 1234
  }
}
```

<a id="site-security"></a>

## 73. 网站安全综合评分 `site-security`

五维加权综合评分：SSL 证书 (25%) + 域名安全 (20%) + ICP 备案 (20%) + 微信/QQ 拦截 (15%) + 网站性能 (20%)，总分 0-100，对应 A/B/C/D/F 五级。一次请求得到全面诊断报告。

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/site-security` |
| Method | `GET` |
| 分类 | 开发工具 (`dev`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 2 / s |
| 每日免费额度 | 登录用户 3000 次；匿名 1000 次 |
| 增长层级 | D-inventory |
| 推广动作 | 先收录 |
| 推广角度 | 先放入接口库，等站内调用量或搜索需求上来后再写专项内容。 |
| 官网文档 | https://apizero.cn/aidocs/site-security |
| Marketplace | https://apizero.cn/marketplace/site-security |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Query 参数

| 参数 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| domain | string | 是 | 纯域名，如 baidu.com |  |

### Header 参数

| Header | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| Authorization | string | 是 |  |  |

### cURL 模板

```bash
curl -sS \
  -X GET \
  -H "X-API-Key: $APIZERO_API_KEY" \
  "https://v1.apizero.cn/api/site-security?domain=<domain>"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "domain": "baidu.com",
    "overall_score": 92,
    "grade": "A",
    "detection_time": "4521ms",
    "ssl": {
      "score": 100,
      "https_enabled": true,
      "days_until_expiry": 365
    }
  }
}
```

<a id="company-profile"></a>

## 74. 企业档案深度查询 `company-profile`

输入企业名称，返回单个企业的多维度深度工商档案：工商基本信息、股东结构、主要成员、对外投资、历史变更记录，以及六大类风险信息（被执行、失信被执行、限制高消费、经营异常、严重违法、行政处罚）。并附企业规模标签、经营状态、成立年限、活力评分与自然语言摘要。

区别于「企业工商信息查询」的关键词列表检索，本接口面向单一企业的深度尽调，支持按维度组合按需查询，适用于商业尽调、合作方背调、风控审查等场景。数据来自权威工商数据库，6 小时缓存。

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/company-profile` |
| Method | `POST` |
| 分类 | 身份核验 (`kyc`) |
| 计费 | 按次付费 · 单次 0 积分 |
| QPS | 5 / s |
| 每日免费额度 | 登录用户 3 次；匿名 1 次 |
| 增长层级 | D-inventory |
| 推广动作 | 先收录 |
| 推广角度 | 先放入接口库，等站内调用量或搜索需求上来后再写专项内容。 |
| 官网文档 | https://apizero.cn/aidocs/company-profile |
| Marketplace | https://apizero.cn/marketplace/company-profile |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Header 参数

| Header | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| Authorization | string | 是 | Bearer <你的 API Key> |  |
| Content-Type | string | 否 | 请求体格式 |  |

### Request Body

- Content-Type：`application/json`
- Body 必填：是

| 字段 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| company | string | 是 | 企业名称（2-80 字、含中文，支持简称/全称模糊搜索）；兼容别名 name | 阿里巴巴 |
| dimension | string | 否 | 查询维度，逗号分隔。可选：basic(基本信息)、shareholders(股东)、executives(高管)、investments(对外投资)、changes(变更记录)、risk(风险综合) | basic,shareholders,risk |

请求体示例：

```json
{
  "company": "阿里巴巴",
  "dimension": "basic,shareholders,risk"
}
```

### cURL 模板

```bash
curl -sS \
  -X POST \
  -H "X-API-Key: $APIZERO_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"company": "阿里巴巴", "dimension": "basic,shareholders,risk"}' \
  "https://v1.apizero.cn/api/company-profile"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "basic": {
      "company_name": "阿里巴巴（中国）有限公司",
      "legal_person": "示例",
      "credit_code": "9133...",
      "register_capital": "1.4 亿美元",
      "establish_date": "2007-03-26",
      "business_status": "存续"
    },
    "extension": {
      "company_age_years": 19,
      "register_capital_label": "巨型企业",
      "vitality_score": 92,
      "vitality_level": "极高",
      "summary": "……"
    },
    "stats": {
      "shareholder_count": 3,
      "risk_total": 0
    },
    "dimensions": [
      "basic"
    ]
  },
  "request_id": "..."
}
```

<a id="bili-dynamic"></a>

## 75. B 站用户动态 `bili-dynamic`

通过 B 站官方公开 API 拉取指定用户的最新动态列表（视频投稿/图文/纯文本/转发等），含发布时间、文本、视频/图片附件、点赞/评论/转发统计。

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/bili-dynamic` |
| Method | `GET` |
| 分类 | 内容娱乐 (`content`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 5 / s |
| 每日免费额度 | 登录用户 1000 次；匿名 200 次 |
| 增长层级 | D-inventory |
| 推广动作 | 先收录 |
| 推广角度 | 先放入接口库，等站内调用量或搜索需求上来后再写专项内容。 |
| 官网文档 | https://apizero.cn/aidocs/bili-dynamic |
| Marketplace | https://apizero.cn/marketplace/bili-dynamic |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Query 参数

| 参数 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| uid | string | 是 | B 站用户 UID（纯数字） | 208259 |

### cURL 模板

```bash
curl -sS \
  -X GET \
  -H "X-API-Key: $APIZERO_API_KEY" \
  "https://v1.apizero.cn/api/bili-dynamic?uid=208259"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "uid": "208259",
    "count": 5,
    "list": [
      {
        "dynamic_id": "8xxx",
        "type": "DYNAMIC_TYPE_AV",
        "text": "...",
        "stats": {
          "comments": 100,
          "likes": 1000,
          "forwards": 50
        }
      }
    ]
  }
}
```

<a id="domain-trade"></a>

## 76. 域名交易市场 `domain-trade`

拉取 EDNS 域名交易市场公开数据，支持按价格、长度、后缀、交易类型筛选。适合站长扫货短米、行业研究、域名估值参考。

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/domain-trade` |
| Method | `GET` |
| 分类 | 金融数据 (`finance`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 5 / s |
| 每日免费额度 | 登录用户 1000 次；匿名 200 次 |
| 增长层级 | D-inventory |
| 推广动作 | 先收录 |
| 推广角度 | 先放入接口库，等站内调用量或搜索需求上来后再写专项内容。 |
| 官网文档 | https://apizero.cn/aidocs/domain-trade |
| Marketplace | https://apizero.cn/marketplace/domain-trade |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Query 参数

| 参数 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| page | number | 否 | 页码（默认 1） |  |
| pagesize | number | 否 | 每页数量（≤100，默认 50） |  |
| max_price | number | 否 | 最高价格（元） |  |
| max_length | number | 否 | 域名最大长度 |  |
| suffix | string | 否 | 后缀，如 .com |  |
| sale_type | string | 否 | 交易类型 |  |

### cURL 模板

```bash
curl -sS \
  -X GET \
  -H "X-API-Key: $APIZERO_API_KEY" \
  "https://v1.apizero.cn/api/domain-trade?page=<page>&pagesize=<pagesize>"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "total_pages": 1234,
    "current_page": 1,
    "count": 50,
    "list": [
      {
        "name": "abc.com",
        "price": "5000",
        "sale_type": "一口价"
      }
    ]
  }
}
```

<a id="oil-price"></a>

## 77. 全国油价 `oil-price`

查询全国各省 92/95/98 号汽油及 0 号柴油的今日最新价格。

覆盖 31 个大陆省级行政区，返回汽柴油零售限价、数据更新日期、下次调价时间及后续涨跌预测。支持省份简称或全称（如「广东」「广东省」「内蒙古」），也支持常见城市名自动归属到所在省（如「广州」→ 广东）；同省内各城市油价一致。数据仅供参考，实际价格以当地加油站为准。

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/oil-price` |
| Method | `POST` |
| 分类 | 生活服务 (`life`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 10 / s |
| 每日免费额度 | 登录用户 1000 次；匿名 500 次 |
| 增长层级 | D-inventory |
| 推广动作 | 先收录 |
| 推广角度 | 先放入接口库，等站内调用量或搜索需求上来后再写专项内容。 |
| 官网文档 | https://apizero.cn/aidocs/oil-price |
| Marketplace | https://apizero.cn/marketplace/oil-price |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Header 参数

| Header | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| Authorization | string | 否 | Bearer <你的 API Key>（登录调用可提升额度；匿名可不传） |  |
| Content-Type | string | 否 | 请求体格式 |  |

### Request Body

- Content-Type：`application/json`
- Body 必填：是

| 字段 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| province | string | 是 | 省 / 直辖市 / 自治区名称，支持简称或全称；兼容别名 area / region / msg，也支持常见城市名（自动归属到所在省） | 广东 |

请求体示例：

```json
{
  "province": "广东"
}
```

### cURL 模板

```bash
curl -sS \
  -X POST \
  -H "X-API-Key: $APIZERO_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"province": "广东"}' \
  "https://v1.apizero.cn/api/oil-price"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "province": "广东",
    "update_date": "2026-06-20",
    "prices": [
      {
        "name": "92号汽油",
        "type": "gasoline_92",
        "price": 7.96,
        "unit": "元/升"
      },
      {
        "name": "95号汽油",
        "type": "gasoline_95",
        "price": 8.62,
        "unit": "元/升"
      },
      {
        "name": "98号汽油",
        "type": "gasoline_98",
        "price": 10.62,
        "unit": "元/升"
      },
      {
        "name": "0号柴油",
        "type": "diesel_0",
        "price": 7.62,
        "unit": "元/升"
      }
    ],
    "next_adjustment": "下次油价7月3日24时调整",
    "forecast": "预计下调630元/吨(0.48元/升-0.57元/升)"
  },
  "request_id": "abc123"
}
```

<a id="soul-soup"></a>

## 78. 心灵毒鸡汤 `soul-soup`

随机返回一句"反鸡汤"文案，用于自嘲、解压或段子素材。

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/soul-soup` |
| Method | `POST` |
| 分类 | 内容娱乐 (`content`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 5 / s |
| 每日免费额度 | 登录用户 50 次；匿名 30 次 |
| 增长层级 | D-inventory |
| 推广动作 | 先收录 |
| 推广角度 | 先放入接口库，等站内调用量或搜索需求上来后再写专项内容。 |
| 官网文档 | https://apizero.cn/aidocs/soul-soup |
| Marketplace | https://apizero.cn/marketplace/soul-soup |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Request Body

- Content-Type：`application/json`
- Body 必填：否

### cURL 模板

```bash
curl -sS \
  -X POST \
  -H "X-API-Key: $APIZERO_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{}' \
  "https://v1.apizero.cn/api/soul-soup"
```

### 响应示例 `200`

```json
{
  "code": 200,
  "message": "success",
  "data": {}
}
```

<a id="dns-hijack"></a>

## 79. DNS 劫持检测 `dns-hijack`

通过对比 5 大公共 DoH（Cloudflare / Google / AliDNS / DNSPod / OpenDNS）对同一域名的解析结果，检测是否存在 DNS 劫持/污染/篡改。一致 = 安全，多源不一致 = 高风险。

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/dns-hijack` |
| Method | `GET` |
| 分类 | 开发工具 (`dev`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 5 / s |
| 每日免费额度 | 登录用户 10000 次；匿名 5000 次 |
| 增长层级 | D-inventory |
| 推广动作 | 先收录 |
| 推广角度 | 先放入接口库，等站内调用量或搜索需求上来后再写专项内容。 |
| 官网文档 | https://apizero.cn/aidocs/dns-hijack |
| Marketplace | https://apizero.cn/marketplace/dns-hijack |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Query 参数

| 参数 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| domain | string | 是 | 域名（不含 https://） | baidu.com |

### cURL 模板

```bash
curl -sS \
  -X GET \
  -H "X-API-Key: $APIZERO_API_KEY" \
  "https://v1.apizero.cn/api/dns-hijack?domain=baidu.com"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "domain": "baidu.com",
    "is_hijacked": false,
    "risk_level": "low",
    "summary": "5 个 DoH 服务商解析结果一致，未检测到劫持迹象",
    "unique_ips": [
      "110.242.68.3",
      "110.242.68.4"
    ]
  }
}
```

<a id="idcard-organ"></a>

## 80. 身份证签发机关查询 `idcard-organ`

通过身份证号前 6 位行政区划代码，查询对应的签发机关（地方公安局）。覆盖全国 3000+ 区划代码，本地数据零上游。仅供身份证真伪辅助核验、户籍查询等合规场景。

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/idcard-organ` |
| Method | `GET` |
| 分类 | 身份核验 (`kyc`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 10 / s |
| 每日免费额度 | 登录用户 1000 次；匿名 300 次 |
| 增长层级 | D-inventory |
| 推广动作 | 先收录 |
| 推广角度 | 先放入接口库，等站内调用量或搜索需求上来后再写专项内容。 |
| 官网文档 | https://apizero.cn/aidocs/idcard-organ |
| Marketplace | https://apizero.cn/marketplace/idcard-organ |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Query 参数

| 参数 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| id | string | 是 | 6 位行政区划代码或完整身份证号（自动截取前 6 位） | 110101 |

### Header 参数

| Header | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| Authorization | string | 是 |  |  |

### cURL 模板

```bash
curl -sS \
  -X GET \
  -H "X-API-Key: $APIZERO_API_KEY" \
  "https://v1.apizero.cn/api/idcard-organ?id=110101"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "code": "110101",
    "organization": "北京市公安局东城分局"
  }
}
```

<a id="idcard-2c"></a>

## 81. 身份证二要素核验 `idcard-2c`

校验「真实姓名 + 18 位身份证号」是否与公安权威库一致，秒级返回。

适用于注册实名、下单风控、账户绑定等场景。仅返回是否一致，不返回任何额外户籍信息；身份证号在响应中脱敏回显。请在取得被核验人授权后调用。

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/idcard-2c` |
| Method | `POST` |
| 分类 | 身份核验 (`kyc`) |
| 计费 | 按次付费 · 单次 0 积分 |
| QPS | 5 / s |
| 每日免费额度 | 登录用户 0 次；匿名 0 次 |
| 增长层级 | D-inventory |
| 推广动作 | 先收录 |
| 推广角度 | 先放入接口库，等站内调用量或搜索需求上来后再写专项内容。 |
| 官网文档 | https://apizero.cn/aidocs/idcard-2c |
| Marketplace | https://apizero.cn/marketplace/idcard-2c |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Header 参数

| Header | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| Authorization | string | 是 | Bearer <你的 API Key> |  |
| Content-Type | string | 否 | 请求体格式 |  |

### Request Body

- Content-Type：`application/json`
- Body 必填：是

| 字段 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| name | string | 是 | 真实姓名（中文） | 张三 |
| idcard | string | 是 | 18 位身份证号（末位可为 X） | 11010519491231002X |

请求体示例：

```json
{
  "name": "张三",
  "idcard": "11010519491231002X"
}
```

### cURL 模板

```bash
curl -sS \
  -X POST \
  -H "X-API-Key: $APIZERO_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"name": "张三", "idcard": "11010519491231002X"}' \
  "https://v1.apizero.cn/api/idcard-2c"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "name": "张三",
    "idcard": "110***********002X",
    "valid": true,
    "result_code": 100,
    "message": "一致"
  },
  "request_id": "abc123"
}
```

<a id="tencent-weather"></a>

## 82. 腾讯天气 `tencent-weather`

腾讯天气官方数据源，中文省市名直接查询全国天气，无需经纬度。

输入省 + 市（可选区县）即可获取：实时天气、空气质量（AQI/PM2.5/PM10）、未来 7 天预报、24 小时逐时预报、23 项生活指数、日出日落与机动车限行信息。数据来自腾讯天气，权威稳定。完全免费：未登录每日 500 次，登录用户每日 1000 次。

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/tencent-weather` |
| Method | `POST` |
| 分类 | 生活服务 (`life`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 10 / s |
| 每日免费额度 | 登录用户 1000 次；匿名 500 次 |
| 增长层级 | D-inventory |
| 推广动作 | 先收录 |
| 推广角度 | 先放入接口库，等站内调用量或搜索需求上来后再写专项内容。 |
| 官网文档 | https://apizero.cn/aidocs/tencent-weather |
| Marketplace | https://apizero.cn/marketplace/tencent-weather |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Header 参数

| Header | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| Authorization | string | 否 | Bearer <你的 API Key>（可选，登录用户额度更高） |  |

### Request Body

- Content-Type：`application/json`
- Body 必填：是

| 字段 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| province | string | 是 | 省 / 直辖市（中文名） | 广东 |
| city | string | 是 | 市（中文名） | 深圳 |
| county | string | 否 | 区 / 县（中文名），可提升定位精度与限行准确度 | 南山 |

请求体示例：

```json
{
  "province": "广东",
  "city": "深圳",
  "county": "南山"
}
```

### cURL 模板

```bash
curl -sS \
  -X POST \
  -H "X-API-Key: $APIZERO_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"province": "广东", "city": "深圳", "county": "南山"}' \
  "https://v1.apizero.cn/api/tencent-weather"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "location": {
      "province": "广东",
      "city": "深圳",
      "county": "南山"
    },
    "observe": {
      "weather": "多云",
      "temperature": 30,
      "humidity": 77,
      "wind_direction": "北风",
      "wind_power": "3-4",
      "update_time": "2026-07-01 10:15"
    },
    "air": {
      "aqi": 13,
      "level": 1,
      "quality": "优",
      "pm25": 4,
      "pm10": 12
    },
    "daily_forecast": [
      {
        "date": "2026-07-01",
        "temperature": {
          "min": 26,
          "max": 33
        }
      }
    ],
    "hourly_forecast": [
      {
        "time": "07-01 10:00",
        "temperature": 30,
        "weather": "多云"
      }
    ],
    "life_index": [
      {
        "key": "clothes",
        "name": "穿衣",
        "level": "炎热",
        "detail": "..."
      }
    ],
    "sunrise_sunset": [
      {
        "date": "2026-07-01",
        "sunrise": "05:43",
        "sunset": "19:12"
      }
    ],
    "alarm": [],
    "limit": {
      "tail_number": "3和8",
      "date": "2026-07-01"
    }
  },
  "request_id": "abc123"
}
```

<a id="typhoon"></a>

## 83. 台风实时路径 `typhoon`

台风实时路径：西北太平洋 / 南海台风实时监测，返回活跃台风列表、单个台风完整移动路径（实况点 + 官方预报点）及风云卫星云图。

数据含台风编号、中英文名、等级、经纬度、中心气压、最大风速、移动方向与逐时路径点；`action=all` 一次取活跃台风 + 最新云图。适用于防灾预警、气象可视化、航运出行等场景。

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/typhoon` |
| Method | `POST` |
| 分类 | 生活服务 (`life`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 10 / s |
| 每日免费额度 | 登录用户 1000 次；匿名 500 次 |
| 增长层级 | D-inventory |
| 推广动作 | 先收录 |
| 推广角度 | 先放入接口库，等站内调用量或搜索需求上来后再写专项内容。 |
| 官网文档 | https://apizero.cn/aidocs/typhoon |
| Marketplace | https://apizero.cn/marketplace/typhoon |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Header 参数

| Header | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| Authorization | string | 否 | Bearer <你的 API Key>（登录调用时提高额度） |  |

### Request Body

- Content-Type：`application/json`
- Body 必填：否

| 字段 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| action | string | 否 | 操作类型：list 活跃台风列表 / detail 单台风详情+路径 / images 卫星云图 / all 综合 |  |
| id | string | 否 | 台风 ID（action=detail 必填，从 list 获取） | 3257931 |
| status | string | 否 | list 用：active 仅活跃 / all 含停编 |  |
| limit | number | 否 | images 用：云图数量（1-50） |  |

请求体示例：

```json
{
  "id": "3257931"
}
```

### cURL 模板

```bash
curl -sS \
  -X POST \
  -H "X-API-Key: $APIZERO_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"id": "3257931"}' \
  "https://v1.apizero.cn/api/typhoon"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "id": "3257931",
    "tc_num": "2609",
    "name_cn": "巴威",
    "name_en": "BAVI",
    "is_active": true,
    "current": {
      "time_cst": "2026-07-02 08:00",
      "grade": "热带风暴",
      "longitude": 118.5,
      "latitude": 14.2,
      "pressure": 1002,
      "wind_speed": 18,
      "wind_dir": "西西北",
      "type": "analysis"
    },
    "point_count": 15,
    "points": [
      {
        "time_cst": "2026-07-02 08:00",
        "grade": "热带风暴",
        "longitude": 118.5,
        "latitude": 14.2,
        "pressure": 1002,
        "wind_speed": 18,
        "type": "analysis"
      }
    ]
  },
  "request_id": "abc123"
}
```

<a id="business-license"></a>

## 84. 营业执照识别 `business-license`

上传营业执照图片（URL 或 base64），自动识别证件字段，包括统一社会信用代码、公司名称、法人、注册资本、经营范围等共 12 个关键字段信息。

适用于企业资质核验、合规审查、供应商管理、电商开店审核等场景。支持 JPG / PNG，文件大小不超过 10 MB，建议图片清晰、完整、无遮挡。

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/business-license` |
| Method | `POST` |
| 分类 | 文档识别 (`ocrdata`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 2 / s |
| 每日免费额度 | 登录用户 20 次；匿名 0 次 |
| 增长层级 | D-inventory |
| 推广动作 | 先收录 |
| 推广角度 | 先放入接口库，等站内调用量或搜索需求上来后再写专项内容。 |
| 官网文档 | https://apizero.cn/aidocs/business-license |
| Marketplace | https://apizero.cn/marketplace/business-license |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Header 参数

| Header | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| Authorization | string | 是 | Bearer <你的 API Key> |  |
| Content-Type | string | 是 | 请求体格式 |  |

### Request Body

- Content-Type：`application/json`
- Body 必填：是

| 字段 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| input_type | string | 是 | 图片传入方式 | url |
| input_data | string | 是 | 图片 URL 或 base64 编码字符串（jpg/png，base64 最大 10 MB） | https://example.com/business-license.jpg |

请求体示例：

```json
{
  "input_type": "url",
  "input_data": "https://example.com/business-license.jpg"
}
```

### cURL 模板

```bash
curl -sS \
  -X POST \
  -H "X-API-Key: $APIZERO_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"input_type": "url", "input_data": "https://example.com/business-license.jpg"}' \
  "https://v1.apizero.cn/api/business-license"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "certificate_type": "营业执照",
    "unified_social_credit_code": "91310000XXXXXXXXXX",
    "company_name": "某某科技有限公司",
    "company_type": "有限责任公司（自然人独资）",
    "domicile": "上海市浦东新区某某路123号",
    "legal_representative": "张三",
    "registered_capital": "100万人民币",
    "established_time": "2015-06-01",
    "established_date": "2015-06-01至无固定期限",
    "business_scope": "软件开发；信息技术咨询服务",
    "website": "http://www.gsxt.gov.cn",
    "approval_date": "2025-01-10"
  },
  "request_id": "req_abc123"
}
```

<a id="ocr-vehicle-invoice"></a>

## 85. 机动车发票识别 `ocr-vehicle-invoice`

结构化识别机动车销售发票，提取发票代码、号码、开票日期、购买方/销售方信息、车辆信息及价税明细等 20 个字段。

适用场景：二手车交易核验、购车报销录入、增值税抵扣材料整理等。支持 jpg/png 格式，文件大小不超过 10MB，建议发票平整、拍摄清晰以提高识别准确率。

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/ocr-vehicle-invoice` |
| Method | `POST` |
| 分类 | 文档识别 (`ocrdata`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 2 / s |
| 每日免费额度 | 登录用户 20 次；匿名 0 次 |
| 增长层级 | D-inventory |
| 推广动作 | 先收录 |
| 推广角度 | 先放入接口库，等站内调用量或搜索需求上来后再写专项内容。 |
| 官网文档 | https://apizero.cn/aidocs/ocr-vehicle-invoice |
| Marketplace | https://apizero.cn/marketplace/ocr-vehicle-invoice |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Header 参数

| Header | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| Authorization | string | 是 | Bearer <你的 API Key> |  |
| Content-Type | string | 否 | 请求体格式 |  |

### Request Body

- Content-Type：`application/json`
- Body 必填：是

| 字段 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| input_type | string | 是 | 图片传输方式，支持 url（公网图片地址）或 base64（图片的 base64 编码） | url |
| input_data | string | 是 | 图片内容：input_type=url 时填 http/https 图片链接；input_type=base64 时填 base64 字符串（可含 data:image/xxx;base64, 前缀），文件大小不超过 10MB | https://example.com/vehicle-invoice.jpg |

请求体示例：

```json
{
  "input_type": "url",
  "input_data": "https://example.com/vehicle-invoice.jpg"
}
```

### cURL 模板

```bash
curl -sS \
  -X POST \
  -H "X-API-Key: $APIZERO_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"input_type": "url", "input_data": "https://example.com/vehicle-invoice.jpg"}' \
  "https://v1.apizero.cn/api/ocr-vehicle-invoice"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "invoice_code": "31100000000",
    "invoice_num": "12345678",
    "date": "2024年01月15日",
    "print_code": "",
    "print_num": "",
    "machine_num": "499098765432",
    "buyer_name": "张三",
    "buyer_id": "",
    "vehicle_type": "小型轿车",
    "product_model": "丰田/CAMRY",
    "certificate_num": "LSXXXXXXXXXXXXX",
    "vin": "4A123456",
    "total_price": "壹拾柒万元整",
    "total_price_little": "170000.00",
    "tax_rate": "9%",
    "tax": "13944.95",
    "price": "156055.05",
    "saler_name": "XX汽车销售有限公司",
    "saler_id": "91310000XXXXXXXXXX",
    "saler_addr": "上海市XX路XX号 021-XXXXXXXX"
  },
  "request_id": "req_abc123"
}
```

<a id="wechat-archive"></a>

## 86. 微信文章转存 `wechat-archive`

输入微信公众号文章链接，抓取并转为Markdown/纯文本，提取标题、作者、公众号名称、发布时间，下载正文图片资源。

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/wechat-archive` |
| Method | `POST` |
| 分类 | 内容娱乐 (`content`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 1 / s |
| 每日免费额度 | 登录用户 1000 次；匿名 100 次 |
| 增长层级 | D-inventory |
| 推广动作 | 先收录 |
| 推广角度 | 先放入接口库，等站内调用量或搜索需求上来后再写专项内容。 |
| 官网文档 | https://apizero.cn/aidocs/wechat-archive |
| Marketplace | https://apizero.cn/marketplace/wechat-archive |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Header 参数

| Header | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| Authorization | string | 是 |  |  |

### Request Body

- Content-Type：`application/json`
- Body 必填：是

| 字段 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| url | string | 是 | 微信公众号文章链接 | https://mp.weixin.qq.com/s/hy31xZK6FH3H51qh1zeSKA |
| format | string | 否 | 输出格式：markdown/text/both | both |
| timeout | number | 否 | 超时秒数 | 20 |

请求体示例：

```json
{
  "url": "https://mp.weixin.qq.com/s/hy31xZK6FH3H51qh1zeSKA",
  "format": "both",
  "timeout": "20"
}
```

### cURL 模板

```bash
curl -sS \
  -X POST \
  -H "X-API-Key: $APIZERO_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"url": "https://mp.weixin.qq.com/s/hy31xZK6FH3H51qh1zeSKA", "format": "both", "timeout": "20"}' \
  "https://v1.apizero.cn/api/wechat-archive"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "meta": {
      "title": "GitHub史上最快破10万星项目来了",
      "author": "作者名",
      "account_name": "公众号名",
      "publish_time": "2026-05-01T10:00:00+08:00",
      "read_num": null,
      "like_num": null
    },
    "content": {
      "markdown": "# 文章标题\\n\\n正文...",
      "text": "文章标题\\n\\n正文..."
    },
    "images": [
      {
        "url": "https://mmbiz.qpic.cn/...",
        "size_bytes": 45000
      }
    ]
  },
  "request_id": "req_abc123"
}
```

<a id="app-store"></a>

## 87. App Store 查询 `app-store`

输入 App 数字 ID 或应用商店链接，查询 App Store 应用的名称、开发者、版本、评分、价格、截图、兼容性等完整信息。

支持全球多国家/地区（cn/us/jp 等）查询，可用于应用竞品分析、上架状态监控、ASO 运营、开发者工具集成等场景。数据实时聚合、结构清晰，数值/日期已标准化。完全免费。

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/app-store` |
| Method | `POST` |
| 分类 | 开发工具 (`dev`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 10 / s |
| 每日免费额度 | 登录用户 1000 次；匿名 500 次 |
| 增长层级 | D-inventory |
| 推广动作 | 先收录 |
| 推广角度 | 先放入接口库，等站内调用量或搜索需求上来后再写专项内容。 |
| 官网文档 | https://apizero.cn/aidocs/app-store |
| Marketplace | https://apizero.cn/marketplace/app-store |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Header 参数

| Header | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| Content-Type | string | 是 | 请求体格式 |  |

### Request Body

- Content-Type：`application/json`
- Body 必填：否

| 字段 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| app_id | string | 否 | App 数字 ID（与 url 二选一，优先） | 414478124 |
| url | string | 否 | 应用商店链接，自动提取数字 ID（与 app_id 二选一） | https://apps.apple.com/cn/app/id414478124 |
| country | string | 否 | 两位国家/地区码，非法自动回退 cn | cn |

请求体示例：

```json
{
  "app_id": "414478124",
  "url": "https://apps.apple.com/cn/app/id414478124",
  "country": "cn"
}
```

### cURL 模板

```bash
curl -sS \
  -X POST \
  -H "X-API-Key: $APIZERO_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"app_id": "414478124", "url": "https://apps.apple.com/cn/app/id414478124", "country": "cn"}' \
  "https://v1.apizero.cn/api/app-store"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "app_id": 414478124,
    "name": "微信",
    "bundle_id": "com.tencent.xin",
    "developer": {
      "name": "WeChat",
      "seller_name": "Tencent Technology (Shenzhen) Company Limited",
      "url": "https://.../developer/..."
    },
    "icon": {
      "small": "https://.../60x60.jpg",
      "medium": "https://.../100x100.jpg",
      "large": "https://.../512x512.jpg"
    },
    "price": {
      "amount": 0,
      "currency": "CNY",
      "formatted": "免费",
      "is_free": true
    },
    "version": {
      "current": "8.0.75",
      "release_date": "2026-06-14",
      "release_notes": "本次更新：解决了一些已知问题。",
      "days_since_update": 17
    },
    "rating": {
      "average": 4.15,
      "count": 8010302,
      "current_version_average": 4.15,
      "current_version_count": 8010302
    },
    "category": {
      "primary": "Social Networking",
      "primary_id": 6005,
      "all": [
        "社交",
        "效率"
      ]
    },
    "compatibility": {
      "min_os_version": "15.0",
      "supported_devices_count": 128,
      "features": [
        "iosUniversal"
      ],
      "game_center": false
    },
    "content_rating": "12+",
    "advisories": [],
    "file_size": {
      "bytes": 851213312,
      "display": "811.78 MB"
    },
    "description": "微信是一个生活方式……",
    "screenshots": {
      "iphone": [
        "https://.../1.jpg"
      ],
      "ipad": [],
      "appletv": []
    },
    "languages": [
      "ZH",
      "EN",
      "JA"
    ],
    "language_count": 25,
    "store_url": "https://apps.apple.com/cn/app/id414478124",
    "first_release_date": "2011-01-21",
    "country": "CN"
  },
  "request_id": "a1b2c3d4"
}
```

<a id="stock-trend"></a>

## 88. A股实时行情 `stock-trend`

A股实时行情与分时数据分析接口，覆盖沪深北交易所全部 A 股。

输入 6 位股票代码即可获取：实时行情快照（最新价、涨跌额/涨跌幅、振幅、换手率、量比、市盈率、市净率、总市值/流通市值）、当日分时数据（逐分钟 开/高/低/收、均价、成交量、成交额）、以及技术分析（趋势研判、强弱评分、振幅分级、逐分钟涨跌分布、VWAP 偏离度）。支持 full/simple 两种粒度，数据稳定、字段规范，适合行情展示、量化选股与投研场景。

免费额度：未登录每日 5 次；登录用户每日 50 次免费，超出部分按 ¥0.01/次计费（可用余额抵扣）；开通会员享更高额度与更大并发。

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/stock-trend` |
| Method | `POST` |
| 分类 | 金融数据 (`finance`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 5 / s |
| 每日免费额度 | 登录用户 50 次；匿名 5 次 |
| 增长层级 | D-inventory |
| 推广动作 | 先收录 |
| 推广角度 | 先放入接口库，等站内调用量或搜索需求上来后再写专项内容。 |
| 官网文档 | https://apizero.cn/aidocs/stock-trend |
| Marketplace | https://apizero.cn/marketplace/stock-trend |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Header 参数

| Header | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| Authorization | string | 否 | Bearer <你的 API Key>（登录用户额度更高） |  |

### Request Body

- Content-Type：`application/json`
- Body 必填：是

| 字段 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| code | string | 是 | 6 位股票代码，可带 sh/sz 前缀 | 600519 |
| type | string | 否 | 返回粒度：full=完整分析（默认）/ simple=精简核心行情 | full |
| limit | number | 否 | 分时点返回数量，0=全部（默认），最大 240，>0 返回最近 N 个 | 30 |

请求体示例：

```json
{
  "code": "600519",
  "type": "full",
  "limit": "30"
}
```

### cURL 模板

```bash
curl -sS \
  -X POST \
  -H "X-API-Key: $APIZERO_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"code": "600519", "type": "full", "limit": "30"}' \
  "https://v1.apizero.cn/api/stock-trend"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "stock": {
      "code": "600519",
      "name": "贵州茅台",
      "market": "SH",
      "market_name": "上海证券交易所",
      "board": "主板",
      "trade_status": "交易中",
      "trade_date": "2026-07-01",
      "update_time": "2026-07-01 10:44"
    },
    "quote": {
      "price": 1190,
      "open": 1180.1,
      "high": 1191.99,
      "low": 1166.33,
      "pre_close": 1185.49,
      "avg_price": 1180.64,
      "change": 4.51,
      "change_percent": 0.38,
      "amplitude": 2.16,
      "turnover_rate": 0.15,
      "volume_ratio": 1.19,
      "volume": 18422,
      "volume_display": "1.84万手",
      "amount": 2168779690,
      "amount_display": "21.69亿",
      "pe_ttm": 13.65,
      "pb": 6.31,
      "total_mv": 1487597105190,
      "total_mv_display": "1.49万亿"
    },
    "change_status": {
      "direction": "up",
      "label": "上涨",
      "color": "#EB5454"
    },
    "analysis": {
      "trend": "震荡上行",
      "trend_direction": "up",
      "amplitude_level": "小幅波动",
      "strength": {
        "level": "较弱",
        "score": 2,
        "max_score": 5
      },
      "up_minutes": 40,
      "down_minutes": 30,
      "flat_minutes": 5,
      "up_ratio": 53.3,
      "vwap_deviation": 0.79
    },
    "minute": {
      "count": 75,
      "total": 241,
      "list": [
        {
          "time": "09:31",
          "price": 1184.88,
          "open": 1180.1,
          "high": 1184.95,
          "low": 1177.5,
          "avg": 1180.64,
          "volume": 562,
          "amount": 66354042,
          "change_percent": -0.05
        }
      ]
    },
    "summary": "贵州茅台今日上涨0.38%，现价1190.00元，振幅2.16%，换手0.15%，整体呈震荡上行态势，波动强度较弱。"
  },
  "request_id": "abc123"
}
```

<a id="air-history"></a>

## 89. 历史空气质量 `air-history`

按城市 + 年月查询历史空气质量档案：逐日 AQI 与六项污染物（PM2.5/PM10/SO₂/NO₂/CO/O₃）浓度、空气质量等级、首要污染物、全国排名，并给出月度汇总（均值、最佳/最差日、优良天数分布）。

可查范围 2013 年 1 月至当前月，覆盖全国主要城市。首要污染物按国标 HJ 633-2012 IAQI 分指数计算。数据仅供参考。

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/air-history` |
| Method | `POST` |
| 分类 | 生活服务 (`life`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 5 / s |
| 每日免费额度 | 登录用户 50 次；匿名 5 次 |
| 增长层级 | D-inventory |
| 推广动作 | 先收录 |
| 推广角度 | 先放入接口库，等站内调用量或搜索需求上来后再写专项内容。 |
| 官网文档 | https://apizero.cn/aidocs/air-history |
| Marketplace | https://apizero.cn/marketplace/air-history |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Header 参数

| Header | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| Authorization | string | 否 | Bearer <你的 API Key>（登录调用可提升额度/享会员免费；匿名可不传） |  |
| Content-Type | string | 否 | 请求体格式 |  |

### Request Body

- Content-Type：`application/json`
- Body 必填：是

| 字段 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| city | string | 是 | 城市中文名 | 北京 |
| month | string | 是 | 年月，格式 YYYYMM，可查 2013-01 至当前月 | 202503 |

请求体示例：

```json
{
  "city": "北京",
  "month": "202503"
}
```

### cURL 模板

```bash
curl -sS \
  -X POST \
  -H "X-API-Key: $APIZERO_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"city": "北京", "month": "202503"}' \
  "https://v1.apizero.cn/api/air-history"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "meta": {
      "city": "北京",
      "month": "202412",
      "month_format": "2024年12月",
      "total_days": 31
    },
    "summary": {
      "aqi_avg": 58.3,
      "aqi_avg_level": "良",
      "best_day": {
        "date": "2024-12-05",
        "aqi": 28,
        "level": "优"
      },
      "worst_day": {
        "date": "2024-12-20",
        "aqi": 168,
        "level": "中度污染"
      },
      "days_distribution": {
        "excellent": 8,
        "good": 18,
        "light": 3,
        "medium": 2,
        "heavy": 0,
        "severe": 0
      }
    },
    "daily": [
      {
        "date": "2024-12-01",
        "aqi": 53,
        "level": "良",
        "primary_pollutant": "细颗粒物(PM2.5)",
        "rank": 123,
        "pollutants": {
          "pm2_5": 26,
          "pm10": 55,
          "so2": 4,
          "no2": 35,
          "co": 0.5,
          "o3": 52
        }
      }
    ]
  },
  "request_id": "abc123"
}
```

<a id="mingyan"></a>

## 90. 名人名言 `mingyan`

返回一条名人名言，支持按类型筛选。action=types 可获取所有类型列表。

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/mingyan` |
| Method | `POST` |
| 分类 | 内容娱乐 (`content`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 5 / s |
| 每日免费额度 | 登录用户 50 次；匿名 30 次 |
| 增长层级 | D-inventory |
| 推广动作 | 先收录 |
| 推广角度 | 先放入接口库，等站内调用量或搜索需求上来后再写专项内容。 |
| 官网文档 | https://apizero.cn/aidocs/mingyan |
| Marketplace | https://apizero.cn/marketplace/mingyan |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Request Body

- Content-Type：`application/json`
- Body 必填：否

| 字段 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| action | string | 否 | 可选：'types' 列出所有类型 |  |
| typeid | string | 否 | 名言类型 ID（数字） |  |

### cURL 模板

```bash
curl -sS \
  -X POST \
  -H "X-API-Key: $APIZERO_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"action": "<action>", "typeid": "<typeid>"}' \
  "https://v1.apizero.cn/api/mingyan"
```

### 响应示例 `200`

```json
{
  "code": 200,
  "message": "success",
  "data": {}
}
```

<a id="image-nsfw"></a>

## 91. 图片鉴黄 `image-nsfw`

图片NSFW内容检测。支持本地NudeNet推理（零调用费）及百度/腾讯/阿里云端三家，统一返回 decision（block/review/pass）+ 分类得分 + 检测框。

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/image-nsfw` |
| Method | `POST` |
| 分类 | AI 能力 (`ai`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 2 / s |
| 每日免费额度 | 登录用户 100 次；匿名 10 次 |
| 增长层级 | D-inventory |
| 推广动作 | 先收录 |
| 推广角度 | 先放入接口库，等站内调用量或搜索需求上来后再写专项内容。 |
| 官网文档 | https://apizero.cn/aidocs/image-nsfw |
| Marketplace | https://apizero.cn/marketplace/image-nsfw |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Request Body

- Content-Type：`application/json`
- Body 必填：否

| 字段 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| image_url | string | 否 | 图片HTTP(S) URL（与image_b64二选一） | https://example.com/photo.jpg |
| image_b64 | string | 否 | 图片base64（兼容data:URI） |  |
| backend | string | 否 | 检测后端：auto/nudenet/baidu/tencent/aliyun | auto |
| timeout | number | 否 | 超时秒（3~60） |  |

请求体示例：

```json
{
  "image_url": "https://example.com/photo.jpg",
  "backend": "auto",
  "timeout": ""
}
```

### cURL 模板

```bash
curl -sS \
  -X POST \
  -H "X-API-Key: $APIZERO_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"image_url": "https://example.com/photo.jpg", "backend": "auto", "timeout": ""}' \
  "https://v1.apizero.cn/api/image-nsfw"
```

### 响应示例 `200`

```json
{
  "code": 200,
  "desc": "success",
  "backend": "nudenet",
  "elapsed_ms": 115,
  "input": {
    "type": "url",
    "source": "https://i0.hdslb.com/bfs/article/922692ab728ebb5f284f64296f3590a0959110a3.jpg",
    "size_bytes": 993377,
    "sha256": "f9a414bd6925f6c870e18d75252cbd764ce964fbd20387639577c9455db5a598",
    "mime": "image/jpeg",
    "width": 1080,
    "height": 1920
  },
  "decision": "pass",
  "label": "normal",
  "score": 0.8511,
  "categories": {
    "porn": 0,
    "sexy": 0,
    "normal": 1
  },
  "detections": [
    {
      "cls": "FACE_FEMALE",
      "score": 0.8511,
      "box": [
        381,
        291,
        528,
        575
      ]
    }
  ],
  "raw": {
    "nudenet": [
      {
        "class": "FACE_FEMALE",
        "score": 0.8510541915893555,
        "box": [
          381,
          291,
          528,
          575
        ]
      }
    ]
  },
  "notes": [],
  "tips": "极数本源 · https://apizero.cn"
}
```

<a id="2fa"></a>

## 92. 2FA 动态验证码 `2fa`

2FA 动态验证码（TOTP）生成与校验，兼容 Google Authenticator 等主流身份验证器。

基于 RFC 6238 标准由本平台自建实现，支持「生成 / 验证 / 批量」三种模式，可自定义验证码位数（4~8）与有效周期（10~120 秒）。密钥仅参与计算、不存储、不回显，验证采用恒定时间比较防时序攻击。接口完全免费，支持匿名调用。

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/2fa` |
| Method | `POST` |
| 分类 | 开发工具 (`dev`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 20 / s |
| 每日免费额度 | 登录用户 1000 次；匿名 200 次 |
| 增长层级 | D-inventory |
| 推广动作 | 先收录 |
| 推广角度 | 先放入接口库，等站内调用量或搜索需求上来后再写专项内容。 |
| 官网文档 | https://apizero.cn/aidocs/2fa |
| Marketplace | https://apizero.cn/marketplace/2fa |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Header 参数

| Header | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| Authorization | string | 否 | Bearer <你的 API Key>（可选，登录用户额度更高） |  |
| Content-Type | string | 是 | 请求体格式 |  |

### Request Body

- Content-Type：`application/json`
- Body 必填：是

| 字段 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| secret | string | 是 | Base32 编码的 TOTP 密钥（自动忽略空格与连字符，不区分大小写） | JBSWY3DPEHPK3PXP |
| action | string | 否 | 操作类型：generate=生成当前验证码 / verify=校验验证码 / batch=批量返回上一·当前·下一周期 |  |
| code | string | 否 | action=verify 时必填，待校验的验证码 | 123456 |
| period | number | 否 | 验证码有效期（秒），范围 10~120 |  |
| digits | number | 否 | 验证码位数，范围 4~8 |  |

请求体示例：

```json
{
  "secret": "JBSWY3DPEHPK3PXP",
  "code": "123456"
}
```

### cURL 模板

```bash
curl -sS \
  -X POST \
  -H "X-API-Key: $APIZERO_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"secret": "JBSWY3DPEHPK3PXP", "code": "123456"}' \
  "https://v1.apizero.cn/api/2fa"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "totp_code": "123456",
    "remaining_seconds": 17,
    "period": 30,
    "digits": 6,
    "timestamp": 1782000000,
    "next_refresh": "2026-06-30 12:00:30"
  },
  "request_id": "abc123"
}
```

<a id="carrier-3c"></a>

## 93. 运营商三要素核验 `carrier-3c`

运营商三要素实名核验：一次校验「姓名 + 手机号 + 身份证号」三者是否一致，返回核验结论。

适用于账户实名、风控准入、身份一致性校验等场景。仅返回是否一致的结论，不返回、不存储任何明文个人信息；接口需登录并按次计费，请在取得信息主体明确授权后合规调用。

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/carrier-3c` |
| Method | `POST` |
| 分类 | 身份核验 (`kyc`) |
| 计费 | 按次付费 · 单次 0 积分 |
| QPS | 5 / s |
| 每日免费额度 | 登录用户 0 次；匿名 0 次 |
| 增长层级 | D-inventory |
| 推广动作 | 先收录 |
| 推广角度 | 先放入接口库，等站内调用量或搜索需求上来后再写专项内容。 |
| 官网文档 | https://apizero.cn/aidocs/carrier-3c |
| Marketplace | https://apizero.cn/marketplace/carrier-3c |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Header 参数

| Header | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| Authorization | string | 是 | Bearer <你的 API Key> |  |
| Content-Type | string | 否 | 请求体格式 |  |

### Request Body

- Content-Type：`application/json`
- Body 必填：是

| 字段 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| name | string | 是 | 真实姓名（中文）；兼容别名 realname / xm | 张三 |
| mobile | string | 是 | 11 位手机号；兼容别名 phone / sj | 13800138000 |
| idcard | string | 是 | 18 位身份证号（末位兼容 X）；兼容别名 id_card / sf | 110101199001011234 |

请求体示例：

```json
{
  "name": "张三",
  "mobile": "13800138000",
  "idcard": "110101199001011234"
}
```

### cURL 模板

```bash
curl -sS \
  -X POST \
  -H "X-API-Key: $APIZERO_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"name": "张三", "mobile": "13800138000", "idcard": "110101199001011234"}' \
  "https://v1.apizero.cn/api/carrier-3c"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "name": "张三",
    "mobile": "138****0000",
    "idcard": "110***********001X",
    "match": true,
    "result": "三要素一致"
  },
  "request_id": "abc123"
}
```

<a id="ocr-vehicle-license"></a>

## 94. 行驶证识别 `ocr-vehicle-license`

结构化识别行驶证，提取号牌号码、车辆类型、所有人、住址、品牌型号、VIN、发动机号、注册日期、总质量、核定载质量、外廓尺寸、检验记录等 20 个字段。

适用场景：二手车交易核验、车辆过户信息录入、保险投保材料自动提取等。含所有人姓名及住址，仅限已登录用户调用，匿名访问不开放。

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/ocr-vehicle-license` |
| Method | `POST` |
| 分类 | 文档识别 (`ocrdata`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 2 / s |
| 每日免费额度 | 登录用户 20 次；匿名 0 次 |
| 增长层级 | D-inventory |
| 推广动作 | 先收录 |
| 推广角度 | 先放入接口库，等站内调用量或搜索需求上来后再写专项内容。 |
| 官网文档 | https://apizero.cn/aidocs/ocr-vehicle-license |
| Marketplace | https://apizero.cn/marketplace/ocr-vehicle-license |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Header 参数

| Header | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| Authorization | string | 是 | Bearer <你的 API Key> |  |
| Content-Type | string | 否 | 请求体格式 |  |

### Request Body

- Content-Type：`application/json`
- Body 必填：是

| 字段 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| input_type | string | 是 | 图片传输方式，支持 url（公网图片地址）或 base64（图片的 base64 编码） | url |
| input_data | string | 是 | 图片内容：input_type=url 时填 http/https 图片链接；input_type=base64 时填 base64 字符串（可含 data:image/xxx;base64, 前缀） | https://example.com/vehicle-license.jpg |

请求体示例：

```json
{
  "input_type": "url",
  "input_data": "https://example.com/vehicle-license.jpg"
}
```

### cURL 模板

```bash
curl -sS \
  -X POST \
  -H "X-API-Key: $APIZERO_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"input_type": "url", "input_data": "https://example.com/vehicle-license.jpg"}' \
  "https://v1.apizero.cn/api/ocr-vehicle-license"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "plate_number": "沪A12345",
    "vehicle_type": "小型轿车",
    "owner": "张三",
    "address": "上海市XX路XX号",
    "use_character": "非营运",
    "model": "丰田CAMRY",
    "vin": "LSXXXXXXXXXXXXXXXXX",
    "engine_number": "4A123456",
    "register_date": "2020-06-01",
    "issue_date": "2020-06-05",
    "license_issuing_authority": "上海市公安局交通警察总队",
    "document_id": "SHXXXXXXXXXX",
    "approved_passengers_capacity": "5人",
    "total_mass": "1945",
    "curb_weight": "1495",
    "ratified_load_capacity": "",
    "gabarite": "4885x1840x1455",
    "traction_mass": "",
    "remarks": "",
    "inspection_record": "2022 2024"
  },
  "request_id": "req_abc123"
}
```

<a id="car-owner-check"></a>

## 95. 车牌车主核验 `car-owner-check`

车牌号与车主姓名一致性核验，快速判断二者是否匹配。

适用于二手车交易、租车、物流承运、金融风控等场景的车辆归属初步核验。仅返回「相符 / 不符」结论，不返回车主任何隐私详情。本接口为身份核验类敏感能力，需登录并携带 API Key 调用，按次计费。

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/car-owner-check` |
| Method | `POST` |
| 分类 | 身份核验 (`kyc`) |
| 计费 | 按次付费 · 单次 0 积分 |
| QPS | 5 / s |
| 每日免费额度 | 登录用户 0 次；匿名 0 次 |
| 增长层级 | D-inventory |
| 推广动作 | 先收录 |
| 推广角度 | 先放入接口库，等站内调用量或搜索需求上来后再写专项内容。 |
| 官网文档 | https://apizero.cn/aidocs/car-owner-check |
| Marketplace | https://apizero.cn/marketplace/car-owner-check |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Header 参数

| Header | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| Authorization | string | 是 | Bearer <你的 API Key> |  |
| Content-Type | string | 否 | 请求体格式 |  |

### Request Body

- Content-Type：`application/json`
- Body 必填：是

| 字段 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| cp | string | 是 | 车牌号（中文省份简称开头 + 6~7 位字母数字）；兼容别名 plate | 京A12345 |
| m | string | 是 | 车主姓名；兼容别名 name / owner | 张三 |

请求体示例：

```json
{
  "cp": "京A12345",
  "m": "张三"
}
```

### cURL 模板

```bash
curl -sS \
  -X POST \
  -H "X-API-Key: $APIZERO_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"cp": "京A12345", "m": "张三"}' \
  "https://v1.apizero.cn/api/car-owner-check"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "plate": "京A12345",
    "owner": "张三",
    "matched": true,
    "result": "此车牌号与车主相符"
  },
  "request_id": "abc123"
}
```

<a id="driving-license"></a>

## 96. 驾驶证识别 `driving-license`

上传驾驶证图片（URL 或 base64），自动识别证件字段，包括证号、姓名、性别、国籍、住址、出生日期、准驾车型、有效期等共 12 个关键字段信息。

适用于网约车平台司机资质核验、二手车交易身份核实、物流企业驾照信息录入等场景。支持 JPG / PNG / BMP，图片建议清晰、无遮挡。

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/driving-license` |
| Method | `POST` |
| 分类 | 文档识别 (`ocrdata`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 2 / s |
| 每日免费额度 | 登录用户 20 次；匿名 0 次 |
| 增长层级 | D-inventory |
| 推广动作 | 先收录 |
| 推广角度 | 先放入接口库，等站内调用量或搜索需求上来后再写专项内容。 |
| 官网文档 | https://apizero.cn/aidocs/driving-license |
| Marketplace | https://apizero.cn/marketplace/driving-license |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Header 参数

| Header | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| Authorization | string | 是 | Bearer <你的 API Key> |  |
| Content-Type | string | 是 | 请求体格式 |  |

### Request Body

- Content-Type：`application/json`
- Body 必填：是

| 字段 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| input_type | string | 是 | 图片传入方式 | url |
| input_data | string | 是 | 图片 URL 或 base64 编码字符串（base64 最大 5 MB） | https://example.com/driving-license.jpg |

请求体示例：

```json
{
  "input_type": "url",
  "input_data": "https://example.com/driving-license.jpg"
}
```

### cURL 模板

```bash
curl -sS \
  -X POST \
  -H "X-API-Key: $APIZERO_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"input_type": "url", "input_data": "https://example.com/driving-license.jpg"}' \
  "https://v1.apizero.cn/api/driving-license"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "id_number": "310***********1234",
    "name": "张三",
    "sex": "男",
    "nationality": "中国",
    "address": "上海市浦东新区",
    "date_of_birth": "1990-01-01",
    "date_of_first_issue": "2010-05-20",
    "class": "C1",
    "valid_begin": "2020-05-20",
    "valid_end": "2026-05-20",
    "license_issuing_authority": "上海市公安局交通警察总队",
    "id_photo_location": "{\"x\":10,\"y\":10,\"w\":80,\"h\":100}"
  },
  "request_id": "req_abc123"
}
```

<a id="ocr-bank-card"></a>

## 97. 银行卡识别 `ocr-bank-card`

识别银行卡图片中的卡号与有效期，支持 URL 和 base64 两种输入方式。

适用场景：在线开户填卡、支付绑卡辅助录入、卡号核对等。支持 jpg/png 格式，文件大小不超过 10MB，建议图片清晰、卡面完整以提高识别率。

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/ocr-bank-card` |
| Method | `POST` |
| 分类 | 文档识别 (`ocrdata`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 2 / s |
| 每日免费额度 | 登录用户 20 次；匿名 0 次 |
| 增长层级 | D-inventory |
| 推广动作 | 先收录 |
| 推广角度 | 先放入接口库，等站内调用量或搜索需求上来后再写专项内容。 |
| 官网文档 | https://apizero.cn/aidocs/ocr-bank-card |
| Marketplace | https://apizero.cn/marketplace/ocr-bank-card |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Header 参数

| Header | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| Authorization | string | 是 | Bearer <你的 API Key> |  |
| Content-Type | string | 否 | 请求体格式 |  |

### Request Body

- Content-Type：`application/json`
- Body 必填：是

| 字段 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| input_type | string | 是 | 图片传输方式，支持 url（公网图片地址）或 base64（图片二进制的 base64 编码） | url |
| input_data | string | 是 | 图片内容：input_type=url 时填写 http/https 图片链接；input_type=base64 时填写 base64 字符串（可含 data:image/xxx;base64, 前缀），文件大小不超过 10MB | https://example.com/bankcard.jpg |

请求体示例：

```json
{
  "input_type": "url",
  "input_data": "https://example.com/bankcard.jpg"
}
```

### cURL 模板

```bash
curl -sS \
  -X POST \
  -H "X-API-Key: $APIZERO_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"input_type": "url", "input_data": "https://example.com/bankcard.jpg"}' \
  "https://v1.apizero.cn/api/ocr-bank-card"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "card_number": "6222 0202 0001 5920 8",
    "date_of_expiry": "12/28"
  },
  "request_id": "req_abc123"
}
```

<a id="ocr-cn-passport"></a>

## 98. 中国护照识别 `ocr-cn-passport`

结构化识别中国护照，提取护照号码、中英文姓名、出生日期、有效期至、签发地点共 6 个关键字段。

适用场景：出行实名核验、酒店/机构入住登记、跨境业务证件录入等。护照属于高敏感度身份证件，仅限已登录用户调用，匿名访问不开放。

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/ocr-cn-passport` |
| Method | `POST` |
| 分类 | 文档识别 (`ocrdata`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 2 / s |
| 每日免费额度 | 登录用户 20 次；匿名 0 次 |
| 增长层级 | D-inventory |
| 推广动作 | 先收录 |
| 推广角度 | 先放入接口库，等站内调用量或搜索需求上来后再写专项内容。 |
| 官网文档 | https://apizero.cn/aidocs/ocr-cn-passport |
| Marketplace | https://apizero.cn/marketplace/ocr-cn-passport |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Header 参数

| Header | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| Authorization | string | 是 | Bearer <你的 API Key> |  |
| Content-Type | string | 否 | 请求体格式 |  |

### Request Body

- Content-Type：`application/json`
- Body 必填：是

| 字段 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| input_type | string | 是 | 图片传输方式，支持 url（公网图片地址）或 base64（图片的 base64 编码） | url |
| input_data | string | 是 | 图片内容：input_type=url 时填 http/https 图片链接；input_type=base64 时填 base64 字符串（可含 data:image/xxx;base64, 前缀） | https://example.com/passport.jpg |

请求体示例：

```json
{
  "input_type": "url",
  "input_data": "https://example.com/passport.jpg"
}
```

### cURL 模板

```bash
curl -sS \
  -X POST \
  -H "X-API-Key: $APIZERO_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"input_type": "url", "input_data": "https://example.com/passport.jpg"}' \
  "https://v1.apizero.cn/api/ocr-cn-passport"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "passport_number": "E12345678",
    "full_name_cn": "张三",
    "full_name_en": "ZHANG SAN",
    "date_of_birth": "1990-01-01",
    "date_of_expiry": "2034-12-31",
    "place_of_issue": "上海"
  },
  "request_id": "req_abc123"
}
```

<a id="ocr-health-cert"></a>

## 99. 健康证识别 `ocr-health-cert`

结构化识别健康证，提取姓名、发证机关、办证日期、发证日期、体检日期、有效日期共 6 个关键字段。

适用场景：餐饮/食品行业从业人员健康证核验、HR 入职材料自动录入、证件有效期批量管理等。支持 URL 和 base64 两种输入方式，jpg/png 格式，建议证件图片完整清晰。

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/ocr-health-cert` |
| Method | `POST` |
| 分类 | 文档识别 (`ocrdata`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 2 / s |
| 每日免费额度 | 登录用户 20 次；匿名 0 次 |
| 增长层级 | D-inventory |
| 推广动作 | 先收录 |
| 推广角度 | 先放入接口库，等站内调用量或搜索需求上来后再写专项内容。 |
| 官网文档 | https://apizero.cn/aidocs/ocr-health-cert |
| Marketplace | https://apizero.cn/marketplace/ocr-health-cert |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Header 参数

| Header | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| Authorization | string | 是 | Bearer <你的 API Key> |  |
| Content-Type | string | 否 | 请求体格式 |  |

### Request Body

- Content-Type：`application/json`
- Body 必填：是

| 字段 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| input_type | string | 是 | 图片传输方式，支持 url（公网图片地址）或 base64（图片的 base64 编码） | url |
| input_data | string | 是 | 图片内容：input_type=url 时填 http/https 图片链接；input_type=base64 时填 base64 字符串（可含 data:image/xxx;base64, 前缀） | https://example.com/health-cert.jpg |

请求体示例：

```json
{
  "input_type": "url",
  "input_data": "https://example.com/health-cert.jpg"
}
```

### cURL 模板

```bash
curl -sS \
  -X POST \
  -H "X-API-Key: $APIZERO_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"input_type": "url", "input_data": "https://example.com/health-cert.jpg"}' \
  "https://v1.apizero.cn/api/ocr-health-cert"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "name": "张三",
    "issued_by": "XX市卫生健康委员会",
    "date_of_handling": "2024-01-15",
    "date_of_issue": "2024-01-20",
    "date_of_medical_examination": "2024-01-10",
    "valid_date": "2025-01-19"
  },
  "request_id": "req_abc123"
}
```

<a id="ocr-train-ticket"></a>

## 100. 火车票识别 `ocr-train-ticket`

结构化识别国内全类型火车票（含高铁、动车、普通车票），提取出发/到达站、车次、姓名、座位号、票价、出发时间、身份证号、售卖站等 13 个字段。

适用场景：差旅费报销自动录入、行程管理、票据核对等。因返回内容含姓名和身份证号，仅限已登录用户调用，匿名访问不开放。

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/ocr-train-ticket` |
| Method | `POST` |
| 分类 | 文档识别 (`ocrdata`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 2 / s |
| 每日免费额度 | 登录用户 20 次；匿名 0 次 |
| 增长层级 | D-inventory |
| 推广动作 | 先收录 |
| 推广角度 | 先放入接口库，等站内调用量或搜索需求上来后再写专项内容。 |
| 官网文档 | https://apizero.cn/aidocs/ocr-train-ticket |
| Marketplace | https://apizero.cn/marketplace/ocr-train-ticket |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Header 参数

| Header | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| Authorization | string | 是 | Bearer <你的 API Key> |  |
| Content-Type | string | 否 | 请求体格式 |  |

### Request Body

- Content-Type：`application/json`
- Body 必填：是

| 字段 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| input_type | string | 是 | 图片传输方式，支持 url（公网图片地址）或 base64（图片的 base64 编码） | url |
| input_data | string | 是 | 图片内容：input_type=url 时填 http/https 图片链接；input_type=base64 时填 base64 字符串（可含 data:image/xxx;base64, 前缀） | https://example.com/train-ticket.jpg |

请求体示例：

```json
{
  "input_type": "url",
  "input_data": "https://example.com/train-ticket.jpg"
}
```

### cURL 模板

```bash
curl -sS \
  -X POST \
  -H "X-API-Key: $APIZERO_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"input_type": "url", "input_data": "https://example.com/train-ticket.jpg"}' \
  "https://v1.apizero.cn/api/ocr-train-ticket"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "start_station": "北京南",
    "end_station": "上海虹桥",
    "train_num": "G101",
    "time": "2024-01-15 09:00",
    "seat_cls": "二等座",
    "seat_num": "05车12A号",
    "name": "张三",
    "id_num": "110101199001011234",
    "ticket_num": "E123456789",
    "total_amount": "￥553.00",
    "price": "553.00",
    "sale_station": "北京南",
    "sale_num": "G123456"
  },
  "request_id": "req_abc123"
}
```

<a id="hot-search"></a>

## 101. 全网热搜聚合 `hot-search`

聚合微博热搜、知乎热榜、B站日榜、百度贴吧热议，一次请求获取全网热门话题。支持按平台筛选和条数限制。

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/hot-search` |
| Method | `POST` |
| 分类 | 内容娱乐 (`content`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 3 / s |
| 每日免费额度 | 登录用户 3000 次；匿名 2000 次 |
| 增长层级 | D-inventory |
| 推广动作 | 先收录 |
| 推广角度 | 先放入接口库，等站内调用量或搜索需求上来后再写专项内容。 |
| 官网文档 | https://apizero.cn/aidocs/hot-search |
| Marketplace | https://apizero.cn/marketplace/hot-search |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Header 参数

| Header | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| Authorization | string | 是 |  |  |

### Request Body

- Content-Type：`application/json`
- Body 必填：否

| 字段 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| platform | string | 否 | 平台筛选：all/weibo/zhihu/bilibili/tieba，逗号分隔 | all |
| limit | number | 否 | 每个平台返回条数，最大50 | 10 |
| timeout | number | 否 | 请求超时秒数 | 15 |

请求体示例：

```json
{
  "platform": "all",
  "limit": "10",
  "timeout": "15"
}
```

### cURL 模板

```bash
curl -sS \
  -X POST \
  -H "X-API-Key: $APIZERO_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"platform": "all", "limit": "10", "timeout": "15"}' \
  "https://v1.apizero.cn/api/hot-search"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "generated_at": "2026-05-08T13:00:00+08:00",
    "requested_platforms": [
      "weibo",
      "zhihu",
      "bilibili",
      "tieba"
    ],
    "limit_per_platform": 10,
    "total_items": 40,
    "failed_platforms": {},
    "platforms": {
      "weibo": {
        "name": "微博热搜",
        "status": "success",
        "count": 10,
        "items": [
          {
            "rank": 1,
            "title": "热搜标题",
            "hot": "5234567"
          }
        ]
      }
    }
  },
  "request_id": "req_abc123"
}
```

<a id="ocr-taxi-invoice"></a>

## 102. 出租车发票识别 `ocr-taxi-invoice`

结构化识别全国出租车机打发票，提取车号、上下车时间、里程、金额、发票代码/号码、单价、附加费、印章信息等 16 个字段。

适用场景：差旅费报销自动录入、行程费用核对、财务审计等。支持 URL 和 base64 两种输入方式，发票图片建议拍摄平整清晰。

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/ocr-taxi-invoice` |
| Method | `POST` |
| 分类 | 文档识别 (`ocrdata`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 2 / s |
| 每日免费额度 | 登录用户 20 次；匿名 0 次 |
| 增长层级 | D-inventory |
| 推广动作 | 先收录 |
| 推广角度 | 先放入接口库，等站内调用量或搜索需求上来后再写专项内容。 |
| 官网文档 | https://apizero.cn/aidocs/ocr-taxi-invoice |
| Marketplace | https://apizero.cn/marketplace/ocr-taxi-invoice |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Header 参数

| Header | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| Authorization | string | 是 | Bearer <你的 API Key> |  |
| Content-Type | string | 否 | 请求体格式 |  |

### Request Body

- Content-Type：`application/json`
- Body 必填：是

| 字段 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| input_type | string | 是 | 图片传输方式，支持 url（公网图片地址）或 base64（图片的 base64 编码） | url |
| input_data | string | 是 | 图片内容：input_type=url 时填 http/https 图片链接；input_type=base64 时填 base64 字符串（可含 data:image/xxx;base64, 前缀） | https://example.com/taxi-invoice.jpg |

请求体示例：

```json
{
  "input_type": "url",
  "input_data": "https://example.com/taxi-invoice.jpg"
}
```

### cURL 模板

```bash
curl -sS \
  -X POST \
  -H "X-API-Key: $APIZERO_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"input_type": "url", "input_data": "https://example.com/taxi-invoice.jpg"}' \
  "https://v1.apizero.cn/api/ocr-taxi-invoice"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "invoice_code": "31100000000",
    "invoice_no": "87654321",
    "title_trial": "出租汽车发票",
    "car_number": "沪A12345",
    "start_date": "2024-01-15",
    "start_time": "09:30",
    "end_time": "10:05",
    "distance": "12.3",
    "total_amount": "38.50",
    "price": "3.00",
    "dispatch_fee": "",
    "additional_fee": "1.00",
    "is_sealed": "true",
    "seller_name_in_seal": "XX出租汽车有限公司",
    "seller_taxpayer_no_in_seal": "91310000XXXXXXXXXX",
    "invoice_special_seal": "XX出租汽车发票专用章"
  },
  "request_id": "req_abc123"
}
```

<a id="ocr-tw-permit"></a>

## 103. 台胞证识别 `ocr-tw-permit`

结构化识别台湾居民来往大陆通行证（台胞证），提取中文姓名、英文姓名、出生日期、证件号码、有效期共 5 个关键字段。

适用场景：台湾居民实名认证、酒店入住登记、证件信息自动录入等。涉及个人身份信息，仅限已登录用户调用，匿名访问不开放。支持 URL 和 base64 两种输入方式，jpg/png 格式，建议证件图片完整清晰。

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/ocr-tw-permit` |
| Method | `POST` |
| 分类 | 文档识别 (`ocrdata`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 2 / s |
| 每日免费额度 | 登录用户 20 次；匿名 0 次 |
| 增长层级 | D-inventory |
| 推广动作 | 先收录 |
| 推广角度 | 先放入接口库，等站内调用量或搜索需求上来后再写专项内容。 |
| 官网文档 | https://apizero.cn/aidocs/ocr-tw-permit |
| Marketplace | https://apizero.cn/marketplace/ocr-tw-permit |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Header 参数

| Header | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| Authorization | string | 是 | Bearer <你的 API Key> |  |
| Content-Type | string | 否 | 请求体格式 |  |

### Request Body

- Content-Type：`application/json`
- Body 必填：是

| 字段 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| input_type | string | 是 | 图片传输方式，支持 url（公网图片地址）或 base64（图片的 base64 编码） | url |
| input_data | string | 是 | 图片内容：input_type=url 时填 http/https 图片链接；input_type=base64 时填 base64 字符串（可含 data:image/xxx;base64, 前缀） | https://example.com/tw-permit.jpg |

请求体示例：

```json
{
  "input_type": "url",
  "input_data": "https://example.com/tw-permit.jpg"
}
```

### cURL 模板

```bash
curl -sS \
  -X POST \
  -H "X-API-Key: $APIZERO_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"input_type": "url", "input_data": "https://example.com/tw-permit.jpg"}' \
  "https://v1.apizero.cn/api/ocr-tw-permit"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "full_name_cn": "张三",
    "full_name_en": "ZHANG SAN",
    "date_of_birth": "1990-01-01",
    "date_of_expiry": "2029-12-31",
    "card_number": "12345678901234567"
  },
  "request_id": "req_abc123"
}
```

<a id="bili-danmaku"></a>

## 104. B站弹幕分析 `bili-danmaku`

输入B站BV/AV号，拉取视频弹幕并分析高频重复弹幕、热词/梗、整体情感倾向（正向/负向/中性）。

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/bili-danmaku` |
| Method | `POST` |
| 分类 | 内容娱乐 (`content`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 2 / s |
| 每日免费额度 | 登录用户 20 次；匿名 10 次 |
| 增长层级 | D-inventory |
| 推广动作 | 先收录 |
| 推广角度 | 先放入接口库，等站内调用量或搜索需求上来后再写专项内容。 |
| 官网文档 | https://apizero.cn/aidocs/bili-danmaku |
| Marketplace | https://apizero.cn/marketplace/bili-danmaku |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Header 参数

| Header | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| Authorization | string | 是 |  |  |

### Request Body

- Content-Type：`application/json`
- Body 必填：是

| 字段 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| video | string | 是 | AV号或BV号 | BV1w8RBBUEYy |
| limit | number | 否 | 返回高频弹幕/热词数量 | 10 |
| timeout | number | 否 | 超时秒数 | 15 |
| page_mode | string | 否 | all=抓全部分P / first=只抓第一页 | all |

请求体示例：

```json
{
  "video": "BV1w8RBBUEYy",
  "limit": "10",
  "timeout": "15",
  "page_mode": "all"
}
```

### cURL 模板

```bash
curl -sS \
  -X POST \
  -H "X-API-Key: $APIZERO_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"video": "BV1w8RBBUEYy", "limit": "10", "timeout": "15", "page_mode": "all"}' \
  "https://v1.apizero.cn/api/bili-danmaku"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "video": {
      "bvid": "BV1w8RBBUEYy",
      "title": "视频标题",
      "duration": 300
    },
    "danmaku_summary": {
      "total_count": 1500,
      "top_meme": "哈哈哈哈",
      "top_repeat_comments": [
        {
          "text": "哈哈哈哈",
          "count": 120
        }
      ],
      "top_terms": [
        {
          "term": "牛逼",
          "count": 200
        }
      ]
    },
    "sentiment_summary": {
      "label": "positive",
      "average_score": 0.72
    }
  },
  "request_id": "req_abc123"
}
```

<a id="dns-lookup"></a>

## 105. DNS 查询 `dns-lookup`

零依赖DNS查询API。自实现DNS协议（RFC 1035），支持A/AAAA/MX/TXT/CNAME/NS/SOA/SRV/PTR/CAA记录类型，UDP自动fallback到DoH（DNS over HTTPS）防劫持。

### 基础信息

| 字段 | 值 |
| --- | --- |
| Endpoint | `https://v1.apizero.cn/api/dns-lookup` |
| Method | `POST` |
| 分类 | 开发工具 (`dev`) |
| 计费 | 免费试用 · 单次 0 积分 |
| QPS | 5 / s |
| 每日免费额度 | 登录用户 500 次；匿名 300 次 |
| 增长层级 | B-support |
| 推广动作 | 补充推广 |
| 推广角度 | DNS 查询 API；适合开发者工具、域名诊断、运维脚本。 |
| 官网文档 | https://apizero.cn/aidocs/dns-lookup |
| Marketplace | https://apizero.cn/marketplace/dns-lookup |

### 鉴权

推荐使用 Header：`X-API-Key: $APIZERO_API_KEY`。匿名额度适合测试，正式集成建议创建 Key；部分文档也兼容 `?key=YOUR_API_KEY` 或 `Authorization`。

### Header 参数

| Header | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| Authorization | string | 是 |  |  |

### Request Body

- Content-Type：`application/json`
- Body 必填：是

| 字段 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| domain | string | 是 | 要查询的域名（别名 name/host） | example.com |
| type | string | 否 | 记录类型，逗号分隔；ALL=A,AAAA,MX,TXT,CNAME | A |
| server | string | 否 | 指定DNS服务器（逗号分隔） | 8.8.8.8,1.1.1.1 |
| timeout | number | 否 | 单请求超时秒（1~30） | 5 |
| doh | number | 否 | 1=强制走DoH（HTTPS） | 0 |
| doh_provider | string | 否 | DoH服务商：cloudflare/google/alibaba | cloudflare |

请求体示例：

```json
{
  "domain": "example.com",
  "type": "A",
  "server": "8.8.8.8,1.1.1.1",
  "timeout": "5",
  "doh": "0",
  "doh_provider": "cloudflare"
}
```

### cURL 模板

```bash
curl -sS \
  -X POST \
  -H "X-API-Key: $APIZERO_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"domain": "example.com", "type": "A", "server": "8.8.8.8,1.1.1.1", "timeout": "5", "doh": "0", "doh_provider": "cloudflare"}' \
  "https://v1.apizero.cn/api/dns-lookup"
```

### 响应示例 `200`

```json
{
  "code": 0,
  "msg": "成功",
  "data": {
    "domain": "example.com",
    "type": "A",
    "server": "114.114.114.114",
    "transport": "udp",
    "rcode": 0,
    "rcode_text": "NOERROR",
    "elapsed_ms": 32,
    "answers": [
      {
        "type": "A",
        "name": "example.com",
        "ttl": 300,
        "value": "93.184.216.34"
      }
    ],
    "truncated": false
  },
  "request_id": "req_abc123"
}
```
